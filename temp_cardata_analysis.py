...
log(f"總記錄數: {len(records)}")
log(f"比賽時長約 2 小時 = 7200 秒")
log(f"平均採樣率: {len(records) / 7200:.2f} Hz")

log("\n時間戳樣本 (連續 20 筆):")
for i, rec in enumerate(records[1000:1020]):
    log(f"  {i}: {rec['timestamp']}")
...
if intervals:
    ...
    log(f"  平均間隔: {avg_interval*1000:.1f} ms")
    log(f"  最小間隔: {min_interval*1000:.1f} ms")  
    log(f"  最大間隔: {max_interval*1000:.1f} ms")
    log(f"  採樣率: {1/avg_interval:.1f} Hz")
