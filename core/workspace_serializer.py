...
class WorkspaceSerializer:
    ...
    # 視窗類型映射（類別名稱 → 類型標識）
    WINDOW_TYPE_MAPPING = {
        # Temperature Analysis (原 Rain Analysis)
        "TempAnalysisModuleAdapter": "temp_analysis",
        "TempAnalysisModule": "temp_analysis",
        "TempAnalysisUniversal": "temp_analysis",
        # 向後相容舊名稱
        "RainAnalysisModuleAdapter": "temp_analysis",
        "RainAnalysisModule": "temp_analysis",
    ...
    def _find_analysis_widget(self, root_widget, max_depth: int = 5) -> Optional[Any]:
        ...
        def search(widget, depth):
            ...
            # 優先級 2: 檢查是否有 _temp_analysis_core（TempAnalysisModule 特有）
            if hasattr(widget, '_temp_analysis_core'):
                logger.debug(f"[WORKSPACE]    發現 _temp_analysis_core，深入檢查")
                core = widget._temp_analysis_core
                # 遞歸檢查 core
                result = search(core, depth + 1)
                if result:
                    return result
            # 向後相容: 舊版 _rain_analysis_core
            if hasattr(widget, '_rain_analysis_core'):
                logger.debug(f"[WORKSPACE]    發現 _rain_analysis_core (舊版)，深入檢查")
    ...
    def _rebuild_mdi_window(self, mdi_area, window_config: Dict) -> bool:
        ...
        try:
            ...
            if hasattr(analysis_module, 'set_parent_window'):
                ...
                logger.debug(f"[WORKSPACE]    模組沒有 set_parent_window 方法")
                # 檢查是否有內部的 MDI 實例
                if hasattr(analysis_module, '_temp_analysis_core'):
                    logger.debug(f"[WORKSPACE]    發現 _temp_analysis_core 屬性")
                    core = analysis_module._temp_analysis_core
                    if core and hasattr(core, 'set_parent_window'):
                        core.set_parent_window(analysis_window)
                        logger.debug(f"[WORKSPACE]    已在 _temp_analysis_core 設置 parent_window")
                # 向後相容
                elif hasattr(analysis_module, '_rain_analysis_core'):
                    logger.debug(f"[WORKSPACE]    發現 _rain_analysis_core 屬性 (舊版)")
    ...
    def _create_module_instance(self, window_type: str, parameters: Dict):
        ...
        try:
            ...
            # Temperature Analysis (原 Rain Analysis，支援多種類型名稱)
            if window_type in ("temp_analysis", "temp_weather", "rain_analysis", "rain_weather"):
                from modules.gui.race_analysis.temp.temp_analysis_module import TempAnalysisModule
                module = TempAnalysisModule(
                    year=year,
                    race=race,
                    session=session
                )
                logger.debug(f"[WORKSPACE] Temp Analysis 模組已創建 (type={window_type})")
                return module
