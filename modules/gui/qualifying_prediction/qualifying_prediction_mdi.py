...
"""
排位賽預測 MDI 視窗
Qualifying Prediction MDI

負責管理排位賽預測的 MDI 視窗，整合資料載入器和表格元件
基於機器學習模型提供排位賽成績預測
...
try:
    ...
    from modules.gui.qualifying_prediction.qualifying_prediction_widget import QualifyingPredictionWidget
...
class QualifyingPredictionMDI(UniversalAnalysisMDI):
    ...
    整合 QualifyingPredictionDataLoader 和 QualifyingPredictionWidget
    """
    ...
    def ensure_registered(cls):
        ...
        if not cls._REGISTERED:
            config = AnalysisMDIConfig(
                ...
                display_name="Qualifying Prediction",
    ...
    def __init__(self, parent=None):
        ...
        # 調用基類初始化
        super().__init__(analysis_type="qualifying_prediction", parent=parent)
    ...
    def create_data_manager(self) -> QualifyingPredictionDataLoader:
        """
        創建資料載入器（由基類調用）
    ...
    def load_initial_data(self):
        ...
        if os.environ.get("F1T_RUNTIME_MODE", "").lower() == "local" and self.data_manager:
            try:
                ...
                self._logger.exception("Local JSON load failed: %s", exc)
    ...
    def _on_api_failure(self, error_msg: str):
        ...
        if hasattr(self, 'lbl_control_status'):
            self.lbl_control_status.setText(
                tr("qualifying_pred_api_failure", "API 請求失敗，請稍後再試")
            )

        self._show_error(
            tr("qualifying_pred_api_failure_title", "載入失敗"),
            tr(
                "qualifying_pred_api_failure_message",
    ...
    def get_window_title(self, year: str = None, race: str = None, session: str = None) -> str:
        ...
        """
        # 使用 tr() 支援多國語言
        translated_title = tr("qualifying_prediction", "Qualifying Prediction")
    ...
    def _show_error(self, title: str, message: str):
        ...
        if os.environ.get("QT_QPA_PLATFORM", "").lower() == "offscreen":
            self._logger.error("[QUALIFYING_PRED_MDI] %s: %s", title, message)
            return
...
if __name__ == "__main__":
    ...
    if mdi.initialize_module():
        ...
        if widget and hasattr(widget, 'setWindowTitle'):
            widget.setWindowTitle("Qualifying Prediction MDI - Test")
