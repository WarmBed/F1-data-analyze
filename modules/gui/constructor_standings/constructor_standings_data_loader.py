...
class ConstructorStandingsDataLoader(UniversalDataLoader):
    ...
    def __init__(self, year: str, parent=None):
        ...
        self._debug(f"[CONSTRUCTOR_STANDINGS_LOADER] 初始化完成: year={year}")
    ...
    def _validate_data_format(self, raw_data: Dict[str, Any]) -> bool:
        ...
        if not isinstance(constructors, list):
            self._debug("[VALIDATION] ❌ constructors 必須是列表")
            return False
        
        if len(constructors) == 0:
            self._debug("[VALIDATION] ⚠️  constructors 為空")
            return False
        ...
        if not isinstance(constructor_info, dict):
            self._debug("[VALIDATION] ❌ constructor 必須是 dict")
            return False
        
        self._debug(f"[VALIDATION] ✅ 數據驗證通過 ({len(constructors)} 支車隊)")
        return True
    ...
    def _transform_data_for_display(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._debug(f"[TRANSFORM] ✅ 轉換 {len(transformed_rows)} 支車隊資料")
    ...
    def load_data(self, force_refresh: bool = False):
        ...
        self._debug(f"[LOAD] 開始載入車隊積分資料: {params}")
