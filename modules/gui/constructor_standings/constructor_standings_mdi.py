...
"""
車隊積分榜 MDI 視窗
Constructor Standings MDI

負責管理車隊積分榜的 MDI 視窗，整合資料載入器和表格元件
...
class ConstructorStandingsMDI(QWidget):
    """
    車隊積分榜 MDI 視窗
    ...
    def __init__(self, year: str = "2024", parent=None):
        ...
        # 初始化資料載入器
        self.data_loader = ConstructorStandingsDataLoader(year=self.year, parent=self)
    ...
    def _setup_ui(self):
        ...
        # 積分榜 Widget
        self.standings_widget = ConstructorStandingsWidget(parent=self)
    ...
    def _on_api_failure(self, error_msg: str):
        ...
        if year_int >= current_year:
            ...
            logger.info(f"[CONSTRUCTOR_MDI] 當前/未來年份 {year_int}，API 失敗，判定為賽季尚未開始")
        else:
            # 過去年份：檢查特定錯誤訊息
    ...
    def _on_data_loaded(self, data: Dict[str, Any]):
        ...
        self.status_label.setText(tr("load_success_status", "載入成功 ({count} 支車隊)").format(count=num_teams))
...
if __name__ == "__main__":
    ...
    # 測試 MDI 視窗
    mdi = ConstructorStandingsMDI(year="2024")
    mdi.setWindowTitle("車隊積分榜測試")
    mdi.resize(800, 500)
    mdi.show()
