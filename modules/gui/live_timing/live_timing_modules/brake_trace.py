r"""
Live Timing Brake Trace Module
==============================

即時煞車追蹤圖，顯示車手煞車狀態 vs 距離曲線。
使用 PyQt5 原生繪圖 (QPainter) 實現，與 Lap Analysis Brake Analysis 風格一致。
...
logger = get_logger("live_timing.brake_trace", component="gui")
...
class LapBrakeData:
    ...
    brakes: List[float] = field(default_factory=list)      # 煞車狀態點 (%)
    timestamps: List[float] = field(default_factory=list)  # 時間點 (秒)
    lap_time: Optional[float] = None  # 完成圈時間
    is_complete: bool = False
    
    def add_point(self, distance: float, brake: float, timestamp: float):
        """添加數據點"""
...
class BrakeTraceWidget(HoverTooltipMixin, QWidget):
    """
    煞車追蹤圖表 Widget - 使用 PyQt5 原生繪圖
    ...
    def __init__(self, parent=None):
        ...
        self._primary_current_lap: Optional[LapBrakeData] = None
        self._primary_best_lap: Optional[LapBrakeData] = None
        
        # 對比車手資料
        self._compare_driver: Optional[str] = None
        self._compare_current_lap: Optional[LapBrakeData] = None
        self._compare_best_lap: Optional[LapBrakeData] = None
        
        # 所有車手的最速圈快取 {driver_num: LapBrakeData}
        self._all_best_laps: Dict[str, LapBrakeData] = {}
        
        # 所有車手的當前圈資料 {driver_num: LapBrakeData}
        self._all_current_laps: Dict[str, LapBrakeData] = {}
        ...
        logger.info("[brake_trace] BrakeTraceWidget initialized (PyQt5 native)")
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            self._draw_gap_curve(painter, chart_rect)  # Gap 填充在煞車曲線下面
    ...
    def _get_brake_at_distance(self, lap_data, target_distance: float) -> Optional[float]:
        """Get brake at a specific distance using interpolation."""
        if not lap_data or not lap_data.distances or not lap_data.brakes:
            return None
        ...
        for i in range(len(lap_data.distances) - 1):
            if lap_data.distances[i] <= target_distance <= lap_data.distances[i + 1]:
                ...
                if d_range > 0:
                    ...
                    return lap_data.brakes[i] + ratio * (lap_data.brakes[i + 1] - lap_data.brakes[i])
        return None
    ...
    def _get_hover_data_at_x(self, x_value: float):
        ...
        if self._show_primary_current and self._primary_current_lap and self._primary_driver:
            brake = self._get_brake_at_distance(self._primary_current_lap, distance)
            if brake is not None:
                ...
                brake_text = "ON" if brake > 0.5 else "OFF"
                data_points.append(HoverDataPoint(
                    label=f"{tla}",
                    value=brake,
                    formatted_value=brake_text,
                    color=f"#{team_color}"
                ))
        
        if self._show_primary_best and self._primary_best_lap and self._primary_driver:
            brake = self._get_brake_at_distance(self._primary_best_lap, distance)
            if brake is not None:
                driver_info = self._driver_info.get(self._primary_driver, {})
                tla = driver_info.get('tla', self._primary_driver)
                team_color = driver_info.get('team_color', 'FFFFFF')
        ...
        if self._show_compare_current and self._compare_current_lap and self._compare_driver:
            brake = self._get_brake_at_distance(self._compare_current_lap, distance)
            if brake is not None:
        ...
        if self._show_compare_best and self._compare_best_lap and self._compare_driver:
            brake = self._get_brake_at_distance(self._compare_best_lap, distance)
            if brake is not None:
                driver_info = self._driver_info.get(self._compare_driver, {})
                tla = driver_info.get('tla', self._compare_driver)
                team_color = driver_info.get('team_color', '888888')
    ...
    def _draw_grid(self, painter: QPainter, chart_rect: QRect):
        ...
        # 水平網格線 (速度)
        brake_range = self.max_brake - self.min_brake
        if brake_range > 0:
            num_lines = 7
            for i in range(1, num_lines):
    ...
    def _draw_axes(self, painter: QPainter, chart_rect: QRect):
        ...
        if brake_range > 0:
            for i in range(0, 8, 2):  # 只顯示偶數刻度
                brake_value = self.min_brake + i * brake_range / 7
                y = chart_rect.bottom() - i * chart_rect.height() / 7
                painter.drawText(5, int(y - 10), self.margin_left - 10, 20, Qt.AlignRight | Qt.AlignVCenter, f"{int(brake_value)}")
        ...
        painter.rotate(-90)
        painter.drawText(-40, -10, 80, 20, Qt.AlignCenter, "Brake (0/1)")
        painter.restore()
    ...
    def _draw_brake_curves(self, painter: QPainter, chart_rect: QRect):
        ...
        if self._show_primary_current and self._primary_current_lap:
            ...
            self._draw_brake_curve(painter, chart_rect, self._primary_current_lap, color, 1.5, Qt.SolidLine)
        
        # 繪製主車手最速圈
        if self._show_primary_best and self._primary_best_lap:
            color = self._get_driver_color(self._primary_driver)
            self._draw_brake_curve(painter, chart_rect, self._primary_best_lap, color, 1.5, Qt.DashLine, 0.7)
        
        # 繪製對比車手當前圈
        if self._show_compare_current and self._compare_current_lap:
            color = self._get_driver_color(self._compare_driver)
            self._draw_brake_curve(painter, chart_rect, self._compare_current_lap, color, 1.5, Qt.SolidLine)
        
        # 繪製對比車手最速圈
        if self._show_compare_best and self._compare_best_lap:
            color = self._get_driver_color(self._compare_driver)
            self._draw_brake_curve(painter, chart_rect, self._compare_best_lap, color, 1.5, Qt.DashLine, 0.7)
    
    def _draw_brake_curve(self, painter: QPainter, chart_rect: QRect, 
                          lap_data: LapBrakeData, color: str,
        ...
        for i in range(len(lap_data.distances)):
            ...
            brake = lap_data.brakes[i]
            
            # 轉換為螢幕座標
            x = chart_rect.left() + (dist - self.min_distance) / distance_range * chart_rect.width()
            y = chart_rect.bottom() - (brake - self.min_brake) / brake_range * chart_rect.height()
    ...
    def set_track_info(self, track_length: float, corners: List[Dict[str, Any]]):
        ...
        logger.info("[brake_trace] Track info set: length=%sm, corners=%d", self._track_length, len(self._corners))
        self.update()
    ...
    def set_primary_driver(self, driver_num: str):
        ...
        logger.info("[brake_trace] Primary driver set: %s (%s)", tla, driver_num)
        self.update()
    ...
    def set_compare_driver(self, driver_num: Optional[str]):
        ...
        logger.info("[brake_trace] Compare driver set: %s, delta_reference=%s", driver_num, self._delta_reference)
        self.update()
    ...
    def update_from_snapshot(self, snapshot: Dict[str, Any]):
        ...
        for driver_num, driver_data in drivers_data.items():
            ...
            # 計算累積距離 (使用 GPS 座標)
            distance = self._calculate_distance(driver_num, brake, race_time_seconds, lap_number, x, y)
            
            # 確保有當前圈資料結構
            if driver_num not in self._all_current_laps:
                self._all_current_laps[driver_num] = LapBrakeData(
            ...
            # 添加數據點
            current_lap_data.add_point(distance, brake, lap_time)
    ...
    def _calculate_distance(self, driver_num: str, brake: float, timestamp: float, 
    ...
    def _finalize_lap(self, driver_num: str, lap_data: LapBrakeData):
        """完成一圈，檢查是否為最速圈"""
        ...
        if lap_data.lap_time and lap_data.lap_time > 0:
            ...
            if is_new_best:
                ...
                logger.info("[brake_trace] New best lap for %s: %.3fs (Lap %s)", tla, lap_data.lap_time, lap_data.lap_number)
...
class LiveTimingBrakeTrace(BaseLiveTimingMDI):
    """
    Brake Trace MDI 視窗
    ...
    MODULE_ID = "live_timing_brake_trace"
    DEFAULT_TITLE = "Brake Trace"
    ...
    def __init__(self, parent=None, data_manager=None):
        ...
        logger.info("[brake_trace_MDI] LiveTimingBrakeTrace initialized (PyQt5 native)")
    
    def _setup_ui(self):
        """Setup UI components"""
        self.brake_widget = BrakeTraceWidget()
        self._main_layout.addWidget(self.brake_widget)
    ...
    def set_linkage_enabled(self, enabled: bool):
        """設置連動狀態 - 被標題列的 L 按鈕調用"""
        if hasattr(self, 'brake_widget'):
            self.brake_widget.set_individual_linkage_enabled(enabled)
            # 同時控制 hover 位置同步
            if hasattr(self.brake_widget, 'set_hover_linkage_enabled'):
                self.brake_widget.set_hover_linkage_enabled(enabled)
    
    def is_linkage_enabled(self) -> bool:
        """獲取連動狀態"""
        if hasattr(self, 'brake_widget'):
            return self.brake_widget.is_individual_linkage_enabled()
        return True
    
    def _on_driver_selected(self, driver_num: str):
        """處理車手選擇信號 - 從 DataManager snapshot 獲取車手資訊"""
        logger.info("[brake_trace_MDI] Driver selected from external: %s", driver_num)
        if hasattr(self, 'brake_widget'):
            ...
            if self._data_manager:
                ...
                if snapshot:
                    ...
                    if driver_info:
                        ...
                        team_color = driver_info.get('team_color', 'FFFFFF')
                        if driver_num not in self.brake_widget._driver_info:
                            self.brake_widget._driver_info[driver_num] = {}
                        self.brake_widget._driver_info[driver_num]['tla'] = tla
                        self.brake_widget._driver_info[driver_num]['team_color'] = team_color
                        logger.debug("[brake_trace_MDI] Driver info from snapshot: %s (%s)", tla, team_color)
            
            self.brake_widget.set_primary_driver(driver_num)
    ...
    def _on_snapshot_updated(self, snapshot: Dict[str, Any]):
        ...
        if hasattr(self, 'brake_widget'):
            ...
            if drivers:
                ...
                brake = sample_driver.get('brake')
                lap = sample_driver.get('lap')
                if brake is not None:
                    logger.debug(
                        "[brake_trace_DEBUG] Snapshot received: %d drivers, sample brake=%s, lap=%s",
                        len(drivers),
                        brake,
                        lap,
                    )
            
            self.brake_widget.update_from_snapshot(snapshot)
    ...
    def _on_race_loaded(self, race_info: Dict[str, Any]):
        ...
        logger.info("[brake_trace_MDI] Race loaded: %s %s", year, race_key)
    ...
    def _load_track_data(self, year: int, race_key: str):
        ...
        # API 失敗，返回錯誤（禁止本地回退）
        logger.error("[brake_trace_MDI] API 獲取失敗，請確認 API 服務器已啟動")
        return False
    ...
    def _load_track_via_api(self, year: int, track_name: str) -> bool:
        ...
        try:
            ...
            for try_year in years_to_try:
                ...
                logger.info("[brake_trace_MDI] 嘗試 API 獲取: %s %s", try_year, track_name)
            ...
            logger.exception("[brake_trace_MDI] API 獲取賽道數據失敗: %s", e)
            return False
    ...
    def _process_track_data(self, data: dict, orig_year: int, loaded_year: int, track_name: str, source: str) -> bool:
        ...
        try:
            ...
            if track_length <= 0:
                track_length = 5000
                logger.warning("[brake_trace_MDI] 無法估算賽道長度，使用默認值 %sm", track_length)
            ...
            if hasattr(self, 'brake_widget'):
                self.brake_widget.set_track_info(track_length, corners)
            
            if loaded_year != orig_year:
                logger.info("[brake_trace_MDI] 使用 %s 賽道數據 (%s, 原始年份 %s 不可用)", loaded_year, source, orig_year)
            logger.info(
                "[brake_trace_MDI] 賽道載入成功 (%s): %s, length=%.0fm, corners=%d",
                source,
                track_name,
                track_length,
                len(corners),
            ...
            logger.exception("[brake_trace_MDI] 處理賽道數據失敗: %s", e)
            return False
    ...
    def _estimate_track_length_from_records(self, position_records: List[Dict], max_dist: float) -> float:
        ...
        if len(lap_starts) >= 2:
            ...
            logger.info(
                "[brake_trace_MDI] 從 position_records 估算賽道長度: %.0fm (detected %d laps)",
                track_length,
                len(lap_starts),
            )
            return track_length
            ...
            logger.warning(
                "[brake_trace_MDI] 無法檢測圈數邊界，估算 %d 圈，賽道長度: %.0fm",
                estimated_laps,
                track_length,
            )
            return track_length
    ...
    def _estimate_track_length_from_corners(self, corners: List[Dict]) -> float:
        ...
        if dist_range < 7000:  # F1 賽道最長約 7km
            ...
            logger.info("[brake_trace_MDI] 從彎道數據估算賽道長度: %.0fm", track_length)
            return track_length
        ...
        logger.info("[brake_trace_MDI] 從彎道分布估算賽道長度: %.0fm", estimated_track_length)
    ...
    def _process_corners_from_json(self, corners: List[Dict], track_length: float = 5000.0) -> List[Dict]:
        ...
        for corner in corners:
            ...
            if lap_distance <= 0:
                ...
                if mapped_dist > 0 and track_length > 0:
                    ...
                    logger.debug(
                        "[brake_trace_MDI] T%d: mapped_distance=%.0fm -> lap_distance=%.0fm",
                        corner_num,
                        mapped_dist,
                        lap_distance,
                    )
            ...
            if lap_distance <= 0:
                logger.warning("[brake_trace_MDI] T%d has no valid distance, skipping", corner_num)
                continue
        ...
        logger.debug(
            "[brake_trace_MDI] Processed corners: %s",
