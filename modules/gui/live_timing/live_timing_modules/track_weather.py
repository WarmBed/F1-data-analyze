...
class TrackWeatherWidget(QWidget):
    ...
    def update_from_snapshot(self, snapshot: Dict[str, Any]):
        ...
        # 更新比賽時間（使用顯示時間，從 0:00 開始）
        race_time_seconds = snapshot.get('race_time_seconds', 0)
        
        # 減去起跑偏移
        if self._data_manager:
            race_time_seconds = race_time_seconds - self._data_manager.get_race_start_offset()
            race_time_seconds = max(0.0, race_time_seconds)
        ...
        self._race_time = display_time
        self._time_label.setText(display_time)
        ...
        # 從 data_manager 獲取 track status（使用原始 race_time 字串進行查詢）
        race_time_for_query = snapshot.get('race_time', '0:00:00')
        if self._data_manager:
            track_status = self._data_manager.get_track_status_at_time(race_time_for_query)
...
class TrackWeatherMDI(BaseLiveTimingMDI):
    ...
    def __init__(self, parent=None, data_manager=None):
        # 性能優化: 追蹤上次更新的賽事時間
        # ★ 必須在 super().__init__() 之前初始化，因為基類會調用 _setup_ui() → _on_snapshot_updated()
