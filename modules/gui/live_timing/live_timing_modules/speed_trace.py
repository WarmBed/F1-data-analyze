"""
Live Timing Speed Trace Module
==============================

即時速度追蹤圖，顯示車手速度 vs 距離曲線。
使用 PyQt5 原生繪圖 (QPainter) 實現，與 Lap Analysis Speed Analysis 風格一致。
...
class LapSpeedData:
    ...
    speeds: List[float] = field(default_factory=list)      # 速度點 (km/h)
    timestamps: List[float] = field(default_factory=list)  # 時間點 (秒)
    lap_time: Optional[float] = None  # 完成圈時間
    is_complete: bool = False
    
    def add_point(self, distance: float, speed: float, timestamp: float):
        """添加數據點"""
...
class SpeedTraceWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._primary_current_lap: Optional[LapSpeedData] = None
        self._primary_best_lap: Optional[LapSpeedData] = None
        
        # 對比車手資料
        self._compare_driver: Optional[str] = None
        self._compare_current_lap: Optional[LapSpeedData] = None
        self._compare_best_lap: Optional[LapSpeedData] = None
        
        # 所有車手的最速圈快取 {driver_num: LapSpeedData}
        self._all_best_laps: Dict[str, LapSpeedData] = {}
        
        # 所有車手的當前圈資料 {driver_num: LapSpeedData}
        self._all_current_laps: Dict[str, LapSpeedData] = {}
    ...
    def _draw_axes(self, painter: QPainter, chart_rect: QRect):
        ...
        painter.rotate(-90)
        painter.drawText(-40, -10, 80, 20, Qt.AlignCenter, "Speed (km/h)")
        painter.restore()
    ...
    def _draw_speed_curves(self, painter: QPainter, chart_rect: QRect):
        ...
        if self._show_primary_best and self._primary_best_lap:
            color = self._get_driver_color(self._primary_driver)
            self._draw_speed_curve(painter, chart_rect, self._primary_best_lap, color, 1.5, Qt.DashLine, 0.7)
        ...
        if self._show_compare_best and self._compare_best_lap:
            color = self._get_driver_color(self._compare_driver)
            self._draw_speed_curve(painter, chart_rect, self._compare_best_lap, color, 1.5, Qt.DashLine, 0.7)
    
    def _draw_speed_curve(self, painter: QPainter, chart_rect: QRect, 
                          lap_data: LapSpeedData, color: str,
    ...
    def update_from_snapshot(self, snapshot: Dict[str, Any]):
        ...
        for driver_num, driver_data in drivers_data.items():
            ...
            # 計算累積距離 (使用 GPS 座標)
            distance = self._calculate_distance(driver_num, speed, race_time_seconds, lap_number, x, y)
            
            # 確保有當前圈資料結構
            if driver_num not in self._all_current_laps:
                self._all_current_laps[driver_num] = LapSpeedData(
            ...
            # 添加數據點
            current_lap_data.add_point(distance, speed, lap_time)
    ...
    def _calculate_distance(self, driver_num: str, speed: float, timestamp: float, 
    ...
    def _finalize_lap(self, driver_num: str, lap_data: LapSpeedData):
        """完成一圈，檢查是否為最速圈"""
...
class LiveTimingSpeedTrace(BaseLiveTimingMDI):
    """
    Speed Trace MDI 視窗
    ...
    MODULE_ID = "live_timing_speed_trace"
    DEFAULT_TITLE = "Speed Trace"
