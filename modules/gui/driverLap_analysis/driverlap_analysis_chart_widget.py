#!/usr/bin/env python3
"""
driverLapAnalysisChartWidget - F1T 輪胎策略分析圖表組件        # 佈局參數
...
class driverLapAnalysisChartWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 輪胎配方顏色映射
        self.driverLap_colors = {
            'SOFT': driverLapChartTheme.SOFT_COLOR,
            'MEDIUM': driverLapChartTheme.MEDIUM_COLOR,
            'HARD': driverLapChartTheme.HARD_COLOR,
            'INTERMEDIATE': driverLapChartTheme.INTERMEDIATE_COLOR,
            'WET': driverLapChartTheme.WET_COLOR
        }
        ...
        print("[driverLap_CHART] 輪胎策略圖表組件初始化完成")
    
    def update_data(self, data: Dict[str, Any], selected_driver: str = None):
        """更新圖表數據"""
        try:
            # 兼容 processed_data 格式：若上層傳入的是包含 charts_data 的包裝結構
            if isinstance(data, dict) and 'charts_data' in data and 'all_drivers_driverLap_strategy' not in data:
                data = data.get('charts_data', {})
                print("[driverLap_CHART] 解包 charts_data -> 原始 JSON")
            
            print(f"[driverLap_CHART] 收到數據更新: {type(data)}")
            print(f"[driverLap_CHART] 選中車手: {selected_driver}")
            ...
            self.all_drivers_data = (data.get('drivers_analysis', {}) or      # 新格式 v2
                                   data.get('all_drivers_driverLap_strategy', {}) or 
                                   data.get('driverLap_timing_corrected', {}) or 
                                   data.get('driverLap_analysis', {}))
            
            # 從輪胎策略分析數據中提取所有車手的 Stint 數據
            # 支援多種 JSON 結構格式，優先支援新格式
            driverLap_analysis = (data.get('drivers_analysis', {}) or           # 新格式 v2
                           data.get('all_drivers_driverLap_strategy', {}) or 
                           data.get('driverLap_timing_corrected', {}) or 
                           data.get('driverLap_analysis', {}))
            drivers_analyzed = data.get('drivers_analyzed', list(driverLap_analysis.keys()))
            
            print(f"[driverLap_CHART] 可用車手: {drivers_analyzed} (共 {len(drivers_analyzed)} 位)")
            
            # 處理所有車手的數據，不只是單一車手
            self.all_drivers_stint_data = {}
            
            for driver in drivers_analyzed:
                if driver in driverLap_analysis:
                    driver_data = driverLap_analysis[driver]
                    ...
                    if len(driver_stints) > 0:
                        ...
                        compound = first_stint.get('compound') or first_stint.get('driverLap_compound')
            ...
            print(f"[driverLap_CHART] 主要顯示車手: {selected_driver}")
            print(f"[driverLap_CHART] 總共載入 {len(self.all_drivers_stint_data)} 位車手的輪胎策略數據")
            
            # 如果沒有 Stint 數據，使用示例數據
            if not self.stint_data:
                print("[driverLap_CHART] 沒有找到 Stint 數據，使用示例數據")
                self.stint_data = [
                    {
                        'stint_number': 1,
                        'start_lap': 1,
            ...
            print(f"[driverLap_CHART] 圈數範圍: {self.min_lap}-{self.max_lap}")
            print(f"[driverLap_CHART] 顯示 {len(self.all_drivers_stint_data) if hasattr(self, 'all_drivers_stint_data') else 0} 位車手的輪胎策略")
            
            self.update()
            
        except Exception as e:
            print(f"[driverLap_CHART] 數據更新錯誤: {e}")
            import traceback
            traceback.print_exc()
    ...
    def paintEvent(self, event):
        ...
        # 清空背景
        painter.fillRect(self.rect(), driverLapChartTheme.BACKGROUND)
        ...
        # 繪製圖表背景
        painter.fillRect(chart_rect, driverLapChartTheme.CHART_BACKGROUND)
        
        if not self.stint_data:
            # 沒有數據時顯示提示
            painter.setPen(QPen(driverLapChartTheme.TEXT_COLOR))
    ...
    def _draw_axes(self, painter: QPainter, chart_rect: QRect):
        """繪製座標軸"""
        painter.setPen(QPen(driverLapChartTheme.AXIS_COLOR, 2))
        ...
        # 繪製X軸刻度 (圈數)
        painter.setPen(QPen(driverLapChartTheme.LABEL_COLOR))
        font = QFont()
        font.setPointSize(9)
        ...
        for driver_index, driver in enumerate(drivers):
            ...
            for stint in driver_stints:
                ...
                if end_lap <= start_lap:
                    print(f"[driverLap_CHART] 檢測到錯誤 end_lap: driver={driver}, start={start_lap}, end={stint['end_lap']}")
                    
                    if 'length' in stint and stint['length'] > 0:
                        # 使用 length 字段重新計算 end_lap
                        length = stint.get('length', 1)
                        end_lap = start_lap + length - 1
                        print(f"[driverLap_CHART] 使用 length 修正: driver={driver}, length={length}, 新end={end_lap}")
                    else:
                        # 嘗試根據車手在該配方上的總圈數估算
                        compound = (stint.get('compound') or 
                                   stint.get('driverLap_compound') or 
                                   stint.get('tyre_compound', 'MEDIUM'))
                        
                        # 從 driverLap_performance 獲取該配方的總圈數
                        if hasattr(self, 'all_drivers_data') and driver in self.all_drivers_data:
                            driverLap_perf = self.all_drivers_data[driver].get('driverLap_performance', {})
                            if compound in driverLap_perf:
                                laps_used = driverLap_perf[compound].get('laps_used', 15)
                                end_lap = start_lap + laps_used - 1
                                print(f"[driverLap_CHART] 使用 driverLap_performance 修正: driver={driver}, compound={compound}, laps_used={laps_used}, 新end={end_lap}")
                            else:
                                # 最後的備用方案：估算合理圈數
                                end_lap = start_lap + 15
                                print(f"[driverLap_CHART] 使用默認估算: driver={driver}, 新end={end_lap}")
                        else:
                            end_lap = start_lap + 15
                            print(f"[driverLap_CHART] 使用默認估算: driver={driver}, 新end={end_lap}")
                    ...
                    if end_lap > 60:
                        end_lap = 53
                        print(f"[driverLap_CHART] 限制最大圈數: driver={driver}, 最終end={end_lap}")
                    
                    # 如果修正後仍然無效，強制設置一個最小值
                    if end_lap <= start_lap:
                        end_lap = start_lap + 5  # 至少5圈
                        print(f"[driverLap_CHART] 強制最小值: driver={driver}, 最終end={end_lap}")
                ...
                # 選擇顏色
                color = self.driverLap_colors.get(compound, driverLapChartTheme.MEDIUM_COLOR)
                ...
                # 繪製邊框 (硬胎需要黑邊框)
                if compound == 'HARD':
                    painter.setPen(QPen(driverLapChartTheme.HARD_BORDER_COLOR, 2))
                else:
                    painter.setPen(QPen(driverLapChartTheme.DEFAULT_BORDER_COLOR, 1))
                ...
                if width > min_width_for_text and stint_height > min_height_for_text:
                    ...
                    # 設置文字顏色
                    painter.setPen(QPen(driverLapChartTheme.STINT_TEXT_COLOR))  # 普通黑色
    ...
    def _calculate_fastest_per_compound(self) -> Dict[str, str]:
        ...
        try:
            ...
            for driver in self.all_drivers_data:
                ...
                if 'driverLap_performance' in driver_data:
                    driverLap_perf = driver_data['driverLap_performance']
                    for compound, perf_data in driverLap_perf.items():
            ...
            for compound, driver_times in compound_times.items():
                if driver_times:
                    ...
                    print(f"[driverLap_CHART] {compound} 最快車手: {fastest_driver[0]} ({fastest_driver[1]:.3f}s)")
            
        except Exception as e:
            print(f"[driverLap_CHART] 計算最快車手失敗: {e}")
    ...
    def _get_best_lap_time(self, driver: str, compound: str) -> float:
        ...
        try:
            ...
            # 檢查 driverLap_performance 數據
            if 'driverLap_performance' in driver_data:
                driverLap_perf = driver_data['driverLap_performance']
                if compound in driverLap_perf and 'fastest_lap_time' in driverLap_perf[compound]:
                    return driverLap_perf[compound]['fastest_lap_time']
            
            return None
        except Exception as e:
            print(f"[driverLap_CHART] 獲取最佳圈速失敗: driver={driver}, compound={compound}, error={e}")
            return None
    ...
    def _format_lap_time(self, lap_time_seconds: float) -> str:
        ...
        try:
            ...
            print(f"[driverLap_CHART] 格式化圈速失敗: {lap_time_seconds}, error={e}")
            return ""
    ...
    def _draw_legend(self, painter: QPainter):
        ...
        for i, compound in enumerate(compounds):
            x = legend_x + i * 80
            color = self.driverLap_colors[compound]
            
            # 繪製色塊
            painter.fillRect(x, legend_y, 15, 15, color)
            if compound == 'HARD':
                painter.setPen(QPen(driverLapChartTheme.HARD_BORDER_COLOR, 1))
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if clicked_stint:
                ...
                print(f"[driverLap_CHART] 選中 Stint {clicked_stint['stint_number']}")
