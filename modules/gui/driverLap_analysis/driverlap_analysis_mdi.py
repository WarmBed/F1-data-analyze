#!/usr/bin/env python3
"""
driverLapAnalysisUniversal - F1T 通用輪胎策略分析模組
===============================================
...
數據來源：CLI -f26 生成的 driverLap_strategy JSON 檔案
圖表類型：橫向長條圖
...
class driverLapAnalysisDataManager(UniversalDataLoader):
    ...
    def __init__(self, parent=None):
        # 註冊輪胎策略分析類型（如果尚未註冊）
        if "driverLap_strategy" not in UniversalDataLoader.ANALYSIS_TYPES:
            driverLap_config = AnalysisConfig(
                display_name="輪胎策略分析",
                debug_prefix="[driverLap_ANALYSIS]",
                data_source="json",
                cli_function="26",  # CLI -f26: 輪胎換胎時機推論
                file_patterns=[
                    "driverLap_strategy_{year}_{race}_{session}.json",           # 新格式 (不含 all_drivers)
                    "driverLap_strategy_{year}_{race}_{session}_all_drivers.json", # 新格式變體
                    "driverLap_timing_inference_{year}_{race_full}_None_all_drivers.json"  # 向下兼容舊格式
                ],
                search_directories=["json", "json_exports", "cache"],
                supports_realtime=False,
                cache_enabled=True
            )
            UniversalDataLoader.register_analysis_type("driverLap_strategy", driverLap_config)
        
        super().__init__("driverLap_strategy", parent)
        ...
        print(f"[driverLap_ANALYSIS] 初始化完成, 搜索目錄: {self.config.search_directories}")
        print(f"[driverLap_ANALYSIS] 文件模式: {self.config.file_patterns}")
    ...
    def _validate_data_format(self, data: Any) -> bool:
        ...
        valid_formats = [
            ...
            "driverLap_timing_corrected",      # CLI -f26 舊格式
            "all_drivers_driverLap_strategy",  # 舊格式
    ...
    def process_loaded_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "drivers_analysis" in data:
                # CLI -f26 v2 新格式
                self.driverLap_data = data["drivers_analysis"]
                self._debug("使用 drivers_analysis 格式 (CLI -f26 v2)")
            elif "driverLap_timing_corrected" in data:
                # CLI -f26 舊格式
                self.driverLap_data = data["driverLap_timing_corrected"]
                self._debug("使用 driverLap_timing_corrected 格式")
            elif "all_drivers_driverLap_strategy" in data:
                # 舊格式
                self.driverLap_data = data["all_drivers_driverLap_strategy"]
                self._debug("使用 all_drivers_driverLap_strategy 格式")
            elif "corrected_stint_analysis" in data:
                # 另一種格式
                self.driverLap_data = data["corrected_stint_analysis"]
            ...
            # 轉換為分析用數據格式
            processed_data = {
                "driverLap_data": self._process_driverLap_strategy_data(),
                "summary": self.strategy_stats,
                "metadata": data.get("metadata", {}),
                "analysis_mode": data.get("analysis_mode", "unknown"),
                "drivers_analyzed": list(self.driverLap_data.keys()),  # 修復：使用實際的車手列表
                "charts_data": self._prepare_driverLap_chart_data()
            }
            
            self._debug(f"成功處理 {len(self.driverLap_data)} 車手輪胎策略數據")
    ...
    def _process_driverLap_strategy_data(self) -> Dict[str, List]:
        """處理輪胎策略數據"""
        drivers_data = []
        
        # 處理所有車手的輪胎策略數據
        for driver_code, driver_data in self.driverLap_data.items():
    ...
    def _prepare_driverLap_chart_data(self) -> Dict[str, Any]:
        ...
        chart_data = {
            # 原始 JSON 數據的關鍵字段
            "drivers_analyzed": list(self.driverLap_data.keys()),
            "driverLap_analysis": self.driverLap_data,  # 新格式使用 drivers_analysis
            "all_drivers_driverLap_strategy": self.driverLap_data,  # 為了兼容性
    ...
    def get_driverLap_summary(self) -> Dict[str, Any]:
        """獲取輪胎策略摘要統計"""
        return {
            "total_drivers": len(self.driverLap_data),
            "total_stints": sum(len(driver_data.get("stint_analysis", [])) 
                              for driver_data in self.driverLap_data.values() 
            ...
            "has_driverLap_data": len(self.driverLap_data) > 0,
...
class driverLapAnalysisUniversal(UniversalAnalysisMDI):
    """
    通用輪胎策略分析 MDI 模組
    ...
    def __init__(self, parent=None):
        print(f"[driverLap_MDI] driverLapAnalysisUniversal 開始初始化...")
        
        # 註冊輪胎策略分析模組類型
        if "driverLap_analysis" not in UniversalAnalysisMDI.MDI_MODULE_TYPES:
            driverLap_config = AnalysisMDIConfig(
                analysis_type="driverLap_analysis",
                display_name="輪胎策略分析",
            ...
            UniversalAnalysisMDI.register_mdi_module_type("driverLap_analysis", driverLap_config)
            
        super().__init__("driverLap_analysis", parent)
        print(f"[driverLap_MDI] 基類初始化完成, 數據管理器: {self.data_manager}")
        
        # 初始化模組組件
        print(f"[driverLap_MDI] 開始初始化模組組件...")
        if not self.initialize_module():
            print(f"[driverLap_MDI] ❌ 模組組件初始化失敗")
            return
        
        print(f"[driverLap_MDI] ✅ 模組組件初始化完成")
        print(f"[driverLap_MDI] 數據管理器: {self.data_manager}")
        print(f"[driverLap_MDI] 圖表組件: {self.chart_widget}")
        
        # 參照遙測分析：設置響應式佈局
        self.set_responsive_layout()
        
    def create_data_manager(self) -> driverLapAnalysisDataManager:
        """創建輪胎策略分析數據管理器"""
        return driverLapAnalysisDataManager(self)
        
    def create_chart_widget(self) -> driverLapAnalysisChartWidget:
        """創建輪胎策略分析圖表組件"""
        return driverLapAnalysisChartWidget(parent=None)
        
    def create_control_widget(self) -> driverLapAnalysisControlWidget:
        """創建輪胎策略分析控制面板"""
        control_widget = driverLapAnalysisControlWidget(self)
    ...
    def update_lap_parameters(self, year: str, race: str, session: str, **kwargs) -> bool:
        """更新輪胎策略分析參數"""
        try:
            print(f"[driverLap_MDI] ========== 輪胎策略參數更新 ==========")
            print(f"[driverLap_MDI] 收到參數: {year} {race} {session}")
            ...
            if hasattr(self, 'data_manager') and self.data_manager:
                print(f"[driverLap_MDI] 更新數據管理器參數...")
                ...
                # 載入數據 - 傳遞正確的參數
                print(f"[driverLap_MDI] 開始載入數據...")
                result = self.data_manager.load_data(
                    year=self.current_year,
                    race=self.current_race,
                    session=self.current_session
                )
                print(f"[driverLap_MDI] 數據載入結果: {result}")
                ...
                if result:
                    print("[driverLap_MDI] 等待 data_loaded 信號進行圖表更新 (非同步載入) ...")
            
            print(f"[driverLap_MDI] 參數更新完成")
            return True
            
        except Exception as e:
            print(f"[driverLap_MDI] 參數更新失敗: {str(e)}")
            import traceback
            print(f"[driverLap_MDI] 錯誤詳情:")
            traceback.print_exc()
            return False
    ...
    def resizeEvent(self, event):
        ...
        try:
            ...
            print(f"[driverLap_MDI] resizeEvent: MDI視窗縮放 {old_size.width()}x{old_size.height()} -> {new_size.width()}x{new_size.height()}")
            
            # 通知圖表組件更新佈局
            if hasattr(self, 'chart_widget') and self.chart_widget:
                if hasattr(self.chart_widget, 'update_chart_layout'):
                    print("[driverLap_MDI] resizeEvent: 觸發圖表重新佈局")
                    self.chart_widget.update_chart_layout()
                else:
                    print("[driverLap_MDI] resizeEvent: 圖表組件不支援動態佈局更新")
            else:
                print("[driverLap_MDI] resizeEvent: 圖表組件尚未初始化")
                
        except Exception as e:
            print(f"[ERROR] [driverLap_MDI] resizeEvent 處理失敗: {e}")
    ...
    def set_responsive_layout(self):
        ...
        try:
            ...
            print("[driverLap_MDI] 響應式佈局已設置")
            
        except Exception as e:
            print(f"[ERROR] [driverLap_MDI] 設置響應式佈局失敗: {e}")
    ...
    def get_analysis_summary(self) -> Dict[str, Any]:
        ...
        try:
            driverLap_summary = self.data_manager.get_driverLap_summary()
            ...
            return {
                ...
                "data_info": {
                    "total_laps": driverLap_summary.get("total_laps", 0),
                    "driverLap_laps": driverLap_summary.get("driverLap_laps", 0),
                    "driverLap_percentage": driverLap_summary.get("driverLap_percentage", 0.0),
                    "has_weather_data": driverLap_summary.get("has_driverLap_data", False)
                },
...
# 模組註冊 - 確保在導入時自動註冊
def register_driverLap_analysis_module():
    """註冊下雨分析模組"""
    try:
        # 這裡可以添加到全局模組註冊表
        pass
...
# 自動註冊
register_driverLap_analysis_module()
