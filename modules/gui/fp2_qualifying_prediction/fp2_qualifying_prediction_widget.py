...
"""
FP2→Q 排位賽預測表格元件
FP2 to Qualifying Prediction Widget

負責顯示 FP2→Q 排位賽預測分析的表格，包含車手排名、FP2時間、預測時間、Q結果等資訊
基於機器學習模型的預測結果
...
class FP2QualifyingPredictionWidget(QWidget):
    """
    FP2→Q 排位賽預測表格元件
    
    顯示所有車手的排位賽預測，包含：
    - 10 欄位表格（排名、車手、車隊、FP2時間、預測時間、Q結果、△FP2、預測名次、Q名次、變化）
    - 車隊顏色編碼
    - 改善幅度梯度顏色
    - 模型統計摘要（R²、MAE、可靠性）
    - Tooltip 懸停資訊
    ...
    def _create_table(self) -> QTableWidget:
        ...
        # 欄位順序: 車手 → 車隊 → FP2時間 → 預測時間 → Q時間 → △FP2 → 預測名次 → Q名次 → 變化
        columns = [
            tr("rank", "排名"),           # 0: position (隱藏，用於排序)
            tr("driver", "車手"),          # 1: driver (背景色)
            tr("team", "車隊"),            # 2: team
            tr("fp2_time", "FP2 時間"),    # 3: fp2_time
            tr("predicted_time", "預測時間"),  # 4: predicted_time
            tr("q_result", "Q 時間"),      # 5: actual_q_time
            tr("delta_fp2", "FP2 Delta"),     # 6: improvement (梯度顏色)
            tr("fp2_rank", "FP2 名次"),    # 7: fp2_predicted_rank
        ...
        table.setColumnWidth(3, 120)  # FP2 時間
        table.setColumnWidth(4, 120)  # 預測時間
        table.setColumnWidth(5, 120)  # Q 時間
        table.setColumnWidth(6, 120)  # △ FP2
    ...
    def _create_summary_panel(self) -> QGroupBox:
        ...
        # 行 2: 模型指標（所有車手共用）
        self.lbl_model_r2 = self._create_stat_label(tr("model_r2_label", "Model R2: -"), label_font)
        self.lbl_model_mae = self._create_stat_label(tr("model_mae_label", "Model MAE: -"), label_font)
    ...
    def update_display(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if not predictions:
                logger.warning("[FP2_QUAL_WIDGET] 沒有預測數據")
                return
            ...
            logger.info("[FP2_QUAL_WIDGET] 已更新顯示 (%s 位車手)", len(predictions))
            
        except Exception as e:
            logger.exception("[FP2_QUAL_WIDGET] 更新顯示失敗: %s", e)
    ...
    def _set_row_data(self, row: int, pred: Dict[str, Any]):
        ...
        try:
            ...
            # 3. FP2 時間 (使用 fp2_time 欄位)
            fp2_time = pred.get("fp2_time")
            fp2_item = QTableWidgetItem(self._format_time(fp2_time))
            fp2_item.setTextAlignment(Qt.AlignCenter)
            
            # 使用系統預設字體（8pt，無粗體）
            font = QFont()
            font.setPointSize(8)
            fp2_item.setFont(font)
            self.table.setItem(row, 3, fp2_item)
            ...
            # 6. △ FP2（使用 Driver Race Position 風格：淡色背景 + 三角形）
            ...
            if improvement > 0:
                ...
                delta_item.setForeground(QBrush(QColor(0, 120, 0)))  # 深綠色文字
            elif improvement < 0:
                # 退步 - 淺紅色背景 + 向下三角形
                delta_item.setText(f"{improvement:.3f}s")
                delta_item.setBackground(QBrush(QColor(255, 200, 200)))  # 淺紅色
                delta_item.setForeground(QBrush(QColor(180, 0, 0)))  # 深紅色文字
            else:
                # 持平 - 淺灰色背景
                delta_item.setText(f"{improvement:.3f}s")
                delta_item.setBackground(QBrush(QColor(230, 230, 230)))  # 淺灰色
            ...
            # 7. 預測名次（FP2 預測名次）
            fp2_rank = pred.get("fp2_predicted_rank", pred.get("rank"))
            fp2_rank_item = QTableWidgetItem()
            fp2_rank_item.setData(Qt.DisplayRole, int(fp2_rank))
            fp2_rank_item.setTextAlignment(Qt.AlignCenter)
            
            font = QFont()
            font.setPointSize(8)
            fp2_rank_item.setFont(font)
            self.table.setItem(row, 7, fp2_rank_item)
            ...
            if actual_q_rank is not None:
                ...
                q_rank_item.setForeground(QBrush(QColor(0, 120, 0)))  # 深綠色
            else:
                q_rank_item = QTableWidgetItem("N/A")
                q_rank_item.setForeground(QBrush(QColor(100, 100, 100)))  # 深灰色
            ...
            if rank_change is not None:
                ...
                if rank_change > 0:
                    ...
                    change_item.setForeground(QBrush(QColor(0, 120, 0)))  # 深綠色文字
                elif rank_change < 0:
                    # 退步（排名下降）- 淺紅色背景 + 向下三角形
                    change_item.setText(f"{abs(rank_change)} -")  # 使用絕對值，更清楚
                    change_item.setBackground(QBrush(QColor(255, 200, 200)))  # 淺紅色
                    change_item.setForeground(QBrush(QColor(180, 0, 0)))  # 深紅色文字
                else:
                    # 持平 - 淺灰色背景 + 橫線
                    change_item.setText("0")
                    change_item.setBackground(QBrush(QColor(230, 230, 230)))  # 淺灰色
                ...
                change_item.setBackground(QBrush(QColor(230, 230, 230)))  # 淺灰色背景
                change_item.setForeground(QBrush(QColor(100, 100, 100)))  # 深灰色文字
    ...
    def _update_statistics_panel(self, metadata: Dict[str, Any], predictions: List[Dict[str, Any]]):
        ...
        try:
            ...
            self.lbl_model_r2.setText(
                tr("model_r2_value", "Model R2: {r2} (Samples: {count})").format(
                    r2=f"{model_r2:.4f}",
                    count=sample_count
                )
            )
            ...
            self.lbl_model_mae.setText(
                tr("model_mae_value", "Model MAE: {mae}s (Mean Error)").format(mae=f"{model_mae:.3f}")
            )
            ...
            if reliability_text:
                full_text = tr("r2_explanation", "R2 Note: {text}").format(text=reliability_text)
    ...
    def clear_display(self):
        ...
        self.lbl_model_r2.setText(tr("model_r2_label", "Model R2: -"))
        self.lbl_model_mae.setText(tr("model_mae_label", "Model MAE: -"))
