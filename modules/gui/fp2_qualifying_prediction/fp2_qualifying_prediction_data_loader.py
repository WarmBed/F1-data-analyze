...
"""
FP2→Q 排位賽預測資料載入器
FP2 to Qualifying Prediction Data Loader

負責載入和轉換 CLI Function 76 輸出的 FP2→Q 排位賽預測資料
...
logger = get_logger(component="fp2_qualifying_prediction_loader")


class FP2QualifyingPredictionDataLoader(UniversalDataLoader):
    """
    FP2→Q 排位賽預測資料載入器
    ...
    資料來源：
    - API: refactored_api.py (function_id=76)
    - 本地 JSON: json/fp2_qualifying_prediction_{year}_{race}.json
        ...
        "predictions": [
            {
                ...
                "improvement": float  # predicted - fp2
            }
        ]
    }
    """
    ...
    # CLI 功能編號
    CLI_FUNCTION = 76  # CLI Function 76 - FP2→Q 排位賽預測生成器
    
    # JSON 檔案命名模式
    JSON_PATTERN = "fp2_qualifying_prediction_{year}_{race}.json"
    ...
    def _validate_data_format(self, data: Any) -> bool:
        ...
        try:
            ...
            if len(predictions) == 0:
                self._debug("[VALIDATE] 'predictions' 列表為空")
                return False
            
            # 檢查第一個預測資料是否包含必要欄位（fp2_time 而非 fp3_time）
            first_pred = predictions[0]
            required_pred_fields = [
                "rank", "driver", "team", 
                "fp2_time", "predicted_time", "improvement"
            ]
            missing_pred_fields = [
            ...
            self._debug(f"[VALIDATE]   - 模型 R2: {metadata['model_r2']:.4f}")
    ...
    def _process_data(self, raw_data: Any) -> Dict[str, Any]:
        ...
        if raw_data is None:
            self._debug("[PROCESS_DATA] 原始數據為 None")
            return {}
        
        self._debug("[PROCESS_DATA] 原始數據類型異常，包裝為字典")
    ...
    def _transform_data_for_display(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # ========== 3. 驗證 FP2 預測名次和 Q 名次 ==========
            # 檢查 fp2_predicted_rank 是否存在
            missing_fp2_rank = [p for p in predictions if "fp2_predicted_rank" not in p]
            if missing_fp2_rank:
                self._debug(f"警告: {len(missing_fp2_rank)} 位車手缺少 fp2_predicted_rank")
                self._debug("   執行補償計算...")
                # 補償計算（向後兼容舊 JSON）
                fp2_sorted = sorted(predictions, key=lambda x: x["fp2_time"])
                for rank, pred in enumerate(fp2_sorted, start=1):
                    if "fp2_predicted_rank" not in pred:
                        pred["fp2_predicted_rank"] = rank
            else:
                self._debug(f"所有車手都有 fp2_predicted_rank")
            ...
            if missing_q_rank:
                self._debug(f"警告: {len(missing_q_rank)} 位車手有 Q 時間但缺少 actual_q_rank")
                self._debug("   執行補償計算...")
                # 補償計算（向後兼容舊 JSON）
            ...
            if r2 >= 0.90:
                reliability = tr("r2_excellent", "Excellent (90%+)")
                reliability_color = "green"
            elif r2 >= 0.85:
                reliability = tr("r2_good", "Good (85%+)")
                reliability_color = "darkgreen"
            elif r2 >= 0.75:
                reliability = tr("r2_fair", "Fair (75%+)")
                reliability_color = "orange"
            else:
                reliability = tr("r2_moderate", "Moderate (<75%)")
    ...
    def _generate_data_via_cli(self, **kwargs) -> bool:
        ...
        只能通過以下方式獲取資料：
        1. REST API (refactored_api.py) - function_id=76
        2. 讀取已存在的本地 JSON 檔案
        ...
        self._debug("CLI 命令範例: python f1_analysis_modular_main.py -f 76 -y 2025 -r Austria")
        return False
...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("FP2→Q 排位賽預測資料載入器 - 獨立測試")
    logger.info("=" * 60)
    ...
    def on_data_loaded(data):
        ...
        if "metadata" in data and "predictions" in data:
            ...
            logger.info("  模型 R2: %.4f", metadata.get("model_r2", 0))
