...
try:
    ...
    from modules.gui.race_analysis.track_map.historical_track_map_data_loader import HistoricalTrackMapDataLoader, HistoricalTrackMapApiWorker

# 導入賽道組件
try:
    from modules.gui.race_analysis.track.track_map_widget import TrackMapWidget
    from modules.gui.race_analysis.track_elevation.elevation_chart_widget_pyqt5 import ElevationChartWidget
...
class HistoricalTrackMapMDI(UniversalAnalysisMDI):
    ...
    def initialize_module(self, parent_widget=None, **kwargs) -> bool:
        ...
        try:
            ...
            # ✅ 修復：initialize_module 後立即載入數據
            logger.debug("[HISTORICAL_TRACK_MAP_MDI] 準備調用 _load_data_with_current_parameters()...")
            if self.current_race:  # 確保有 race 參數
                self._load_data_with_current_parameters()
                logger.debug("[HISTORICAL_TRACK_MAP_MDI] _load_data_with_current_parameters() 已調用")
            else:
                logger.warning("[HISTORICAL_TRACK_MAP_MDI] ⚠️ 缺少 race 參數，跳過初始數據載入")
    ...
    def _load_position_changes_data(self) -> Dict[str, int]:
        ...
        try:
            ...
            if len(yearly_summary) == 1 and '2025' in yearly_summary:
                ...
                for year in years:
                    logger.debug(f"   🔍 檢查年份: {year}")
                    if year in yearly_summary:
                        year_data = yearly_summary[year]
                        logger.debug(f"      - {year} 存在於 yearly_summary")
