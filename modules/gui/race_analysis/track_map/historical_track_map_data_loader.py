...
class HistoricalTrackMapDataLoader(UniversalDataLoader):
    ...
    def _validate_load_parameters(self, params: Dict[str, Any]) -> bool:
        ...
        # ⚠️ 檢查年份範圍：Function 100 只支援 2022-2025
        year = params.get('year')
        if year:
            year_int = int(year)
            if year_int < 2022 or year_int > 2025:
                self._debug(f"❌ 年份超出範圍: {year} (Function 100 只支援 2022-2025)")
                return False
        
        self._debug(f"✅ 參數驗證通過: race={race}, year={year or '預設(2022-2025)'}")
        return True
    ...
    def load_data(self, **kwargs) -> bool:
        ...
        if not self._validate_load_parameters(kwargs):
            ...
            if year and (int(year) < 2022 or int(year) > 2025):
                error_msg = f"Historical Track Map 只支援 2022-2025 年的資料，{year} 年資料不可用"
