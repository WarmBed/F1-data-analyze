#!/usr/bin/env python3
"""
TempAnalysisUniversal - F1T 通用溫度分析模組
=============================================

基於通用 MDI 架構實現的溫度分析模組，支援：
- 氣溫變化分析（氣溫、賽道溫度）
- 降雨狀態顯示（有雨/無雨）
- 濕度和風速分析
- 雙Y軸圖表顯示
...
logger = get_logger(component="temp_analysis_mdi")


class TempAnalysisApiWorker(QThread):
    ...
    def run(self):
        try:
            ...
            if self.isInterruptionRequested():
                logger.debug("[TEMP_API_WORKER] 啟動前已被請求中斷，跳過執行")
                return
            ...
            # 再次檢查中斷（在發送請求前）
            if self.isInterruptionRequested():
                logger.debug("[TEMP_API_WORKER] 發送請求前被請求中斷")
                return
            ...
            # 請求完成後檢查中斷
            if self.isInterruptionRequested():
                logger.debug("[TEMP_API_WORKER] API 回應後被請求中斷，放棄處理結果")
                return
            ...
            # 發送信號前最後檢查中斷
            if self.isInterruptionRequested():
                logger.debug("[TEMP_API_WORKER] 發送成功信號前被請求中斷，放棄發送")
                return
...
class TempAnalysisDataManager(UniversalDataLoader):
    """溫度分析數據管理器"""
    
    def __init__(self, parent=None):
        # 註冊溫度分析類型（如果尚未註冊）
        if "temp_weather" not in UniversalDataLoader.ANALYSIS_TYPES:
            temp_config = AnalysisConfig(
                display_name="Temperature Analysis",
                debug_prefix="[TEMP_ANALYSIS]",
                data_source="api",
            ...
            UniversalDataLoader.register_analysis_type("temp_weather", temp_config)
        
        super().__init__("temp_weather", parent)
        ...
        self._api_worker: Optional[TempAnalysisApiWorker] = None
        ...
        logger.debug("[TEMP_DATA_MANAGER] 初始化完成, 搜索目錄: %s", self.config.search_directories)
        logger.debug("[TEMP_DATA_MANAGER] 文件模式: %s", self.config.file_patterns)
    ...
    def _resolve_local_fallback_policy(self) -> Tuple[bool, str]:
        ...
        if env_value is not None:
            ...
            if normalized in {"1", "true", "yes", "on"}:
                return True, f"環境變數 F1T_ALLOW_TEMP_JSON_FALLBACK={env_value}"
    ...
    def _start_api_request(self, params: Dict[str, Any]) -> None:
        ...
        self._api_worker = TempAnalysisApiWorker(self._api_base_url, worker_params, timeout=timeout, parent=self)
    ...
    def _fallback_to_local(self, reason: str) -> None:
        ...
        if not self._allow_local_fallback:
            ...
            message = (
                ...
                " 如需啟用，請設定環境變數 F1T_ALLOW_TEMP_JSON_FALLBACK=1 或使用 set_local_fallback_allowed。"
            )
...
# 導入專用圖表組件
from .temp_analysis_chart_widget import TempAnalysisChartWidget

from core.logger import get_logger
logger = get_logger(__name__)


class TempAnalysisControlWidget(QWidget):
    """溫度分析控制面板"""
    ...
    def setup_ui(self):
        ...
        self.chart_combo.addItems([
            tr("main_chart_temp_wind_rain", "Temperature & Wind (with Rainfall)"),
            tr("temperature_comparison_air_track", "Temperature Comparison (Air vs Track)"),
            tr("humidity_windspeed", "Humidity & Wind Speed"),
            tr("pressure_changes", "Pressure Changes")
        ])
        self.chart_combo.currentTextChanged.connect(self._on_chart_type_changed)
        
        chart_layout.addWidget(QLabel(tr("select_chart", "Select Chart:")), 0, 0)
    ...
    def _on_chart_type_changed(self, text: str):
        ...
        chart_type_map = {
            ...
            "Temperature & Wind (with Rainfall)": "primary",
...
class TempAnalysisUniversal(UniversalAnalysisMDI):
    """
    通用溫度分析 MDI 模組
    
    基於通用 MDI 架構實現的完整溫度分析功能，
    支援多種天氣數據的視覺化和分析。
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info("[TEMP_MDI] TempAnalysisUniversal 開始初始化...")
        
        # 註冊溫度分析模組類型
        if "temp_weather" not in UniversalAnalysisMDI.MDI_MODULE_TYPES:
            temp_config = AnalysisMDIConfig(
                analysis_type="temp_weather",
                display_name="Temperature Analysis",
            ...
            UniversalAnalysisMDI.register_mdi_module_type("temp_weather", temp_config)
            
        super().__init__("temp_weather", parent)
        logger.info("[TEMP_MDI] 基類初始化完成, 數據管理器: %s", self.data_manager)
        
        # 初始化模組組件
        logger.info("[TEMP_MDI] 開始初始化模組組件...")
        if not self.initialize_module():
            logger.error("[TEMP_MDI] ❌ 模組組件初始化失敗")
            return
        
        logger.info("[TEMP_MDI] ✅ 模組組件初始化完成")
        logger.debug("[TEMP_MDI] 數據管理器: %s", self.data_manager)
        logger.debug("[TEMP_MDI] 圖表組件: %s", self.chart_widget)
    ...
    def create_data_manager(self) -> TempAnalysisDataManager:
        """創建溫度分析數據管理器"""
        return TempAnalysisDataManager(self)
        
    def create_chart_widget(self) -> TempAnalysisChartWidget:
        """創建溫度分析圖表組件"""
        return TempAnalysisChartWidget(self)
        
    def create_control_widget(self) -> TempAnalysisControlWidget:
        """創建溫度分析控制面板"""
        control_widget = TempAnalysisControlWidget(self)
    ...
    def update_lap_parameters(self, year: str, race: str, session: str, **kwargs) -> bool:
        """更新溫度分析參數"""
        try:
            logger.info("[TEMP_MDI] ========== 溫度參數更新 ==========")
            logger.info("[TEMP_MDI] 收到參數: %s %s %s", year, race, session)
            ...
            # ✅ 更新視窗標題（響應賽事切換）
            logger.debug("[TEMP_MDI] 更新視窗標題...")
            self.update_window_title()
            
            # 更新數據管理器參數
            if hasattr(self, 'data_manager') and self.data_manager:
                logger.debug("[TEMP_MDI] 更新數據管理器參數...")
                ...
                # 載入數據 - 傳遞正確的參數
                logger.info("[TEMP_MDI] 開始載入數據...")
                result = self.data_manager.load_data(
                    year=self.current_year,
                    race=self.current_race,
                    session=self.current_session
                )
                logger.info("[TEMP_MDI] 數據載入結果: %s", result)
                ...
                if result and hasattr(self, 'chart_widget') and self.chart_widget:
                    ...
                    if charts_payload:
                        logger.info("[TEMP_MDI] 更新圖表數據...")
                        chart_data = {"charts_data": charts_payload}
                        self.chart_widget.update_data(chart_data)
            
            logger.info("[TEMP_MDI] 參數更新完成")
            return True
            
        except Exception as e:
            logger.exception("[TEMP_MDI] 參數更新失敗", exc_info=e)
            return False
    ...
    def resizeEvent(self, event):
        ...
        try:
            ...
            logger.debug(
                "[TEMP_MDI] resizeEvent: MDI視窗縮放 %sx%s -> %sx%s",
                old_size.width(),
                old_size.height(),
                new_size.width(),
                new_size.height(),
            ...
            if hasattr(self, 'chart_widget') and self.chart_widget:
                if hasattr(self.chart_widget, 'update_chart_layout'):
                    logger.debug("[TEMP_MDI] resizeEvent: 觸發圖表重新佈局")
                    self.chart_widget.update_chart_layout()
                else:
                    logger.debug("[TEMP_MDI] resizeEvent: 圖表組件不支援動態佈局更新")
            else:
                logger.debug("[TEMP_MDI] resizeEvent: 圖表組件尚未初始化")
                
        except Exception as e:
            logger.exception("[ERROR] [TEMP_MDI] resizeEvent 處理失敗", exc_info=e)
    ...
    def set_responsive_layout(self):
        ...
        try:
            ...
            logger.info("[TEMP_MDI] 響應式佈局已設置")
            
        except Exception as e:
            logger.exception("[ERROR] [TEMP_MDI] 設置響應式佈局失敗", exc_info=e)
...
# 模組註冊 - 確保在導入時自動註冊
def register_temp_analysis_module():
    """註冊溫度分析模組"""
    try:
        # 這裡可以添加到全局模組註冊表
        pass
    except Exception as e:
        logger.warning("[WARNING] 溫度分析模組註冊失敗: %s", str(e))


# 自動註冊
register_temp_analysis_module()


class TempAnalysisModule(TempAnalysisUniversal):
    """向後相容的別名，供既有匯入路徑使用"""
