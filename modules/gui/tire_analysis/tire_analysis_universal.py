#!/usr/bin/env python3
"""
tireAnalysisUniv    def __init__(self, parent=None):
        # 註冊輪胎分析類型（如果尚未註冊）
        if "tire_analysis" not in UniversalDataLoader.ANALYSIS_TYPES:
            tire_config = AnalysisConfig(
                display_name="輪胎分析",
                debug_prefix="[TIRE_ANALYSIS]",
                data_source="json",
                cli_function="run_cli_modular_main",
                file_patterns=[
                    "tire_timing_inference_{year}_{race}_{session}_all_drivers.json",
                    "tire_timing_inference_{year}_{race_full}_{session}_all_drivers.json",
                    "tire_timing_corrected_{year}_{race}_{session}.json",
                    "all_drivers_tire_strategy_{year}_{race}_{session}.json",
                    "enhanced_tire_analysis_{year}_{race}_{session}.json",
                    "tire_strategy_{year}_{race}_{session}_all_drivers.json",
                    "tire_analysis_{year}_{race}_{session}.json"
                ],
                search_directories=["json", "json_exports", "cache"],
                supports_realtime=False,
                cache_enabled=True
            )
            UniversalDataLoader.register_analysis_type("tire_analysis", tire_config)模組
=============================================

基於通用 MDI 架構實現的輪胎分析模組，支援：
- 降雨狀態分析（有雨/無雨）
- 溫度變化分析（氣溫、賽道溫度）
- 濕度和風速分析
- 雙Y軸圖表顯示
...
數據來源：enhanced_tire_analysis JSON 檔案
圖表類型：雙Y軸折線圖、柱狀圖
...
class tireAnalysisDataManager(UniversalDataLoader):
    ...
    def __init__(self, parent=None):
        # 註冊輪胎分析類型（如果尚未註冊）
        if "tire_weather" not in UniversalDataLoader.ANALYSIS_TYPES:
            tire_config = AnalysisConfig(
                display_name="輪胎分析",
                debug_prefix="[tire_ANALYSIS]",
                data_source="json",
                cli_function="run_tire_intensity_analysis_json",
                file_patterns=[
                    "enhanced_tire_analysis_{year}_{race}_{session}.json",
                    "tire_analysis_{year}_{race}_{session}.json",
            ...
            UniversalDataLoader.register_analysis_type("tire_weather", tire_config)
    ...
    def _build_filename_patterns(self, year: str, race: str, session: str, **kwargs) -> List[str]:
        ...
        race_variants = [
            race,  # 原始格式，例如: Japan
            f"{race}_Grand_Prix",  # 例如: Japan_Grand_Prix  
            f"{race}ese_Grand_Prix" if race == "Japan" else f"{race}_Grand_Prix",  # 特殊處理日本站: Japanese_Grand_Prix
        ]
        
        # Session 變體
        session_variants = [
            session,  # 原始格式，例如: R
            "Race" if session == "R" else session,
            "None"  # CLI -f26 生成的格式
        ]
        ...
        for pattern in self.config.file_patterns:
            for race_variant in race_variants:
                for session_variant in session_variants:
                    try:
                        if "{race_full}" in pattern:
                            filename = pattern.format(
                                year=year, 
                                race_full=race_variant, 
    ...
    def _generate_data_via_cli(self, year: str, race: str, session: str, **kwargs) -> Optional[Dict[str, Any]]:
        """通過 CLI 生成數據"""
        # 輪胎分析通常不需要即時生成，直接返回 None
        self._debug("輪胎分析不支援 CLI 數據生成")
        return None
    ...
    def _process_lap_weather_data(self) -> Dict[str, List]:
        ...
        for lap_num in lap_numbers:
            ...
            # 降雨狀態（布林值轉數值）
            tirefall_status = lap_data.get("weather", {}).get("tirefall", False)
            tirefall.append(1 if tirefall_status else 0)
    ...
    def _prepare_chart_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "primary": {
                ...
                "y1_data": lap_data["tirefall"],  # 左Y軸：降雨狀態
    ...
    def get_tire_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tire_laps": self.summary_stats.get("tire_laps", 0),
            "tire_percentage": self.summary_stats.get("tire_percentage", 0.0),
            "has_tire_data": self.summary_stats.get("has_tire_data", False),
            "tire_timing": self.summary_stats.get("tire_timing_analysis", {})
        }
...
class tireAnalysisChartWidget(TelemetryChartWidgetBase):
    ...
    def __init__(self, parent=None):
        ...
        # 設定圖表樣式
        self.setup_tire_chart_style()
        
    def setup_tire_chart_style(self):
        """設定輪胎分析圖表樣式"""
        # 降雨相關顏色配置
        self.tire_colors = {
            "tirefall": "#3498db",      # 藍色 - 降雨
...
class tireAnalysisUniversal(UniversalAnalysisMDI):
    ...
    基於通用 MDI 架構實現的完整輪胎分析功能，
    支援多種天氣數據的視覺化和分析。
    """
    
    def __init__(self, parent=None):
        # 註冊輪胎分析模組類型
        if "tire" not in UniversalAnalysisMDI.MDI_MODULE_TYPES:
            tire_config = AnalysisMDIConfig(
                analysis_type="tire",
                display_name="輪胎分析",
                default_size=(1400, 900),
                requires_driver_params=False,  # 輪胎分析不需要車手參數
                requires_lap_params=False,     # 輪胎分析不需要圈數參數
                supports_single_driver=False,
                supports_dual_driver=False,
                chart_types=["primary", "temperature", "humidity_wind", "pressure"]
            )
            UniversalAnalysisMDI.register_mdi_module_type("tire", tire_config)
            
        super().__init__("tire", parent)
    ...
    def create_chart_widget(self) -> tireAnalysisChartWidget:
        """創建輪胎分析圖表組件"""
        return tireAnalysisChartWidget(self)
    ...
    def update_lap_parameters(self, year: str, race: str, session: str, **kwargs) -> bool:
        """更新降雨分析參數"""
        try:
            print(f"[tire_UNIVERSAL] ========== 降雨參數更新 ==========")
            print(f"[tire_UNIVERSAL] 收到參數: {year} {race} {session}")
            ...
            print(f"[tire_UNIVERSAL] 參數更新完成")
            return True
            
        except Exception as e:
            print(f"[tire_UNIVERSAL] 參數更新失敗: {str(e)}")
            return False
...
def register_tire_analysis_module():
    ...
    try:
        ...
        print(f"[WARNING] 輪胎分析模組註冊失敗: {str(e)}")
