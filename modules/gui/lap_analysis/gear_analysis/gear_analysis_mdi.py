...
class GearDataManager(QObject):
    ...
    def load_gear_data(self, year: str, race: str, session: str, 
        ...
        try:
            ...
            self.gear_loader.data_loaded.connect(self._on_data_loaded)
            self.gear_loader.load_error.connect(self._on_load_error)
            self.gear_loader.status_changed.connect(self.status_changed.emit)
            self.gear_loader.load_progress.connect(self.loading_progress.emit)
            
            # 開始載入數據
            success = self.gear_loader.load_gear_data(
                year=int(year),
                race=race,
                session=session,
                driver1=driver1,
...
class GearAnalysisModule(IAnalysisModule):
    """檔位分析主模組"""
    ...
    def initialize_module(self, parent_widget=None, **kwargs) -> bool:
        ...
        try:
            ...
            self.gear_chart_widget = GearAnalysisChartWidget()
            
            # 連接圈數變更信號
            self.gear_chart_widget.lap_numbers_changed.connect(self._on_lap_numbers_changed)
            
            # 設置初始圈數
            self.gear_chart_widget.set_lap_numbers(self.lap1, self.lap2)
            ...
            try:
                ...
                # 註冊模組
                module_id = f"gear_analysis_{id(self)}"
                manager.register_module(module_id, self, "gear_analysis")
                
                # 註冊圖表組件
                if self.gear_chart_widget:
                    manager.register_chart_widget(self.gear_chart_widget)
    ...
    def _create_placeholder_widget(self):
        """創建佔位組件（當gear圖表組件不可用時）"""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        
        label = QLabel("🔄 gear分析圖表")
        label.setAlignment(Qt.AlignCenter)
        label.setStyleSheet("font-size: 16pt; padding: 20px;")
        layout.addWidget(label)
        
        info_label = QLabel("gear圖表組件正在載入中...")
    ...
    def _setup_ui(self):
        ...
        # 添加gear圖表
        if self.gear_chart_widget:
            layout.addWidget(self.gear_chart_widget)
    ...
    def get_default_size(self) -> tuple:
        """獲取預設視窗大小"""
        return (1000, 700)  # gear分析需要較大的視窗來顯示詳細圖表
    ...
    def _update_chart(self, data: dict):
        ...
        try:
            ...
            if self.gear_chart_widget:
                ...
                if use_time_axis and hasattr(self.gear_chart_widget, 'set_time_axis_mode'):
                    logger.debug(f"[gear_MDI] ⏱️ 數據載入完成，應用時間軸模式: {use_time_axis}")
                    self.gear_chart_widget.set_time_axis_mode(use_time_axis)
        ...
        try:
            ...
            if "telemetry_comparison" in data:
                ...
                if "nGear" in telemetry_comp:
                    gear_telemetry = telemetry_comp["nGear"]
                    
                    # 構建符合 _update_chart 期望的數據格式
                    chart_data = {
                        "gear_data": {
                            "distance": gear_telemetry.get("distance", []),
                            "driver1_gear": gear_telemetry.get("driver1_data", []),
                            "driver2_gear": gear_telemetry.get("driver2_data", []),
                            # 🆕 新增時間數據
                            "driver1_time_seconds": gear_telemetry.get("driver1_time_seconds", []),
                            "driver2_time_seconds": gear_telemetry.get("driver2_time_seconds", []),
                        },
                        "metadata": data.get("comparison_info", {}),
    ...
    def update_from_shared_params(self, params: dict):
        ...
        try:
            ...
            race1 = params.get('race1', self.driver1_race if hasattr(self, 'driver1_race') else self.current_race)
            session1 = params.get('session1', self.driver1_session if hasattr(self, 'driver1_session') else self.current_session)
            driver1 = params.get('driver1', self.driver1)
            lap1 = params.get('lap1', self.lap1)
            
            year2 = params.get('year2', self.driver2_year if hasattr(self, 'driver2_year') else self.current_year)
            race2 = params.get('race2', self.driver2_race if hasattr(self, 'driver2_race') else self.current_race)
            session2 = params.get('session2', self.driver2_session if hasattr(self, 'driver2_session') else self.current_session)
            driver2 = params.get('driver2', self.driver2)
            lap2 = params.get('lap2', self.lap2)
            
            use_time_axis = params.get('use_time_axis', getattr(self, 'use_time_axis', False))
        ...
        try:
            ...
            if hasattr(self, '_analysis_manager') and self._analysis_manager and hasattr(self, '_module_id'):
                try:
                    # 解除註冊圖表組件
                    if hasattr(self, 'gear_chart_widget') and self.gear_chart_widget:
                        self._analysis_manager.unregister_chart_widget(self.gear_chart_widget)
            ...
            if hasattr(self, 'gear_chart_widget') and self.gear_chart_widget:
                ...
                if hasattr(self.gear_chart_widget, 'cleanup'):
                    self.gear_chart_widget.cleanup()
                self.gear_chart_widget.deleteLater()
    ...
    def load_data(self, **kwargs) -> bool:
        ...
        try:
            ...
            logger.debug(f"[gear_MDI] load_data 參數: use_time_axis={use_time_axis}")
            ...
            if self.data_manager:
                ...
                # 數據載入成功後設置時間軸模式
                if success and use_time_axis and self.gear_chart_widget:
                    if hasattr(self.gear_chart_widget, 'set_time_axis_mode'):
                        logger.debug(f"[gear_MDI] 設置圖表時間軸模式: {use_time_axis}")
                        self.gear_chart_widget.set_time_axis_mode(use_time_axis)
