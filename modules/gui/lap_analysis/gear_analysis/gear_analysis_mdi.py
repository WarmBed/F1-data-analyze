...
class GearDataManager(QObject):
    ...
    def _check_and_load_telemetry_if_needed(self):
        ...
        try:
            ...
            print(f"[gear_MDI_DATA] � 未找到遙測數據，通過CLI生成...")
    ...
    def _generate_telemetry_via_cli(self) -> bool:
        ...
        try:
            ...
            print(f"[gear_MDI_DATA] 🔧 執行CLI命令: {' '.join(command)}")
            
            # 同步執行CLI命令（因為gear分析需要立即使用結果）
            ...
            if process.returncode == 0:
                print(f"[gear_MDI_DATA] ✅ 遙測分析CLI執行成功")
                time.sleep(2)  # 等待檔案寫入完成
                return True
            else:
                print(f"[gear_MDI_DATA] ❌ 遙測分析CLI執行失敗: {stderr}")
                return False
                
        except subprocess.TimeoutExpired:
            print(f"[gear_MDI_DATA] ⏰ 遙測分析CLI執行超時")
            return False
        except Exception as e:
            print(f"[ERROR] [gear_MDI_DATA] _generate_telemetry_via_cli 失敗: {e}")
            return False
...
class GearAnalysisModule(IAnalysisModule):
    ...
    def get_window_title(self, year: str = None, race: str = None, session: str = None) -> str:
        ...
        print(f"[gear_TITLE_DEBUG]     - 年份: {use_year}")
        print(f"[gear_TITLE_DEBUG]     - 賽事: {use_race}")
        print(f"[gear_TITLE_DEBUG]     - 賽段: {use_session}")
        return title
    ...
    def update_lap_parameters(self, year: str, race: str, session: str, 
        ...
        try:
            ...
            if params_changed:
                ...
                print(f"[gear_MDI] ℹ️ 參數未變更，跳過數據重載")
                return True
