...
class GearChartWidget(QWidget, LapAnalysisLinkageMixin, LapAnalysisLinkageDrawingMixin):
    """gear圖表繪製組件 - 使用 PyQt5 原生繪圖"""
    ...
    def __init__(self, parent=None):
        ...
        self.min_gear = 0.5     # 檔位範圍：0.5-8.5，給1-8檔位留出邊距
        self.max_gear = 8.5
    ...
    def set_gear_data(self, distance: List[float], driver1_gear: List[float], 
        ...
        self.driver2_gear = driver2_gear
        ...
        all_gears = []
        if driver1_gear:
            all_gears.extend(driver1_gear)
        if driver2_gear:
            all_gears.extend(driver2_gear)
            
        if all_gears:
            # 檔位範圍：根據實際檔位數據設置，給1-8檔留出適當邊距
            min_gear_data = min(all_gears)
            max_gear_data = max(all_gears)
            self.min_gear = max(0.5, min_gear_data - 0.5)  # 最小0.5檔
            self.max_gear = min(8.5, max_gear_data + 0.5)  # 最大8.5檔
    ...
    def _draw_grid(self, painter: QPainter, chart_rect: QRect):
        ...
        # 水平網格線 (gear) - 檔位專用：為每個檔位繪製網格線
        gear_range = current_max_gear - current_min_gear
        if gear_range > 0:
            # 為1-8檔分別繪製網格線
            for gear_num in range(1, 9):
                if current_min_gear <= gear_num <= current_max_gear:
                    y = chart_rect.bottom() - (gear_num - current_min_gear) / gear_range * chart_rect.height()
    ...
    def _draw_axes(self, painter: QPainter, chart_rect: QRect):
        ...
        # Y軸標籤 (gear) - 檔位專用：只顯示1-8檔
        gear_range = current_max_gear - current_min_gear
        if gear_range > 0:
            # 對於檔位，直接顯示1-8檔的標籤
            for gear_num in range(1, 9):  # 1到8檔
                if current_min_gear <= gear_num <= current_max_gear:
                    y = chart_rect.bottom() - (gear_num - current_min_gear) / gear_range * chart_rect.height()
        ...
        painter.rotate(-90)
        painter.drawText(-50, -10, 100, 20, Qt.AlignCenter, "gear (轉/分)")
        painter.restore()
    ...
    def _draw_tracking_line(self, painter: QPainter, chart_rect: QRect, x_pos: int, is_fixed: bool):
        ...
        if distance_range > 0 and self.distance_data:
            ...
            if self.distance_data and len(self.distance_data) > 0:
                ...
                # 獲取對應的gear值
                if closest_index < len(self.driver1_gear):
                    driver1_gear_at_position = self.driver1_gear[closest_index]
                if closest_index < len(self.driver2_gear):
                    driver2_gear_at_position = self.driver2_gear[closest_index]
            
            # 計算需要顯示的車手數量來調整標籤大小
            drivers_to_show = []
            
            # 只添加有效且不重複的車手資訊
            if driver1_gear_at_position is not None and self.driver1_name:
                drivers_to_show.append((self.driver1_name, driver1_gear_at_position, self.driver1_color))
            
            # 只有在非單車手模式且第二個車手數據不同時才添加第二個車手
            if (not getattr(self, 'is_single_driver', False) and 
                driver2_gear_at_position is not None and 
                self.driver2_name and 
                self.driver2_name != self.driver1_name):
                drivers_to_show.append((self.driver2_name, driver2_gear_at_position, self.driver2_color))
            ...
            for i, (driver_name, gear, color) in enumerate(drivers_to_show):
                ...
                painter.drawText(label_x + 5, text_y + 15 + (i * 15), f"{driver_name}: {gear:.0f} gear")
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self.middle_dragging and not self.last_drag_pos.isNull():
            ...
            if chart_rect.width() > 0 and chart_rect.height() > 0:
                ...
                # Y軸移動（gear）
                gear_range = (self.view_max_gear or self.max_gear) - (self.view_min_gear or self.min_gear)
                gear_move = dy * gear_range / chart_rect.height()  # Y軸是倒置的
                
                # 更新視圖範圍
                if self.view_min_distance is None:
                    self.view_min_distance = self.min_distance
                    self.view_max_distance = self.max_distance
                if self.view_min_gear is None:
                    self.view_min_gear = self.min_gear
                    self.view_max_gear = self.max_gear
                
                self.view_min_distance += distance_move
                self.view_max_distance += distance_move
                self.view_min_gear += gear_move
                self.view_max_gear += gear_move
    ...
    def wheelEvent(self, event: QWheelEvent):
        ...
        if chart_rect.contains(event.pos()):
            ...
            gear_range = self.view_max_gear - self.view_min_gear
            
            mouse_distance = self.view_min_distance + mouse_rel_x * distance_range
            mouse_gear = self.view_min_gear + mouse_rel_y * gear_range
            
            # 計算新的範圍
            new_distance_range = distance_range / zoom_factor
            new_gear_range = gear_range / zoom_factor
            ...
            self.view_min_gear = max(self.min_gear, 
                                  mouse_gear - new_gear_range * mouse_rel_y)
            self.view_max_gear = min(self.max_gear, 
                                  mouse_gear + new_gear_range * (1 - mouse_rel_y))
...
class GearAnalysisChartWidget(QWidget, LapAnalysisLinkageMixin, LapAnalysisLinkageDrawingMixin):
    """gear分析圖表組件主容器"""
    ...
    def __init__(self, parent=None):
        ...
        # 註冊到連動管理器
        if linkage_manager:
            linkage_manager.register_module(self, "gear_analysis")
    ...
    def _create_chart_area(self) -> QWidget:
        ...
        # 創建圖表組件
        self.chart_widget = GearChartWidget()
        layout.addWidget(self.chart_widget)
        
        # 確保內部圖表組件也註冊到連動管理器
        if linkage_manager:
            linkage_manager.register_module(self.chart_widget, "gear_analysis_chart")
    ...
    def update_gear_data(self, data: Dict[str, Any]):
        """更新gear數據 - 採用速度分析的更新邏輯"""
        ...
        try:
            ...
            driver1_gear = gear_data.get('driver1_gear', [])
            driver2_gear = gear_data.get('driver2_gear', [])
            ...
            if metadata.get('is_single_driver', False):
                ...
                # 相同車手比較（如 VER vs VER）
                is_single_driver_mode = True
                print(f"[gear_CHART] 🔍 檢測到相同車手比較: {driver1_name} vs {driver2_name}")
            elif len(drivers) == 1:
                # 只有一個車手的數據
                is_single_driver_mode = True
    ...
    def _prepare_chart_data(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """準備圖表數據"""
        try:
            if 'gear_telemetry' in data:
                # 直接gear數據
                return self._parse_gear_telemetry(data['gear_telemetry'])
            
            elif 'speed_data' in data:
                # 從速度數據模擬gear數據
                return self._simulate_gear_from_speed(data['speed_data'])
    ...
    def _parse_gear_telemetry(self, gear_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 解析車手1數據
        if 'driver1_gear_data' in gear_data:
            for point in gear_data['driver1_gear_data']:
                distance.append(point.get('distance', 0))
                driver1_gear.append(point.get('gear', 0))
        
        # 解析車手2數據
        if 'driver2_gear_data' in gear_data:
            for point in gear_data['driver2_gear_data']:
                driver2_gear.append(point.get('gear', 0))
        ...
        return {
            ...
            'driver1_name': gear_data.get('driver1_name', 'Driver 1'),
            'driver2_name': gear_data.get('driver2_name', 'Driver 2'),
            'sectors': gear_data.get('sectors', []),
            'engine_info': gear_data.get('engine_info', {}),
            'track_info': gear_data.get('track_info', {})
        }
    ...
    def _update_statistics_table(self, statistics: Dict, driver1_name: str, driver2_name: str):
        ...
        try:
            ...
            rows = [
                ("最高轉速 (gear)", 
                 f"{driver1_stats.get('max_gear', 0):.0f}",
                 f"{driver2_stats.get('max_gear', 0):.0f}",
                 f"{comparison.get('max_gear_diff', 0):.0f}"),
                ("平均轉速 (gear)",
                 f"{driver1_stats.get('avg_gear', 0):.0f}",
                 f"{driver2_stats.get('avg_gear', 0):.0f}",
                 f"{comparison.get('avg_gear_diff', 0):.0f}"),
                ("最低轉速 (gear)",
                 f"{driver1_stats.get('min_gear', 0):.0f}",
                 f"{driver2_stats.get('min_gear', 0):.0f}",
    ...
    def reload_data(self):
        ...
        if self.current_data:
            self.update_gear_data(self.current_data)
...
if __name__ == "__main__":
    ...
    # 測試gear圖表組件
    widget = GearAnalysisChartWidget()
    widget.setWindowTitle("🔄 gear分析圖表測試")
