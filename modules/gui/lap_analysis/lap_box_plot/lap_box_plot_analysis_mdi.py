    ...
    def _extract_lap_times(self, all_drivers: Dict) -> Dict[str, List[float]]:
        ...
        for driver_code, driver_data in all_drivers.items():
            ...
            for lap in detailed_laps:
                ...
                # ✅ 絕對閾值過濾：即使未標記為進站圈，仍過濾掉明顯異常的慢圈
                # 使用動態閾值：正常 FP2/練習賽圈速範圍通常不超過 110 秒
                # 這是第一道防線，用於補償進站圈檢測不準確的情況
                if self.filter_settings.get('filter_pit_laps', True):
                    # 對於練習賽/排位賽，110 秒是合理上限（正常圈速通常 80-95 秒）
                    # 超過此值通常是未標記的進站圈、車庫停留、或嚴重異常情況
                    ABSOLUTE_LAP_TIME_THRESHOLD = 110.0  # 秒
                    if lap_time_float > ABSOLUTE_LAP_TIME_THRESHOLD:
                        continue
...
class LapTimeBoxPlotAnalysis(UniversalAnalysisMDI):
    ...
    def _on_global_boxplot_settings_changed(self, settings: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if processed and chart_ready:
                # 過濾設定變更時重置 Y 軸為自動範圍
                if hasattr(self.chart_widget, 'reset_y_axis_range'):
                    self.chart_widget.reset_y_axis_range()
    ...
    def _on_reload_requested(self):
        ...
        # 重新載入時重置 Y 軸為自動範圍
        if self.chart_widget and hasattr(self.chart_widget, 'reset_y_axis_range'):
            self.chart_widget.reset_y_axis_range()
