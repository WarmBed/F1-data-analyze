...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QMessageBox, QMenu, QAction, QInputDialog
...
class LapTimeBoxPlotChartWidget(QWidget):
    ...
    def _ensure_palette_for_data(self, data: Dict[str, Any]) -> None:
        """Ensure the shared colour palette matches the data's season."""
        ...
        try:
            ...
            # Provider already falls back to defaults on failure; no further action required.
            pass

    def _driver_color(self, driver: str) -> QColor:
        """Return the QColor associated with the given driver code."""
    ...
    def paintEvent(self, event):
        ...
        # 🔧 強制寫入檔案調試
        try:
            with open('boxplot_debug.txt', 'a', encoding='utf-8') as f:
                f.write(f"paintEvent called, drivers: {len(self.driver_laptimes)}\n")
        except:
            pass
    ...
    def _draw_box_plots(self, painter: QPainter):
        """繪製所有車手的箱型圖"""
        print(f"[BOXPLOT] 📊 _draw_box_plots 被調用")
        if not self.driver_laptimes:
            return
    ...
    def _draw_single_box_plot(
        ...
        try:
            # 🔧 調試：確認新代碼被執行
            print(f"[BOXPLOT] 🎨 繪製新版箱型圖+散點圖: {driver}, 數據點數: {len(lap_times)}")
    ...
    def _draw_legend(self, painter: QPainter):
        ...
        items = [
            ...
            (tr("lap_box_plot.legend_outlier", "Outlier"), Qt.black, "outlier")
        ]
    ...
    def contextMenuEvent(self, event):
        """右鍵選單事件"""
        logger.debug(f"[BOXPLOT_CHART] contextMenuEvent 觸發, pos=({event.pos().x()}, {event.pos().y()})")
        
        # 只有在 Y 軸區域才顯示選單
        if not self._is_on_y_axis(event.pos()):
            logger.debug(f"[BOXPLOT_CHART] 點擊位置不在 Y 軸區域, margin_left={self.margin_left}")
            return
        
        logger.debug("[BOXPLOT_CHART] 點擊在 Y 軸區域, 顯示選單")
        menu = QMenu(self)
    ...
    def set_custom_y_range(self, y_min: float, y_max: float):
        """
        設定自定義 Y 軸範圍
        ...
        logger.debug(f"[BOXPLOT_CHART] 設定自定義 Y 軸範圍: {y_min:.2f} - {y_max:.2f}")
    
    def reset_y_axis_range(self):
        """
        重置 Y 軸為自動範圍
        
        當使用 "Show All Data" 或重新載入數據時呼叫
        """
        ...
        logger.debug("[BOXPLOT_CHART] Y 軸範圍已重置為自動")
