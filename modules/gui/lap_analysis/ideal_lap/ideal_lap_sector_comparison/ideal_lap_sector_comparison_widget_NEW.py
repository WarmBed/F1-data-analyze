...
class IdealLapSectorComparisonWidget(QWidget):
    ...
    def _calculate_x_range(self):
        ...
        for driver_data in self.comparison_data:
            ...
            fastest_total = sum(driver_data.get("fastest_sectors", [0, 0, 0]))
            all_times.extend([ideal_total, fastest_total])
    ...
    def _draw_single_driver_bars(
        ...
        try:
            ...
            fastest_sectors = driver_data.get("fastest_sectors", [0, 0, 0])
    ...
    def sort_data(self, sort_key: str):
        ...
        if sort_key == "position":
            ...
            sorted_data = sorted(self.comparison_data, key=lambda x: sum(x.get("ideal_sectors", [0, 0, 0])))
        elif sort_key == "fastest_lap":
            sorted_data = sorted(self.comparison_data, key=lambda x: sum(x.get("fastest_sectors", [0, 0, 0])))
        elif sort_key == "delta":
            sorted_data = sorted(
                self.comparison_data,
                key=lambda x: sum(x.get("fastest_sectors", [0, 0, 0])) - sum(x.get("ideal_sectors", [0, 0, 0])),
                reverse=True
            )
        else:
