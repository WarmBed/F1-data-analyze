...
class speeddiffChartWidget(QWidget, LapAnalysisLinkageMixin, LapAnalysisLinkageDrawingMixin):
    """speeddiff圖表繪製組件 - 使用 PyQt5 原生繪圖"""
    ...
    def __init__(self, parent=None):
        ...
        # 數據存儲
        self.speed_data = []
        self.driver1_speeddiff = []
        self.driver2_speeddiff = []
        ...
        self.min_speed = 0
        self.max_speed = 5807
        self.min_speeddiff = -100  # 改為-100
        self.max_speeddiff = 100   # 改為+100
    ...
    def set_speeddiff_data(self, speed: List[float], driver1_speeddiff: List[float], 
        ...
        Args:
            speed: 速度數據
            driver1_speeddiff: 速度差數據
            driver2_speeddiff: (unused for single-curve mode)
            driver1_name: 速度差標籤名稱
        ...
        self.driver2_speeddiff = driver2_speeddiff
        ...
        # 計算速度差的動態範圍
        all_speeddiffs = []
        if driver1_speeddiff:
            all_speeddiffs.extend(driver1_speeddiff)
        if driver2_speeddiff:
            all_speeddiffs.extend(driver2_speeddiff)
            
        if all_speeddiffs:
            # 為速度差計算合適的範圍
            data_min = min(all_speeddiffs)
            data_max = max(all_speeddiffs)
            
            # 添加一些邊距以便更好地顯示
            margin = max(10, (data_max - data_min) * 0.1)  # 至少10單位的邊距
            
            self.min_speeddiff = data_min - margin
            self.max_speeddiff = data_max + margin
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            # 4. 繪製speeddiff曲線
            self._draw_speeddiff_curves(painter, chart_rect)
            
            # 5. 繪製連動線 (使用混入類方法)
            if hasattr(self, 'speed_data') and self.speed_data is not None:
                self.draw_linkage_line(
                    ...
                    getattr(self, 'driver1_speeddiff', []),
                    getattr(self, 'driver2_speeddiff', []),
                    "m"
                )
    ...
    def _draw_grid(self, painter: QPainter, chart_rect: QRect):
        ...
        if speed_range > 0:
            ...
            for i in range(num_v_lines + 1):
                speed = current_min_speed + (speed_range * i / num_v_lines)
        ...
        # 水平網格線 (speeddiff) - 修正：與速度分析保持一致使用10條線
        speeddiff_range = current_max_speeddiff - current_min_speeddiff
        if speeddiff_range > 0:
            num_h_lines = 10  # 修正：改為10條線與速度分析一致
            for i in range(num_h_lines + 1):
                speeddiff = current_min_speeddiff + (speeddiff_range * i / num_h_lines)
    ...
    def _draw_axes(self, painter: QPainter, chart_rect: QRect):
        ...
        if speed_range > 0:
            ...
            for i in range(0, num_labels + 1, 2):  # 只顯示偶數刻度
                speed = current_min_speed + (speed_range * i / num_labels)
                x = chart_rect.left() + (speed - current_min_speed) / speed_range * chart_rect.width()
        ...
        # Y軸標籤 (speeddiff) - 修正：與速度分析一致，只顯示偶數刻度
        speeddiff_range = current_max_speeddiff - current_min_speeddiff
        if speeddiff_range > 0:
            num_labels = 10  # 使用10個間隔
            for i in range(0, num_labels + 1, 2):  # 只顯示偶數刻度
                speeddiff = current_min_speeddiff + (speeddiff_range * i / num_labels)
                y = chart_rect.bottom() - (speeddiff - current_min_speeddiff) / speeddiff_range * chart_rect.height()
    ...
    def _draw_sectors(self, painter: QPainter, chart_rect: QRect):
        ...
        for sector in self.sectors:
            if 'end_speed' in sector:
                end_speed = sector['end_speed']
                x = chart_rect.left() + (end_speed - current_min_speed) / speed_range * chart_rect.width()
                painter.drawLine(int(x), chart_rect.top(), int(x), chart_rect.bottom())
    ...
    def _draw_speeddiff_curves(self, painter: QPainter, chart_rect: QRect):
        ...
        if speed_range <= 0 or speeddiff_range <= 0:
            return
        
        # 繪製零線 (Y=0 的水平線)
        if current_min_speeddiff <= 0 <= current_max_speeddiff:
            zero_y = chart_rect.bottom() - (0 - current_min_speeddiff) / speeddiff_range * chart_rect.height()
    ...
    def _draw_tracking_line(self, painter: QPainter, chart_rect: QRect, x_pos: int, is_fixed: bool):
        ...
        if speed_range > 0 and self.speed_data:
            ...
            if search_data and len(search_data) > 0:
                ...
                # 獲取對應的speeddiff值
                if closest_index < len(self.driver1_speeddiff):
                    driver1_speeddiff_at_position = self.driver1_speeddiff[closest_index]
                if closest_index < len(self.driver2_speeddiff):
                    driver2_speeddiff_at_position = self.driver2_speeddiff[closest_index]
            
            # 計算需要顯示的車手數量來調整標籤大小
            drivers_to_show = []
            
            # 只添加有效且不重複的車手資訊
            if driver1_speeddiff_at_position is not None and self.driver1_name:
                drivers_to_show.append((self.driver1_name, driver1_speeddiff_at_position, self.driver1_color))
            
            # 只有在非單車手模式且第二個車手數據不同時才添加第二個車手
            if (not getattr(self, 'is_single_driver', False) and 
                driver2_speeddiff_at_position is not None and 
        ...
        if search_data and driver1_data:
            ...
            if closest_idx is not None and closest_idx < len(driver1_data):
                ...
                if value1 is not None:
                    # ✅ 僅顯示數值，不顯示車手名稱
                    painter.drawText(label_x + 5, text_y, f"{tr('speed_diff_label', '速度差')}: {value1:.1f} {data_label}")
                else:
                    painter.drawText(label_x + 5, text_y, f"{tr('speed_diff_label', '速度差')}: N/A")
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self.middle_dragging and not self.last_drag_pos.isNull():
            ...
            if chart_rect.width() > 0 and chart_rect.height() > 0:
                ...
                speed_move = -dx * speed_range / chart_rect.width()
                
                # Y軸移動（speeddiff）
                speeddiff_range = (self.view_max_speeddiff or self.max_speeddiff) - (self.view_min_speeddiff or self.min_speeddiff)
                speeddiff_move = dy * speeddiff_range / chart_rect.height()  # Y軸是倒置的
                
                # 更新視圖範圍
                if self.view_min_speed is None:
                    self.view_min_speed = self.min_speed
                    self.view_max_speed = self.max_speed
                if self.view_min_speeddiff is None:
                    self.view_min_speeddiff = self.min_speeddiff
                    self.view_max_speeddiff = self.max_speeddiff
                
                self.view_min_speed += speed_move
                self.view_max_speed += speed_move
                self.view_min_speeddiff += speeddiff_move
                self.view_max_speeddiff += speeddiff_move
        ...
        if chart_rect.contains(event.pos()):
            ...
            if speed_range > 0:
                ...
                if linkage_manager and self._is_linkage_fully_enabled():
                    linkage_manager.send_x_linkage(speed_value, relative_y, self)
    ...
    def wheelEvent(self, event: QWheelEvent):
        ...
        if chart_rect.contains(event.pos()):
            ...
            speeddiff_range = self.view_max_speeddiff - self.view_min_speeddiff
            
            mouse_speed = self.view_min_speed + mouse_rel_x * speed_range
            mouse_speeddiff = self.view_min_speeddiff + mouse_rel_y * speeddiff_range
            
            # 計算新的範圍
            new_speed_range = speed_range / zoom_factor
            new_speeddiff_range = speeddiff_range / zoom_factor
            
            # 更新視圖範圍，保持滑鼠位置不變
            self.view_min_speed = max(self.min_speed, 
                                       mouse_speed - new_speed_range * mouse_rel_x)
            self.view_max_speed = min(self.max_speed, 
                                       mouse_speed + new_speed_range * (1 - mouse_rel_x))
            
            self.view_min_speeddiff = max(self.min_speeddiff, 
                                  mouse_speeddiff - new_speeddiff_range * mouse_rel_y)
            self.view_max_speeddiff = min(self.max_speeddiff, 
                                  mouse_speeddiff + new_speeddiff_range * (1 - mouse_rel_y))
...
class SpeeddiffAnalysisChartWidget(QWidget, LapAnalysisLinkageMixin, LapAnalysisLinkageDrawingMixin):
    """speeddiff分析圖表組件主容器"""
    ...
    def __init__(self, parent=None):
        ...
        # 🔴 移除容器類的重複註冊（內部的 speeddiffChartWidget 將在 _create_chart_area() 中註冊）
        # 避免雙重註冊導致的記憶體洩漏問題
        # if linkage_manager:
        #     linkage_manager.register_module(self, "speeddiff_analysis")
    ...
    def _create_chart_area(self) -> QWidget:
        ...
        # 創建圖表組件
        self.chart_widget = speeddiffChartWidget()
        layout.addWidget(self.chart_widget)
        
        # 確保內部圖表組件也註冊到連動管理器
        if linkage_manager:
            linkage_manager.register_module(self.chart_widget, "speeddiff_analysis_chart")
    ...
    def _prepare_chart_data(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """準備圖表數據"""
        try:
            if 'speeddiff_telemetry' in data:
                # 直接speeddiff數據
                return self._parse_speeddiff_telemetry(data['speeddiff_telemetry'])
            
            elif 'speed_data' in data:
                # 從速度數據模擬speeddiff數據
                return self._simulate_speeddiff_from_speed(data['speed_data'])
    ...
    def _parse_speeddiff_telemetry(self, speeddiff_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 解析車手1數據
        if 'driver1_speeddiff_data' in speeddiff_data:
            for point in speeddiff_data['driver1_speeddiff_data']:
                speed.append(point.get('speed', 0))
                driver1_speeddiff.append(point.get('speeddiff', 0))
        
        # 解析車手2數據
        if 'driver2_speeddiff_data' in speeddiff_data:
            for point in speeddiff_data['driver2_speeddiff_data']:
                driver2_speeddiff.append(point.get('speeddiff', 0))
        
        return {
            'speed': speed,
            'driver1_speeddiff': driver1_speeddiff,
            'driver2_speeddiff': driver2_speeddiff,
            'driver1_name': speeddiff_data.get('driver1_name', 'Driver 1'),
            'driver2_name': speeddiff_data.get('driver2_name', 'Driver 2'),
            'sectors': speeddiff_data.get('sectors', []),
            'engine_info': speeddiff_data.get('engine_info', {}),
            'track_info': speeddiff_data.get('track_info', {})
        }
    ...
    def _update_statistics_table(self, statistics: Dict, driver1_name: str, driver2_name: str):
        ...
        try:
            ...
            rows = [
                ("最大速度差 (km/h)", 
                 f"{driver1_stats.get('max_speeddiff', 0):.1f}",
                 f"{driver2_stats.get('max_speeddiff', 0):.1f}",
                 f"{comparison.get('max_speeddiff_diff', 0):.1f}"),
                ("平均速度差 (km/h)",
                 f"{driver1_stats.get('avg_speeddiff', 0):.1f}",
                 f"{driver2_stats.get('avg_speeddiff', 0):.1f}",
                 f"{comparison.get('avg_speeddiff_diff', 0):.1f}"),
                ("最小速度差 (km/h)",
                 f"{driver1_stats.get('min_speeddiff', 0):.1f}",
                 f"{driver2_stats.get('min_speeddiff', 0):.1f}",
    ...
    def reload_data(self):
        ...
        if self.current_data:
            self.update_speeddiff_data(self.current_data)
...
if __name__ == "__main__":
    ...
    # 測試speeddiff圖表組件
    widget = speeddiffChartWidget()
    widget.setWindowTitle("🔄 speeddiff分析圖表測試")
