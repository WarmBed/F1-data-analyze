...
class speeddiffDataManager(QObject):
    ...
    def load_speeddiff_data(self, year: str, race: str, session: str, 
        ...
        """載入speeddiff對比數據"""
        try:
            print(f"[speeddiff_MDI_DATA] ========== 載入speeddiff數據 ==========")
            print(f"[speeddiff_MDI_DATA] 參數: {year} {race} {session}")
            print(f"[speeddiff_MDI_DATA] 車手: {driver1} vs {driver2}, 圈數: {lap1} vs {lap2}")
            
            if self._is_loading:
                print(f"[speeddiff_MDI_DATA] ⚠️ 數據載入中，忽略新請求")
            ...
            if is_fastest or lap1 == "fastest" or lap2 == "fastest":
                print(f"🔄 [speeddiff_MDI_DATA] 檢測到最速圈選項，檢查遙測分析數據...")
                self._check_and_load_telemetry_if_needed()
                
                # 解析最速圈參數為實際圈數
                lap1, lap2 = self._resolve_lap_numbers(lap1, lap2, driver1, driver2, is_fastest)
                print(f"🔢 [speeddiff_MDI_DATA] 最速圈解析完成: {driver1}=第{lap1}圈, {driver2}=第{lap2}圈")
            
            print(f"[speeddiff_MDI_DATA] 🔗 創建 speeddiffAnalysisDataLoader...")
            
            # 使用現有的speeddiff分析數據載入器
            from .speeddiff_analysis_data_loader import SpeeddiffAnalysisDataLoader
            
            print(f"[speeddiff_MDI_DATA] 🚀 調用 load_speeddiff_data...")
            ...
            if hasattr(self, 'speeddiff_chart_widget') and self.speeddiff_chart_widget:
                ...
                print(f"[speeddiff_MDI] ✅ 已將loader設置給chart widget")
            
            if success:
                print(f"[speeddiff_MDI_DATA] ✅ speeddiff數據載入請求提交成功")
                self.loading_progress.emit(50)
                return True
            else:
                print(f"[speeddiff_MDI_DATA] ❌ speeddiff數據載入請求失敗")
                self._is_loading = False
                self.error_occurred.emit("speeddiff數據載入請求失敗")
                return False
                
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI_DATA] 載入speeddiff數據時發生錯誤: {e}")
            self._is_loading = False
    ...
    def _on_data_loaded(self, data):
        """數據載入完成回調"""
        try:
            print(f"[speeddiff_MDI_DATA] ✅ speeddiff數據載入完成")
            self._is_loading = False
            self.loading_progress.emit(100)
            self.status_changed.emit("speeddiff數據載入完成")
            self.data_loaded.emit(data)
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI_DATA] 處理載入完成回調時發生錯誤: {e}")
            self._on_load_error(f"數據處理失敗: {str(e)}")
    
    def _on_load_error(self, error_msg):
        """數據載入錯誤回調"""
        print(f"[speeddiff_MDI_DATA] ❌ speeddiff數據載入錯誤: {error_msg}")
        self._is_loading = False
        self.loading_progress.emit(0)
    ...
    def _check_and_load_telemetry_if_needed(self):
        """檢查並載入遙測分析數據（最速圈用）"""
        try:
            print(f"[speeddiff_MDI_DATA] 🔍 檢查遙測分析數據可用性...")
                ...
                if os.path.exists(directory):
                    for pattern in telemetry_patterns:
                        ...
                        if os.path.exists(file_path):
                            ...
                            print(f"📁 [speeddiff_MDI_DATA] 找到現有遙測檔案: {telemetry_file}")
                            return True
            
            # 如果沒有找到，透過 API 觸發 Function 13 生成
            print(f"[speeddiff_MDI_DATA] 📡 未找到遙測數據，透過 API 觸發生成...")
            return self._generate_telemetry_via_api()
            
        except Exception as e:
            print(f"❌ [speeddiff_MDI_DATA] 檢查遙測數據時發生錯誤: {e}")
            return False
    ...
    def _generate_telemetry_via_api(self) -> bool:
        ...
        try:
            ...
            if success:
                print("[speeddiff_MDI_DATA] ✅ 遙測分析已透過 API 生成")
                return True

            print(f"[speeddiff_MDI_DATA] ❌ 遙測分析 API 生成失敗: {message}")
            return False

        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI_DATA] _generate_telemetry_via_api 失敗: {e}")
            return False
    
    def _get_fastest_lap_number(self, driver: str) -> int:
        """從遙測分析數據獲取指定車手的最速圈數"""
        try:
            print(f"🔍 [speeddiff_MDI] 開始搜尋 {driver} 的最速圈數據...")
            ...
            for directory in search_dirs:
                if os.path.exists(directory):
                    for pattern in telemetry_patterns:
                        ...
                        if os.path.exists(file_path):
                            telemetry_file = file_path
                            print(f"📁 [speeddiff_MDI] 找到遙測檔案: {telemetry_file}")
                            break
                    if telemetry_file:
                        break
            
            if not telemetry_file:
                print(f"❌ [speeddiff_MDI] 找不到遙測分析檔案，使用預設圈數 1")
                return 1
            ...
            print(f"📊 [speeddiff_MDI] 遙測檔案讀取成功，開始解析最速圈數據...")
            ...
            if 'data' in telemetry_data and 'all_drivers_telemetry' in telemetry_data['data']:
                ...
                if driver_data and 'fastest_lap' in driver_data:
                    ...
                    if fastest_lap_num:
                        print(f"✅ [speeddiff_MDI] 從格式1找到 {driver} 最速圈: 第{fastest_lap_num}圈")
            ...
            if 'data' in telemetry_data and 'fastest_laps' in telemetry_data['data']:
                for fastest_data in telemetry_data['data']['fastest_laps']:
                    if fastest_data.get('driver') == driver:
                        ...
                        if fastest_lap_num:
                            print(f"✅ [speeddiff_MDI] 從格式2找到 {driver} 最速圈: 第{fastest_lap_num}圈")
                            return int(fastest_lap_num)
            ...
            if 'data' in telemetry_data:
                ...
                if driver_data and 'fastest_lap_number' in driver_data:
                    ...
                    print(f"✅ [speeddiff_MDI] 從格式3找到 {driver} 最速圈: 第{fastest_lap_num}圈")
                    return int(fastest_lap_num)
            
            print(f"⚠️ [speeddiff_MDI] 無法找到 {driver} 的最速圈數據，使用預設圈數 1")
            return 1
            
        except Exception as e:
            print(f"❌ [speeddiff_MDI] 解析最速圈數據時發生錯誤: {e}")
            return 1
    ...
    def _resolve_lap_numbers(self, lap1, lap2, driver1, driver2, is_fastest):
        ...
        try:
            ...
            if lap1 == "fastest" or is_fastest:
                print(f"🔄 [speeddiff_MDI] 解析 {driver1} 的最速圈...")
                resolved_lap1 = self._get_fastest_lap_number(driver1)
                
            # 處理lap2
            if lap2 == "fastest" or is_fastest:
                print(f"🔄 [speeddiff_MDI] 解析 {driver2} 的最速圈...")
                resolved_lap2 = self._get_fastest_lap_number(driver2)
            
            print(f"📊 [speeddiff_MDI] 圈數解析結果: {driver1}=第{resolved_lap1}圈, {driver2}=第{resolved_lap2}圈")
...
class SpeeddiffAnalysisModule(IAnalysisModule):
    ...
    def initialize_module(self, parent_widget=None, **kwargs) -> bool:
        """初始化模組 - 實現抽象方法"""
        try:
            print(f"[speeddiff_MDI] 初始化speeddiff分析模組")
            ...
            try:
                ...
                print(f"[speeddiff_MDI] ✅ 已註冊到分析模組管理器: {module_id}")
                
            except ImportError as e:
                print(f"[WARNING] [speeddiff_MDI] 無法導入分析模組管理器: {e}")
                self._analysis_manager = None
                self._module_id = None
            except Exception as e:
                print(f"[ERROR] [speeddiff_MDI] 註冊到分析模組管理器失敗: {e}")
                self._analysis_manager = None
                self._module_id = None
            ...
            try:
                ...
                if linkage_manager and self.speeddiff_chart_widget:
                    ...
                    print(f"[speeddiff_MDI] ✅ 已註冊到連動管理器")
                else:
                    print(f"[WARNING] [speeddiff_MDI] 連動管理器不可用或圖表組件未創建")
            except ImportError as e:
                print(f"[WARNING] [speeddiff_MDI] 無法導入連動管理器: {e}")
            except Exception as e:
                print(f"[ERROR] [speeddiff_MDI] 註冊到連動管理器失敗: {e}")
            
            self._initialized = True
            print(f"[OK] [speeddiff_MDI] speeddiff分析模組初始化完成")
            return True
            
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] 模組初始化失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def get_window_title(self, year: str = None, race: str = None, session: str = None, 
        ...
        print(f"[speeddiff_TITLE_DEBUG] 🏷️ 生成視窗標題: '{title}'")
        return title
    
    def update_window_title(self) -> None:
        """更新視窗標題"""
        try:
            print(f"[speeddiff_TITLE_DEBUG] 🔄 開始更新視窗標題...")
            print(f"[speeddiff_TITLE_DEBUG] 📋 當前狀態檢查:")
            
            # 檢查 parent_window 屬性（MDI 子視窗引用）
            parent = getattr(self, 'parent_window', None)
            print(f"[speeddiff_TITLE_DEBUG]   - parent_window 存在: {parent is not None}")
            
            if parent and hasattr(parent, 'setWindowTitle'):
                old_title = parent.windowTitle()
                print(f"[speeddiff_TITLE_DEBUG]   - 舊標題: '{old_title}'")
                
                new_title = self.get_window_title(self.current_year, self.current_race, self.current_session)
                print(f"[speeddiff_TITLE_DEBUG]   - 新標題: '{new_title}'")
                
                if old_title != new_title:
                    print(f"[speeddiff_TITLE_DEBUG] 🔄 標題需要更新，執行更新...")
                    ...
                    print(f"[speeddiff_TITLE_DEBUG] ✅ 標題更新完成: '{updated_title}'")
                    
                    # 如果直接更新失敗，使用延遲更新
                    if updated_title != new_title:
                        print(f"[speeddiff_TITLE_DEBUG] ⚠️ 直接更新失敗，嘗試延遲更新...")
                        self._delayed_title_update(new_title)
                else:
                    print(f"[speeddiff_TITLE_DEBUG] ✅ 標題無需更新")
            else:
                print(f"[speeddiff_TITLE_DEBUG] ⚠️ 無法更新標題:")
                print(f"[speeddiff_TITLE_DEBUG]   - parent_window: {parent}")
                print(f"[speeddiff_TITLE_DEBUG]   - 有setWindowTitle方法: {hasattr(parent, 'setWindowTitle') if parent else False}")
        
        except Exception as e:
            print(f"[ERROR] [speeddiff_TITLE_DEBUG] 更新視窗標題失敗: {e}")
            import traceback
            traceback.print_exc()
    
    def _delayed_title_update(self, title: str) -> None:
        """延遲標題更新 - 採用進站分析模式"""
        print(f"[speeddiff_TITLE_DEBUG] ⏰ 啟動延遲標題更新: '{title}'")
        ...
        def update_title():
            try:
                if self.parent_window and hasattr(self.parent_window, 'setWindowTitle'):
                    ...
                    print(f"[speeddiff_TITLE_DEBUG] ✅ 延遲更新完成: '{final_title}'")
                else:
                    print(f"[speeddiff_TITLE_DEBUG] ❌ 延遲更新失敗: parent_window 不可用")
            except Exception as e:
                print(f"[ERROR] [speeddiff_TITLE_DEBUG] 延遲更新異常: {e}")
    ...
    def update_lap_parameters(self, year: str, race: str, session: str,
        ...
        try:
            ...
            if self.speeddiff_chart_widget:
                ...
                print(f"[speeddiff_MDI] ✅ 已更新圖表組件的圈數顯示")
            
            if params_changed:
                print(f"[speeddiff_MDI] 🔄 參數已變化，開始重載數據...")
                ...
                if self.data_manager:
                    ...
                    if success:
                        ...
                        print(f"[speeddiff_MDI] ❌ 圈速參數更新後數據重載失敗")
                        return False
                else:
                    print(f"[speeddiff_MDI] ❌ 數據管理器未初始化")
                    return False
            else:
                print(f"[speeddiff_MDI] ℹ️ 圈速參數未變化，保持現有數據")
    ...
    def _update_chart(self, data: dict):
        """更新圖表"""
        try:
            print(f"[speeddiff_MDI] 更新speeddiff圖表")
            if self.speeddiff_chart_widget:
            ...
            print(f"[ERROR] [speeddiff_MDI] 圖表更新失敗: {e}")
            self.module_error.emit(f"圖表更新失敗: {str(e)}")
    
    def _handle_error(self, error_message: str):
        """處理錯誤"""
        print(f"[ERROR] [speeddiff_MDI] {error_message}")
    ...
    def _update_toolbar_status(self, data: dict):
        ...
        try:
            ...
            print(f"[speeddiff_MDI] 已更新工具欄狀態: {module_name}")
            
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] 更新工具欄狀態失敗: {e}")
    ...
    def _get_main_window(self):
        ...
        try:
            ...
            print(f"[ERROR] [speeddiff_MDI] 獲取主視窗失敗: {e}")
            return None

    def _on_lap_numbers_changed(self, lap1: int, lap2: int):
        """處理圈數變更"""
        try:
            print(f"[speeddiff_MDI] ========== 圈數變更處理 ==========")
            print(f"[speeddiff_MDI] 新圈數: 第{lap1}圈 vs 第{lap2}圈")
            ...
            print(f"[speeddiff_MDI] 圈數變更: 第{old_lap1}圈 vs 第{old_lap2}圈 → 第{lap1}圈 vs 第{lap2}圈")
            
            # 重新載入數據
            if self.data_manager:
                print(f"[speeddiff_MDI] 🔄 因圈數變更重新載入數據...")
                success = self.data_manager.load_speeddiff_data(
                    year=self.current_year,
                    race=self.current_race,
                    session=self.current_session,
                ...
                if success:
                    print(f"[speeddiff_MDI] ✅ 圈數變更後數據重載成功")
                else:
                    print(f"[speeddiff_MDI] ❌ 圈數變更後數據重載失敗")
            else:
                print(f"[speeddiff_MDI] ❌ 數據管理器未初始化，無法重載數據")
                
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] 處理圈數變更失敗: {e}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            if self.data_manager:
                ...
                try:
                    ...
                    print(f"[WARNING] [speeddiff_MDI] 斷開數據管理器信號時發生警告: {e}")
            ...
            if self.speeddiff_chart_widget and hasattr(self.speeddiff_chart_widget, 'lap_numbers_changed'):
                try:
                    ...
                    print(f"[WARNING] [speeddiff_MDI] 斷開圖表組件信號時發生警告: {e}")
            
            print(f"[speeddiff_MDI] ✅ 模組清理完成")
                
        except Exception as e:
            print(f"[WARNING] [speeddiff_MDI] 清理模組時發生警告: {e}")
    
    def reset_chart_view(self):
        """重置圖表視圖 - 與 Show All Data 按鈕整合"""
        print(f"[SPEEDDIFF_MDI] 🔄 reset_chart_view() 被調用")
        if hasattr(self, 'speeddiff_chart_widget') and self.speeddiff_chart_widget:
            print(f"[SPEEDDIFF_MDI] ✅ 找到 speeddiff_chart_widget，調用 reset_chart_view()")
            self.speeddiff_chart_widget.reset_chart_view()
        else:
            print(f"[SPEEDDIFF_MDI] ❌ 未找到 speeddiff_chart_widget 屬性")
    ...
    def _check_and_load_telemetry_if_needed(self, year: Optional[str] = None,
        ...
        try:
            ...
            print(f"[speeddiff_MDI] 🔍 [API-ONLY] 檢查遙測分析本地緩存: {{target_year}} {{target_race}} {{target_session}}")
            ...
            print("💡 [speeddiff_MDI] [API-ONLY] 或者手動執行 CLI: python f1_analysis_modular_main.py -f 8")
            return False

        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] _check_and_load_telemetry_if_needed 失敗: {{e}}")
            return False

    def _ensure_telemetry_data_for_fastest_laps(self) -> Optional[Dict[str, int]]:
        """確保最速圈數據的遙測分析可用 - 與速度分析相同功能"""
        try:
            print(f"[speeddiff_MDI] 🔍 檢查最速圈遙測數據可用性...")
            
            # 首先檢查是否已有遙測分析檔案
            telemetry_file = self._find_telemetry_analysis_file()
            
            if not telemetry_file:
                print(f"[speeddiff_MDI] 📡 遙測分析數據不存在，開始自動載入...")
                success = False
                if getattr(self, 'data_manager', None) and hasattr(self.data_manager, '_check_and_load_telemetry_if_needed'):
                    ...
                    print("[speeddiff_MDI] ⚠️ 未初始化資料管理器，無法觸發遙測載入")
                if success:
                    # 重新檢查檔案
                    telemetry_file = self._find_telemetry_analysis_file()
                else:
                    print(f"[speeddiff_MDI] ❌ 遙測分析載入失敗")
                    return None
            
            if telemetry_file:
                print(f"[speeddiff_MDI] 📂 找到遙測分析檔案: {telemetry_file}")
                return self._extract_fastest_laps_from_telemetry(telemetry_file)
            else:
                print(f"[speeddiff_MDI] ⚠️ 無法獲取遙測分析數據")
                return None
                
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] _ensure_telemetry_data_for_fastest_laps 失敗: {e}")
            return None
            ...
            if os.path.exists(json_dir):
                for filename in os.listdir(json_dir):
                    if (filename.startswith(f"telemetry_analysis_{year}_{race}_{session}") and 
                        ...
                        print(f"[speeddiff_MDI] 📂 找到遙測分析檔案: {full_path}")
                        return full_path
            
            print(f"[speeddiff_MDI] 📂 未找到遙測分析檔案")
            return None
            
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] _find_telemetry_analysis_file 失敗: {e}")
            return None
    
    def _trigger_telemetry_analysis(self) -> bool:
        """觸發遙測分析載入/生成 - 與速度分析相同功能"""
        try:
            print(f"[speeddiff_MDI] 🚀 觸發遙測分析載入: {self.current_year} {self.current_race} {self.current_session}")
                ...
                if hasattr(main_window, 'mdi_area'):
                    ...
                    for sub_window in main_window.mdi_area.subWindowList():
                        ...
                        if "遙測分析" in window_title:
                            print(f"[speeddiff_MDI] 🎯 找到現有遙測分析視窗: {window_title}")
                            # 激活並刷新遙測分析視窗
                            main_window.mdi_area.setActiveSubWindow(sub_window)
                            return True
                    
                    # API-ONLY 模式：不自動創建視窗
                    print(f"[speeddiff_MDI] � [API-ONLY] 未找到現有遙測分析視窗")
                    print(f"[speeddiff_MDI] 💡 提示：請手動開啟遙測分析模組或通過 API 獲取數據")
                    return False
            ...
            if data_manager and hasattr(data_manager, '_check_and_load_telemetry_if_needed'):
                print(f"[speeddiff_MDI] � 透過資料管理器觸發遙測 API 生成...")
                return data_manager._check_and_load_telemetry_if_needed()

            print("[speeddiff_MDI] ❌ 找不到資料管理器，無法觸發遙測分析")
            return False
            
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] _trigger_telemetry_analysis 失敗: {e}")
            return False
    
    def _extract_fastest_laps_from_telemetry(self, telemetry_file: str) -> Optional[Dict[str, int]]:
        """從遙測分析JSON檔案中提取最速圈數據 - 與速度分析相同功能"""
        try:
            print(f"[speeddiff_MDI] 📊 從遙測分析中提取最速圈數據: {telemetry_file}")
            ...
            print(f"[speeddiff_MDI] ✅ 最速圈數據提取完成: {fastest_laps}")
            return fastest_laps if fastest_laps else None
            
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] _extract_fastest_laps_from_telemetry 失敗: {e}")
            return None
    
    def receive_main_window_update_notification(self, param_type, value):
        """接收主視窗參數更新通知 - 與速度分析相同功能"""
        try:
            print(f"[speeddiff_NOTIFICATION_DEBUG] ========== 收到主視窗更新通知 ==========")
            print(f"[speeddiff_NOTIFICATION_DEBUG] 📡 原始參數:")
            print(f"[speeddiff_NOTIFICATION_DEBUG]   - param_type: {param_type}")
            print(f"[speeddiff_NOTIFICATION_DEBUG]   - value: {value}")
            ...
            print(f"[speeddiff_NOTIFICATION_DEBUG] 📊 當前模組狀態:")
            print(f"[speeddiff_NOTIFICATION_DEBUG]   - 當前年份: {self.current_year}")
            print(f"[speeddiff_NOTIFICATION_DEBUG]   - 當前賽事: {self.current_race}")
            print(f"[speeddiff_NOTIFICATION_DEBUG]   - 當前賽段: {self.current_session}")
            print(f"[speeddiff_NOTIFICATION_DEBUG]   - 當前車手: {getattr(self, 'driver1', 'VER')} vs {getattr(self, 'driver2', 'VER')}")
            print(f"[speeddiff_NOTIFICATION_DEBUG]   - 當前圈數: 第{getattr(self, 'lap1', 1)}圈 vs 第{getattr(self, 'lap2', 1)}圈")
            ...
            # 重新載入數據 - 與速度分析模組保持一致
            if hasattr(self, 'data_manager') and self.data_manager:
                print(f"[REFRESH] 重新載入speeddiff數據...")
            ...
            print(f"[OK] [NOTIFICATION] ⚡ speeddiff分析模組內容更新成功")
            
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] receive_main_window_update_notification 失敗: {e}")
            import traceback
            traceback.print_exc()
    ...
    def export_data(self, export_path: str, export_format: str = "json") -> bool:
        """匯出數據 - 實現抽象方法"""
        try:
            print(f"[speeddiff_MDI] 匯出數據功能尚未實現 (路徑: {export_path}, 格式: {export_format})")
            return False
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] export_data 失敗: {e}")
            return False
    ...
    def load_data(self, **kwargs) -> bool:
        ...
        try:
            ...
            print(f"[ERROR] [speeddiff_MDI] load_data 失敗: {e}")
            return False
    ...
    def get_current_data(self) -> dict:
        ...
        try:
            ...
            print(f"[ERROR] [speeddiff_MDI] get_current_data 失敗: {e}")
            return {}

    def clear_data(self) -> None:
        """清除數據 - 實現抽象方法"""
        try:
            print(f"[speeddiff_MDI] 清除數據...")
            ...
            print(f"[ERROR] [speeddiff_MDI] clear_data 失敗: {e}")

    def update_parameters(self, year: int, race: str, session: str) -> bool:
        """更新分析參數 - 實現抽象方法"""
        try:
            print(f"[speeddiff_PARAMS_DEBUG] ========== speeddiff參數更新開始 ==========")
            print(f"[speeddiff_PARAMS_DEBUG] 收到參數: year={year}, race={race}, session={session}")
            print(f"[speeddiff_PARAMS_DEBUG] 當前參數: year={self.current_year}, race={self.current_race}, session={self.current_session}")
            ...
            print(f"[speeddiff_PARAMS_DEBUG] 參數變化檢查: {params_changed}")
            print(f"[speeddiff_PARAMS_DEBUG] 舊參數: {old_year} {old_race} {old_session}")
            print(f"[speeddiff_PARAMS_DEBUG] 新參數: {new_year} {new_race} {new_session}")
            ...
            # 檢查是否需要載入數據
            print(f"[speeddiff_PARAMS_DEBUG] 檢查數據載入需求...")
            if params_changed or not hasattr(self, '_data_loaded'):
                print(f"[speeddiff_PARAMS_DEBUG] 需要載入數據：參數變化={params_changed}, 未載入過={not hasattr(self, '_data_loaded')}")
                ...
                if hasattr(self, 'data_manager') and self.data_manager:
                    ...
                    if success:
                        ...
                        print(f"[speeddiff_PARAMS_DEBUG] ✅ speeddiff 數據重載成功")
                        return True
                    else:
                        print(f"[speeddiff_PARAMS_DEBUG] ❌ speeddiff 數據重載失敗")
                        return False
                else:
                    # 檢查並創建數據管理器
                    print(f"[speeddiff_PARAMS_DEBUG] 數據管理器不存在，嘗試創建...")
                    try:
                        self.data_manager = speeddiffDataManager()
                        self.data_manager.data_loaded.connect(self._update_chart)
                        self.data_manager.error_occurred.connect(self._handle_error)
                        print(f"[speeddiff_PARAMS_DEBUG] ✅ 數據管理器創建成功，開始載入數據...")
                        ...
                        if success:
                            self._data_loaded = True
                            print(f"[speeddiff_PARAMS_DEBUG] ✅ speeddiff 數據載入成功")
                            return True
                        else:
                            print(f"[speeddiff_PARAMS_DEBUG] ❌ speeddiff 數據載入失敗")
                            return False
                            
                    except Exception as e:
                        print(f"[speeddiff_PARAMS_DEBUG] ❌ 數據管理器創建失敗: {e}")
                        print(f"[speeddiff_PARAMS_DEBUG] ⚠️ 參數更新完成（無數據載入）: {self.current_year} {self.current_race} {self.current_session}")
                        return False
            else:
                print(f"[speeddiff_PARAMS_DEBUG] 跳過數據載入：參數無變化且已載入過")
                return True
            
        except Exception as e:
            print(f"[ERROR] [speeddiff_PARAMS_DEBUG] update_parameters 失敗: {e}")
            print(f"[ERROR] [speeddiff_PARAMS_DEBUG] update_parameters 失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def refresh_analysis(self) -> None:
        """重新分析 - 實現抽象方法"""
        try:
            print(f"[speeddiff_MDI] 重新分析...")
            self._refresh_data()
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI] refresh_analysis 失敗: {e}")
    ...
    def _on_cross_event_data_loaded(self, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[ERROR] [SPEEDDIFF-CROSS-EVENT] 數據處理失敗: {e}")
            import traceback
            traceback.print_exc()
    
    def _on_cross_event_load_error(self, error_msg: str) -> None:
        """處理跨賽事比較數據載入錯誤"""
        print(f"[SPEEDDIFF-CROSS-EVENT] ❌ 數據載入失敗: {error_msg}")
    ...
    def update_from_shared_params(self, params: dict):
        ...
        try:
            print(f"[SPEEDDIFF_MDI] [SHARED_PARAMS] 🔄 從全域共享池更新參數")
            print(f"[SPEEDDIFF_MDI] [SHARED_PARAMS] 收到參數: {params}")
            ...
            print(f"[ERROR] [SPEEDDIFF_MDI] [SHARED_PARAMS] 更新失敗: {e}")
            import traceback
            traceback.print_exc()
        finally:
    ...
    def _update_info_label(self):
        ...
        try:
            ...
            if sync_enabled:
                ...
                print(f"[SPEEDDIFF_MDI] 同步模式：隱藏資訊標籤")
                return
            ...
            print(f"[SPEEDDIFF_MDI] 取消同步模式：顯示資訊標籤")
            
        except Exception as e:
            print(f"[ERROR] [SPEEDDIFF_MDI] 更新資訊標籤失敗: {e}")
...
try:
    ...
    print(f"[OK] [MODULE_FACTORY] speeddiff分析模組已註冊")
except ImportError as e:
    print(f"[WARNING] [MODULE_FACTORY] speeddiff分析模組註冊失敗: {e}")
