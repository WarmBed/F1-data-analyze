...
class speeddiffDataManager(QObject):
    ...
    def _check_and_load_telemetry_if_needed(self):
        ...
        try:
            ...
            print(f"[speeddiff_MDI_DATA] � 未找到遙測數據，通過CLI生成...")
    ...
    def _generate_telemetry_via_cli(self) -> bool:
        ...
        try:
            ...
            print(f"[speeddiff_MDI_DATA] 🔧 執行CLI命令: {' '.join(command)}")
            
            # 同步執行CLI命令（因為speeddiff分析需要立即使用結果）
            ...
            if process.returncode == 0:
                print(f"[speeddiff_MDI_DATA] ✅ 遙測分析CLI執行成功")
                time.sleep(2)  # 等待檔案寫入完成
                return True
            else:
                print(f"[speeddiff_MDI_DATA] ❌ 遙測分析CLI執行失敗: {stderr}")
                return False
                
        except subprocess.TimeoutExpired:
            print(f"[speeddiff_MDI_DATA] ⏰ 遙測分析CLI執行超時")
            return False
        except Exception as e:
            print(f"[ERROR] [speeddiff_MDI_DATA] _generate_telemetry_via_cli 失敗: {e}")
            return False
