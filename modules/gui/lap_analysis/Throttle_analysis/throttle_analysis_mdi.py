        ...
        try:
            ...
            if hasattr(self, 'parent_window') and self.parent_window:
                if hasattr(self.parent_window, 'open_telemetry_analysis'):
                    ...
                    print(f"❌ [THROTTLE_MDI] 主視窗沒有遙測分析方法")
                    return False
            else:
                print(f"❌ [THROTTLE_MDI] 找不到主視窗引用")
                return False
                
        except Exception as e:
            print(f"❌ [THROTTLE_MDI] 觸發遙測分析時發生錯誤: {e}")
            return False
