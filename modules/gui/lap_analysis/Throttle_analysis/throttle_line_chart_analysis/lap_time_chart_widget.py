# -*- coding: utf-8 -*-
"""圈速折線圖元件，搭配油門分析顯示。"""

from __future__ import annotations

from typing import Dict, Iterable, List, Optional, Sequence

from PyQt5.QtCore import QRect
from PyQt5.QtGui import QBrush, QColor, QFont, QFontMetrics, QPen

from core.gui_i18n import tr
from modules.gui.universal_chart_widget import ChartDataSeries

from .linked_chart_widget import LinkedUniversalChartWidget


class LapTimeChartWidget(LinkedUniversalChartWidget):
    """呈現單車手圈速及最快圈差異。"""

    def __init__(self, parent=None):
        super().__init__(title=tr("throttle_line_chart.lap_time_chart_title", "Lap Times"), parent=parent)
        
        # ⚙️ Throttle Line Chart 專屬設定：禁用舊功能，使用新的通用互動系統
        self.show_value_tooltips = False  # 禁用垂直虛線旁的數值提示
        self.mouse_x = -1  # 禁用垂直虛線追蹤（設為 -1 使其不顯示）
        
        self.set_axis_labels(
            tr("throttle_line_chart.axis_lap", "Lap"),
            tr("throttle_line_chart.axis_lap_time", "Lap Time"),
            tr("throttle_line_chart.axis_lap_delta", "Δ vs Best"),
            x_unit="",
            left_y_unit="s",
            right_y_unit="s",
        )
        
        # 🆕 雙車手模式：分別儲存兩位車手的 tooltip 數據
        self._tooltip_map_driver1: Dict[int, Dict[str, object]] = {}
        self._tooltip_map_driver2: Dict[int, Dict[str, object]] = {}
        self._tooltip_map: Dict[int, Dict[str, object]] = {}  # 保留向下相容性（指向 driver1）
        
        self._settings = {
            "show_delta": True,
            "rolling_average": False,
            "rolling_window": 3,
        }

    # ------------------------------------------------------------------
    def update_series(
        self,
        lap_records: Sequence[Dict[str, object]],
        tooltip_map: Dict[int, Dict[str, object]],
        *,
        show_delta: bool,
        rolling_average: bool,
        rolling_window: int,
        highlight_invalid: Optional[Iterable[int]] = None,
        highlight_caution: Optional[Iterable[int]] = None,
        flag_markers: Optional[Dict[int, str]] = None,
        lap_records_driver2: Optional[Sequence[Dict[str, object]]] = None,  # 新增：第二位車手資料
        tooltip_map_driver2: Optional[Dict[int, Dict[str, object]]] = None,  # 新增：第二位車手提示
    ) -> None:
        self._settings.update(
            {
                "show_delta": show_delta,
                "rolling_average": rolling_average,
                "rolling_window": max(1, int(rolling_window)),
            }
        )
        
        # 🆕 雙車手模式：分別儲存兩位車手的 tooltip 數據
        self._tooltip_map_driver1 = dict(tooltip_map or {})
        self._tooltip_map_driver2 = dict(tooltip_map_driver2 or {})
        self._tooltip_map = self._tooltip_map_driver1  # 向下相容性

        self.clear_data()
        self.set_lap_records(lap_records)

        lap_numbers: List[int] = []
        lap_times: List[float] = []
        lap_deltas: List[float] = []

        for record in lap_records:
            lap_no = self._safe_int(record.get("lap_number"))
            lap_time = self._safe_float(record.get("lap_time_seconds"))
            delta = self._safe_float(record.get("lap_time_delta"))

            if lap_no is None or lap_time is None:
                continue

            lap_numbers.append(lap_no)
            lap_times.append(lap_time)
            lap_deltas.append(delta if delta is not None else float("nan"))

        if lap_numbers:
            series_lap = ChartDataSeries(
                name=tr("throttle_line_chart.series_lap_time", "Lap Time (s)"),
                x_data=lap_numbers,
                y_data=lap_times,
                color="#0064C8",  # 柔和藍色 - 車手1 (參考速度模組)
                line_width=2,
                y_axis="left",
            )
            self.add_data_series(series_lap)

            if rolling_average and len(lap_times) >= self._settings["rolling_window"]:
                smooth = self._rolling_average(lap_times, self._settings["rolling_window"])
                self.add_data_series(
                    ChartDataSeries(
                        name=tr("throttle_line_chart.series_lap_time_avg", "Rolling Avg"),
                        x_data=lap_numbers,
                        y_data=smooth,
                        color="#4D94D9",  # 淺藍色 - 車手1次要線條
                        line_width=1,
                        y_axis="left",
                    )
                )

        if show_delta and lap_numbers:
            cleaned_delta = self._replace_nan_with_previous(lap_deltas)
            self.add_data_series(
                ChartDataSeries(
                    name=tr("throttle_line_chart.series_lap_delta", "Δ vs Best (s)"),
                    x_data=lap_numbers,
                    y_data=cleaned_delta,
                    color="#F06292",
                    line_width=1,
                    y_axis="right",
                )
            )

        # 🆕 處理第二位車手圈速資料（使用紅色）
        if lap_records_driver2:
            lap_numbers_d2: List[int] = []
            lap_times_d2: List[float] = []

            for record in lap_records_driver2:
                lap_no = self._safe_int(record.get("lap_number"))
                lap_time = self._safe_float(record.get("lap_time_seconds"))

                if lap_no is None or lap_time is None:
                    continue

                lap_numbers_d2.append(lap_no)
                lap_times_d2.append(lap_time)

            if lap_numbers_d2:
                # Driver2 圈速（紅色實線）
                series_lap_d2 = ChartDataSeries(
                    name=tr("throttle_line_chart.series_lap_time_driver2", "Lap Time (D2)"),
                    x_data=lap_numbers_d2,
                    y_data=lap_times_d2,
                    color="#C83232",  # 柔和紅色 - 車手2 (參考速度模組)
                    line_width=2,
                    y_axis="left",
                )
                self.add_data_series(series_lap_d2)

                # Driver2 移動平均（淺紅色）
                if rolling_average and len(lap_times_d2) >= self._settings["rolling_window"]:
                    smooth_d2 = self._rolling_average(lap_times_d2, self._settings["rolling_window"])
                    self.add_data_series(
                        ChartDataSeries(
                            name=tr("throttle_line_chart.series_lap_time_avg_driver2", "Rolling Avg (D2)"),
                            x_data=lap_numbers_d2,
                            y_data=smooth_d2,
                            color="#E57373",  # 淺紅色 - 車手2次要線條
                            line_width=1,
                            y_axis="left",
                        )
                    )

        self.set_highlight_laps([])
        self.set_flag_markers(flag_markers)
        self.set_pinned_marker(None, None)

        self.recalculate_data_ranges()
        self.update()

    def get_tooltip_payload(self, lap_number: int, series_name: str = "") -> Dict[str, object]:
        """
        獲取指定圈數的 tooltip 數據
        
        Args:
            lap_number: 圈數
            series_name: 系列名稱（用於判斷是 Driver 1 還是 Driver 2）
        
        Returns:
            Tooltip 數據字典
        """
        # 🆕 雙車手模式：根據系列名稱判斷使用哪個 tooltip map
        # 如果系列名稱包含 "(D2)" 或 "(Driver 2)"，使用 Driver 2 的數據
        if series_name and ("(D2)" in series_name or "(Driver 2)" in series_name or "Driver 2" in series_name):
            return dict(self._tooltip_map_driver2.get(int(lap_number), {}))
        else:
            # 默認使用 Driver 1 的數據
            return dict(self._tooltip_map_driver1.get(int(lap_number), {}))
    
    def format_tooltip_for_data_point(self, lap_number: int, series_name: str = "") -> List[str]:
        """
        為數據點互動系統格式化 tooltip 文字
        只顯示：Lap, Ave Throttle %, Full throttle %, Lap time, DRS %, Tyre
        
        Args:
            lap_number: 圈數
            series_name: 系列名稱（用於雙車手模式判斷）
        """
        payload = self.get_tooltip_payload(lap_number, series_name)
        if not payload:
            return []
        
        lines = []
        
        # Lap
        lines.append(f"Lap {lap_number}")
        
        # Full Throttle %
        ratio = payload.get("full_throttle_ratio_percent")
        if ratio is not None:
            try:
                lines.append(f"Full Throttle %: {float(ratio):.1f}%")
            except (TypeError, ValueError):
                pass
        
        # Average Throttle %
        avg = payload.get("average_throttle_percent")
        if avg is not None:
            try:
                lines.append(f"Ave Throttle %: {float(avg):.1f}%")
            except (TypeError, ValueError):
                pass
        
        # Lap Time
        lap_time_fmt = payload.get("lap_time_formatted") or payload.get("lap_time")
        if lap_time_fmt:
            lines.append(f"Lap Time: {lap_time_fmt}")
        elif payload.get("lap_time_seconds") is not None:
            try:
                lines.append(f"Lap Time: {float(payload['lap_time_seconds']):.3f}s")
            except (TypeError, ValueError):
                pass
        
        # DRS %
        drs = payload.get("drs_percent")
        if drs is not None:
            try:
                lines.append(f"DRS %: {float(drs):.1f}%")
            except (TypeError, ValueError):
                pass
        
        # Tyre
        compound = payload.get("compound", "N/A")
        lines.append(f"Tyre: {compound}")
        
        return lines

    # ------------------------------------------------------------------
    @staticmethod
    def _rolling_average(values: Sequence[float], window: int) -> List[float]:
        if window <= 1 or len(values) < window:
            return list(values)

        averaged: List[float] = []
        cumulative = 0.0
        for idx, value in enumerate(values):
            cumulative += value
            if idx >= window:
                cumulative -= values[idx - window]
            divisor = min(idx + 1, window)
            averaged.append(round(cumulative / divisor, 4))
        return averaged

    @staticmethod
    def _replace_nan_with_previous(series: Sequence[Optional[float]]) -> List[float]:
        cleaned: List[float] = []
        last_value: Optional[float] = 0.0
        for value in series:
            if value is None or (isinstance(value, float) and value != value):
                cleaned.append(last_value if last_value is not None else 0.0)
            else:
                numeric = float(value)
                cleaned.append(numeric)
                last_value = numeric
        return cleaned

    @staticmethod
    def _safe_int(value) -> Optional[int]:
        try:
            return None if value is None else int(value)
        except (TypeError, ValueError):
            return None

    @staticmethod
    def _safe_float(value) -> Optional[float]:
        try:
            return None if value is None else float(value)
        except (TypeError, ValueError):
            return None

    # ------------------------------------------------------------------
    def _draw_pinned_annotation(self, painter, chart_area) -> None:
        """❌ 已禁用舊的 pinned_marker 系統，使用通用的 pinned_data_points 替代"""
        # 不再繪製淺藍色的 "Lap XX lap time" 提示
        # 新系統使用 universal_chart_widget.py 中的 draw_hover_and_pinned_data_points()
        pass

    def _build_pinned_lines(self, lap: int, payload: Dict[str, object]) -> List[str]:
        """❌ 已禁用 - 舊系統不再使用"""
        return []


__all__ = ["LapTimeChartWidget"]
