#!/usr/bin/env python3
"""
F1T timediff分析 MDI 模組
基於速度分析模組的成功架構設計
支援雙車手timediff對比的 GUI 模組，使用新版模組更新機制
"""
...
class timediffDataManager(QObject):
    """timediff數據管理器 - 負責JSON緩存和CLI備援"""
    ...
    def load_timediff_data(self, year: str, race: str, session: str, 
        ...
        try:
            ...
            self.status_changed.emit("開始載入timediff數據...")
            ...
            self.timediff_loader.data_loaded.connect(self._on_data_loaded)
            self.timediff_loader.load_error.connect(self._on_load_error)
            self.timediff_loader.status_changed.connect(self.status_changed.emit)
            self.timediff_loader.load_progress.connect(self.loading_progress.emit)
            
            # 開始載入數據
            success = self.timediff_loader.load_timediff_data(
                year=int(year),
                race=race,
                session=session,
                driver1=driver1,
    ...
    def _check_and_load_telemetry_if_needed(self):
        """檢查並載入遙測分析數據（最速圈用）"""
        try:
            print(f"[timediff_MDI_DATA] 🔍 檢查遙測分析數據可用性...")
                ...
                if os.path.exists(directory):
                    for pattern in telemetry_patterns:
                        ...
                        if os.path.exists(file_path):
                            ...
                            print(f"📁 [timediff_MDI_DATA] 找到現有遙測檔案: {telemetry_file}")
                            return True
            
            # 如果沒有找到，通過CLI生成Function 12數據
            print(f"[timediff_MDI_DATA] � 未找到遙測數據，通過CLI生成...")
            return self._generate_telemetry_via_cli()
            
        except Exception as e:
            print(f"❌ [timediff_MDI_DATA] 檢查遙測數據時發生錯誤: {e}")
            return False
    ...
    def _generate_telemetry_via_cli(self) -> bool:
        ...
        try:
            ...
            print(f"[timediff_MDI_DATA] 🔧 執行CLI命令: {' '.join(command)}")
            
            # 同步執行CLI命令（因為timediff分析需要立即使用結果）
            ...
            if process.returncode == 0:
                print(f"[timediff_MDI_DATA] ✅ 遙測分析CLI執行成功")
                time.sleep(2)  # 等待檔案寫入完成
                return True
            else:
                print(f"[timediff_MDI_DATA] ❌ 遙測分析CLI執行失敗: {stderr}")
                return False
                
        except subprocess.TimeoutExpired:
            print(f"[timediff_MDI_DATA] ⏰ 遙測分析CLI執行超時")
            return False
        except Exception as e:
            print(f"[ERROR] [timediff_MDI_DATA] _generate_telemetry_via_cli 失敗: {e}")
            return False
...
class timediffAnalysisModule(IAnalysisModule):
    """timediff分析主模組"""
    ...
    def __init__(self, parent=None):
        ...
        self.timediff_chart_widget = None
    ...
    def initialize_module(self, parent_widget=None, **kwargs) -> bool:
        ...
        try:
            ...
            self.timediff_chart_widget = timediffAnalysisChartWidget()
            
            # 連接圈數變更信號
            self.timediff_chart_widget.lap_numbers_changed.connect(self._on_lap_numbers_changed)
            
            # 設置初始圈數
            self.timediff_chart_widget.set_lap_numbers(self.lap1, self.lap2)
            ...
            try:
                ...
                # 註冊模組
                module_id = f"timediff_analysis_{id(self)}"
                manager.register_module(module_id, self, "timediff_analysis")
                
                # 註冊圖表組件
                if self.timediff_chart_widget:
                    manager.register_chart_widget(self.timediff_chart_widget)
    ...
    def _create_placeholder_widget(self):
        """創建佔位組件（當timediff圖表組件不可用時）"""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        
        label = QLabel(tr('timediff_chart_title', 'Time Diff Analysis Chart'))
        label.setAlignment(Qt.AlignCenter)
        label.setStyleSheet("font-size: 16pt; padding: 20px;")
        layout.addWidget(label)
        
        info_label = QLabel(tr('timediff_chart_loading', 'Time diff chart component is loading...'))
    ...
    def _setup_ui(self):
        ...
        # 添加timediff圖表
        if self.timediff_chart_widget:
            layout.addWidget(self.timediff_chart_widget)
    ...
    def get_window_title(self, year: str = None, race: str = None, session: str = None, 
        ...
        title = f"{tr('timediff_window_title', 'Time Diff Analysis')}_{use_year}_{use_race}_{use_session}"
        
        print(f"[timediff_TITLE_DEBUG] 🏷️ 生成視窗標題: '{title}'")
        print(f"[timediff_TITLE_DEBUG]   📊 參數詳情:")
        print(f"[timediff_TITLE_DEBUG]     - 年份: {use_year}")
        print(f"[timediff_TITLE_DEBUG]     - 賽事: {use_race}")
        print(f"[timediff_TITLE_DEBUG]     - 賽段: {use_session}")
        return title
    ...
    def update_lap_parameters(self, year: str, race: str, session: str,
        ...
        try:
            ...
            if is_fastest and hasattr(self, '_ensure_telemetry_data_for_fastest_laps'):
                ...
                if fastest_laps:
                    if driver1 in fastest_laps:
                        ...
                        print(f"[timediff_MDI] 🏁 {driver1} 最速圈: 第{lap1}圈")
                    if driver2 and driver2 in fastest_laps:
                        lap2 = fastest_laps[driver2]
                        print(f"[timediff_MDI] 🏁 {driver2} 最速圈: 第{lap2}圈")
                else:
                    print("[timediff_MDI] ⚠️ 無法取得最速圈資訊，沿用既有圈數")
            ...
            if hasattr(self.timediff_chart_widget, 'set_lap_numbers'):
                self.timediff_chart_widget.set_lap_numbers(self.lap1, self.lap2)
            ...
            if not params_changed:
                print("[timediff_MDI] ℹ️ 參數無變化，保持當前資料")
                return True

            if not self.data_manager:
                print("[timediff_MDI] ❌ 數據管理器未初始化，無法更新")
                return False
            ...
            print("[timediff_MDI] 🚀 重新載入timediff數據...")
            ...
            if success:
                ...
                print("[timediff_MDI] ✅ 圈速參數更新完成")
                return True

            print("[timediff_MDI] ❌ 數據載入失敗")
            return False

        except Exception as e:
            print(f"[ERROR] [timediff_MDI] update_lap_parameters 失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def get_default_size(self) -> tuple:
        """獲取預設視窗大小"""
        return (1000, 700)  # timediff分析需要較大的視窗來顯示詳細圖表
    ...
    def update_lap_parameters(self, year: str, race: str, session: str, 
        ...
        try:
            ...
            if params_changed:
                ...
                print(f"[timediff_MDI] ℹ️ 參數未變更，跳過數據重載")
                return True
        ...
        try:
            ...
            if hasattr(self, '_analysis_manager') and self._analysis_manager and hasattr(self, '_module_id'):
                try:
                    # 解除註冊圖表組件
                    if hasattr(self, 'timediff_chart_widget') and self.timediff_chart_widget:
                        self._analysis_manager.unregister_chart_widget(self.timediff_chart_widget)
            ...
            try:
                from ..linkage import linkage_manager
                if linkage_manager and hasattr(self, 'timediff_chart_widget') and self.timediff_chart_widget:
                    linkage_manager.unregister_module(self.timediff_chart_widget)
                    print(f"[timediff_MDI] ✅ 已從連動管理器取消註冊")
            except ImportError as e:
                print(f"[WARNING] [timediff_MDI] 無法導入連動管理器: {e}")
            except Exception as e:
            ...
            if hasattr(self, 'timediff_chart_widget') and self.timediff_chart_widget:
                # 清理圖表組件
                if hasattr(self.timediff_chart_widget, 'cleanup'):
                    self.timediff_chart_widget.cleanup()
                self.timediff_chart_widget.deleteLater()
    ...
    def module_name(self) -> str:
        """模組名稱"""
        return "timediff_analysis"

    @property
    def display_name(self) -> str:
        """顯示名稱"""
        return tr("timediff_analysis", "Time Diff Analysis")

    @property
    def description(self) -> str:
        """模組描述"""
        return tr("timediff_analysis_description", "F1 Time Difference Comparison Analysis Tool")
    ...
    def load_data(self, **kwargs) -> bool:
        ...
        try:
            ...
            if self.data_manager:
                return self.data_manager.load_timediff_data(
