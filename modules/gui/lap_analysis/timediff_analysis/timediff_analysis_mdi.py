        ...
        try:
            ...
            if hasattr(self, 'timediff_loader') and self.timediff_loader:
                try:
                    ...
                    # 斷開信號連接
                    try:
                        self.timediff_loader.data_loaded.disconnect()
                    except Exception:
                        pass
                    try:
                        self.timediff_loader.load_error.disconnect()
                    except Exception:
                        pass
                    try:
                        self.timediff_loader.status_changed.disconnect()
                    except Exception:
                        pass
                    try:
                        self.timediff_loader.load_progress.disconnect()
                    except Exception:
                        pass
...
class timediffAnalysisModule(IAnalysisModule):
    ...
    def __init__(self, parent=None):
        ...
        # ✅ 設置分析類型（用於批次更新識別）
        self.analysis_type = 'Timediff'  # 注意：使用大寫T以匹配規範
    ...
    def _update_chart(self, data: dict):
        ...
        try:
            ...
            if self.timediff_chart_widget:
                ...
                if use_time_axis and hasattr(self.timediff_chart_widget, 'set_time_axis_mode'):
                    logger.debug(f"[timediff_MDI] ⏱️ 數據載入完成，應用時間軸模式: {use_time_axis}")
                    self.timediff_chart_widget.set_time_axis_mode(use_time_axis)
    ...
    def load_data(self, **kwargs) -> bool:
        ...
        try:
            ...
            logger.debug(f"[timediff_MDI] load_data 參數: use_time_axis={use_time_axis}")
            ...
            if self.data_manager:
                ...
                # 數據載入成功後設置時間軸模式
                if success and use_time_axis and self.timediff_chart_widget:
                    if hasattr(self.timediff_chart_widget, 'set_time_axis_mode'):
                        logger.debug(f"[timediff_MDI] 設置圖表時間軸模式: {use_time_axis}")
                        self.timediff_chart_widget.set_time_axis_mode(use_time_axis)
        ...
        try:
            ...
            if "telemetry_comparison" in data:
                ...
                if timediff_key:
                    ...
                    # ✅ 構建符合 chart_widget 期望的數據格式
                    # 參考 Speed Diff 的成功模式
                    chart_data = {
                        "timediff_data": {
                            "time": timediff_telemetry.get("time", []),  # X軸：時間
                            "cumulative_time_difference": timediff_telemetry.get("time_difference", []),  # Y軸：時間差
                            "reference": "Time Difference",  # 參考標籤
                            "driver1_name": "Time Difference",  # 車手名稱
                        },
                        "metadata": {
                            "drivers": [
                                {
                                    "code": self.driver1 or "Driver1",
                                    "lap_number": self.lap1 or 1,
                                },
                                {
                                    "code": self.driver2 or "Driver2",
                                    "lap_number": self.lap2 or 1,
                                }
                            ],
                            "sectors": [],  # 賽道區段（跨賽事比較可能沒有）
                            "reference_info": f"{self.driver1} vs {self.driver2}",
                        },
                        "statistics": {},  # 統計數據（如需要可從 API 獲取）
