...
class ThrottleDataManager(QObject):
    ...
    def load_throttle_data(self, year: str, race: str, session: str, 
        ...
        try:
            ...
            print(f"[SPEED_MDI_DATA] 🔗 創建 ThrottleAnalysisDataLoader...")
            
            # 使用現有的油門分析數據載入器
            from .throttle_analysis_data_loader import ThrottleAnalysisDataLoader
            
            print(f"[SPEED_MDI_DATA] 🚀 調用 load_throttle_data...")
            ...
            print(f"[ERROR] [SPEED_MDI] 油門數據載入失敗: {e}")
...
class ThrottleAnalysisModule(IAnalysisModule):
    ...
    def initialize_module(self, parent_widget=None, **kwargs) -> bool:
        ...
        try:
            ...
            print(f"[OK] [SPEED_MDI] 油門分析模組初始化完成")
            return True
        ...
        try:
            ...
            if params_changed:
                ...
                if self.data_manager:
                    ...
                    if success:
                        print(f"[SPEED_MDI] ✅ 圈速油門參數更新後數據重載成功")
                        # 發送參數更新信號
                        self.parameters_updated.emit({
                            'year': self.current_year,
                            'race': self.current_race,
                        ...
                        print(f"[SPEED_MDI] ❌ 圈速油門參數更新後數據重載失敗")
                        return False
                else:
                    print(f"[SPEED_MDI] ❌ 數據管理器未初始化")
                    return False
            else:
                print(f"[SPEED_MDI] ℹ️ 圈速油門參數未變化，保持現有數據")
            ...
            print(f"[SPEED_MDI] ❌ 圈速油門參數更新失敗: {e}")
            import traceback
            traceback.print_exc()
            self.module_error.emit(f"圈速油門參數更新失敗: {str(e)}")
            return False
    ...
    def update_lap_parameters(self, year: str, race: str, session: str,
                            ...
                            is_fastest: bool = False) -> bool:
        """更新圈速油門參數並重新載入數據 - 供統一更新介面使用"""
        try:
            print(f"[SPEED_MDI] 🔄 更新圈速油門參數...")
