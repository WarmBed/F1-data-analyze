...
class RPMAnalysisDataLoader(TelemetryDataLoader):
    ...
    def get_rpm_statistics(self) -> Optional[Dict[str, Any]]:
        """
        獲取RPM統計數據
        
        Returns:
            Dict: RPM統計數據，如果沒有數據則返回 None
        """
    ...
    def get_max_rpm(self, driver: str = 'driver1') -> Optional[float]:
        """
        獲取指定車手的最大RPM
        ...
        Returns:
            float: 最大RPM值，如果沒有數據則返回 None
        """
        stats = self.get_rpm_statistics()
        if stats and driver in stats:
            return stats[driver].get('max', None)
        return None
    
    def get_avg_rpm(self, driver: str = 'driver1') -> Optional[float]:
        """
        獲取指定車手的平均RPM
...
def create_rpm_loader(parent=None) -> RPMAnalysisDataLoader:
    """
    創建RPM數據載入器的工廠函數
    ...
    Returns:
        RPMAnalysisDataLoader: RPM數據載入器實例
    """
    return RPMAnalysisDataLoader(parent)
...
if __name__ == "__main__":
    ...
    rpm_loader.status_changed.connect(on_status_changed)
