    ...
    def __init__(self, driver1: str, year1: int, race1: str, session1: str, lap1: int,
        ...
        try:
            ...
            try:
                ...
                logger.info(f"[RPM-CROSS-EVENT-WORKER] ✅ API base URL: {self.base_url}")
            except Exception as e:
                # 如果解析失敗，使用硬編碼的公開 API
                from core.api_base_url import PUBLIC_API_BASE_URL
                self.base_url = PUBLIC_API_BASE_URL.rstrip('/')
                logger.warning(f"[RPM-CROSS-EVENT-WORKER] ⚠️ API URL 解析失敗，使用預設: {self.base_url}")
                logger.debug(f"[RPM-CROSS-EVENT-WORKER] 錯誤: {e}")
                
        except Exception as e:
            logger.error(f"[RPM-CROSS-EVENT-WORKER] Worker 初始化失敗: {e}")
            import traceback
            traceback.print_exc()
    ...
    def run(self):
        ...
        try:
            ...
            if self.isInterruptionRequested():
                logger.debug("[RPM-CROSS-EVENT-WORKER] 開始前已被中斷")
                return
            logger.debug(f"[RPM-CROSS-EVENT-WORKER] 開始執行 API 請求")
            ...
            logger.debug(f"[RPM-CROSS-EVENT-WORKER] 目標端點: {endpoint}")
            ...
            logger.debug(f"[RPM-CROSS-EVENT-WORKER] 請求參數: {query_params}")
            ...
            # ✅ 中斷檢查點 2: HTTP 請求前
            if self.isInterruptionRequested():
                logger.debug("[RPM-CROSS-EVENT-WORKER] HTTP 請求前被中斷")
                return
            ...
            logger.debug(f"[RPM-CROSS-EVENT-WORKER] 發送 POST 請求...")
            ...
            # ✅ 中斷檢查點 3: HTTP 請求後
            if self.isInterruptionRequested():
                logger.debug("[RPM-CROSS-EVENT-WORKER] HTTP 請求後被中斷")
                return
            
            logger.debug(f"[RPM-CROSS-EVENT-WORKER] 收到回應，狀態碼: {response.status_code}")
            response.raise_for_status()

            payload = response.json()
            logger.debug(f"[RPM-CROSS-EVENT-WORKER] JSON 解析成功")
            ...
            if self.isInterruptionRequested():
                logger.debug("[RPM-CROSS-EVENT-WORKER] success 信號前被中斷")
                return
            logger.info(f"[RPM-CROSS-EVENT-WORKER] ✅ 請求成功，發送 success 信號")
            ...
            logger.error(f"[RPM-CROSS-EVENT-WORKER] ❌ {error_msg}")
            ...
            try:
                ...
                logger.debug(f"[RPM-CROSS-EVENT-WORKER] Worker 執行完成")
            except:
...
class RPMDataManager(QObject):
    ...
    def load_rpm_data(self, year: str, race: str, session: str, 
        ...
        """載入RPM對比數據"""
        try:
            logger.debug(f"[RPM_MDI_DATA] ========== 載入RPM數據 ==========")
            logger.debug(f"[RPM_MDI_DATA] 參數: {year} {race} {session}")
            logger.debug(f"[RPM_MDI_DATA] 車手: {driver1} vs {driver2}, 圈數: {lap1} vs {lap2}")
            
            if self._is_loading:
                logger.warning(f"[RPM_MDI_DATA] ⚠️ 數據載入中，忽略新請求")
            ...
            if is_fastest or lap1 == "fastest" or lap2 == "fastest":
                logger.debug(f"[RPM_MDI_DATA] 檢測到最速圈選項，檢查遙測分析數據...")
                self._check_and_load_telemetry_if_needed()
                
                # 解析最速圈參數為實際圈數
                lap1, lap2 = self._resolve_lap_numbers(lap1, lap2, driver1, driver2, is_fastest)
                logger.debug(f"🔢 [RPM_MDI_DATA] 最速圈解析完成: {driver1}=第{lap1}圈, {driver2}=第{lap2}圈")
            
            logger.debug(f"[RPM_MDI_DATA] 🔗 創建 RPMAnalysisDataLoader...")
            
            # 使用現有的RPM分析數據載入器
            from .rpm_analysis_data_loader import RPMAnalysisDataLoader
            
            logger.debug(f"[RPM_MDI_DATA] 🚀 調用 load_rpm_data...")
            ...
            if hasattr(self, 'rpm_chart_widget') and self.rpm_chart_widget:
                ...
                logger.info(f"[RPM_MDI] ✅ 已將loader設置給chart widget")
            
            if success:
                logger.info(f"[RPM_MDI_DATA] ✅ RPM數據載入請求提交成功")
                self.loading_progress.emit(50)
                return True
            else:
                logger.error(f"[RPM_MDI_DATA] ❌ RPM數據載入請求失敗")
                self._is_loading = False
                self.error_occurred.emit("RPM數據載入請求失敗")
                return False
                
        except Exception as e:
            logger.error(f"[RPM_MDI_DATA] 載入RPM數據時發生錯誤: {e}")
            self._is_loading = False
    ...
    def _on_data_loaded(self, data):
        """數據載入完成回調"""
        try:
            logger.info(f"[RPM_MDI_DATA] ✅ RPM數據載入完成")
            self._is_loading = False
            self.loading_progress.emit(100)
            self.status_changed.emit("RPM數據載入完成")
            self.data_loaded.emit(data)
        except Exception as e:
            logger.error(f"[RPM_MDI_DATA] 處理載入完成回調時發生錯誤: {e}")
            self._on_load_error(f"數據處理失敗: {str(e)}")
    
    def _on_load_error(self, error_msg):
        """數據載入錯誤回調"""
        logger.error(f"[RPM_MDI_DATA] ❌ RPM數據載入錯誤: {error_msg}")
        self._is_loading = False
        self.loading_progress.emit(0)
    ...
    def _check_and_load_telemetry_if_needed(self):
        """檢查遙測分析數據（最速圈用）"""
        try:
            logger.debug(f"[RPM_MDI_DATA] 🔍 檢查遙測分析數據可用性...")
                ...
                if os.path.exists(directory):
                    for pattern in telemetry_patterns:
                        ...
                        if os.path.exists(file_path):
                            logger.debug(f"📁 [RPM_MDI_DATA] 找到現有遙測檔案: {file_path}")
                            return True

            logger.warning("[RPM_MDI_DATA] API-ONLY 模式下未找到遙測檔案，請透過主視窗遙測模組或 REST API 取得資料")
            return False

        except Exception as e:
            logger.error(f"[RPM_MDI_DATA] 檢查遙測數據時發生錯誤: {e}")
            return False
    
    def _get_fastest_lap_number(self, driver: str) -> int:
        """從遙測分析數據獲取指定車手的最速圈數"""
        try:
            logger.debug(f"[RPM_MDI] 開始搜尋 {driver} 的最速圈數據...")
                ...
                if os.path.exists(directory):
                    for pattern in telemetry_patterns:
                        ...
                        if os.path.exists(file_path):
                            ...
                            logger.debug(f"📁 [RPM_MDI] 找到遙測檔案: {telemetry_file}")
                            break
                    if telemetry_file:
                        break
            
            if not telemetry_file:
                logger.error(f"[RPM_MDI] 找不到遙測分析檔案，使用預設圈數 1")
                return 1
            ...
            logger.debug(f"[RPM_MDI] 遙測檔案讀取成功，開始解析最速圈數據...")
            ...
            if 'data' in telemetry_data and 'all_drivers_telemetry' in telemetry_data['data']:
                ...
                if driver_data and 'fastest_lap' in driver_data:
                    ...
                    if fastest_lap_num:
                        logger.info(f"[RPM_MDI] 從格式1找到 {driver} 最速圈: 第{fastest_lap_num}圈")
            ...
            if 'data' in telemetry_data and 'fastest_laps' in telemetry_data['data']:
                for fastest_data in telemetry_data['data']['fastest_laps']:
                    if fastest_data.get('driver') == driver:
                        ...
                        if fastest_lap_num:
                            logger.info(f"[RPM_MDI] 從格式2找到 {driver} 最速圈: 第{fastest_lap_num}圈")
                            return int(fastest_lap_num)
            ...
            if 'data' in telemetry_data:
                ...
                if driver_data and 'fastest_lap_number' in driver_data:
                    ...
                    logger.info(f"[RPM_MDI] 從格式3找到 {driver} 最速圈: 第{fastest_lap_num}圈")
                    return int(fastest_lap_num)
            
            logger.warning(f"[RPM_MDI] 無法找到 {driver} 的最速圈數據，使用預設圈數 1")
            return 1
            
        except Exception as e:
            logger.error(f"[RPM_MDI] 解析最速圈數據時發生錯誤: {e}")
            return 1
    ...
    def _resolve_lap_numbers(self, lap1, lap2, driver1, driver2, is_fastest):
        ...
        try:
            ...
            if lap1 == "fastest" or is_fastest:
                logger.debug(f"[RPM_MDI] 解析 {driver1} 的最速圈...")
                resolved_lap1 = self._get_fastest_lap_number(driver1)
                
            # 處理lap2
            if lap2 == "fastest" or is_fastest:
                logger.debug(f"[RPM_MDI] 解析 {driver2} 的最速圈...")
                resolved_lap2 = self._get_fastest_lap_number(driver2)
            
            logger.debug(f"[RPM_MDI] 圈數解析結果: {driver1}=第{resolved_lap1}圈, {driver2}=第{resolved_lap2}圈")
            
            return int(resolved_lap1), int(resolved_lap2)
            
        except Exception as e:
            logger.error(f"[RPM_MDI] 解析圈數時發生錯誤: {e}")
            return 1, 1
    ...
    def cleanup(self):
        ...
        """
        try:
            logger.debug(f"[RPMDATAMANAGER] 🧹 開始清理資源...")
            ...
            if hasattr(self, '_speed_loader') and self._speed_loader:
                try:
                    ...
                    if hasattr(self._speed_loader, 'cleanup'):
                        ...
                        logger.info(f"[RPMDATAMANAGER] ✅ 已清理 loader 執行緒")
                    ...
                    logger.error(f"[RPMDATAMANAGER] 清理 loader 失敗: {e}")
            
            # 🔴 關鍵修復：斷開循環引用（data_manager ← module_ref → module）
            if hasattr(self, 'module_ref') and self.module_ref:
                logger.debug(f"[RPMDATAMANAGER] 🔴 斷開循環引用：清理 data_manager.module_ref")
                self.module_ref = None
            ...
            logger.info(f"[RPMDATAMANAGER] ✅ 資源清理完成")
            
        except Exception as e:
            logger.error(f"[RPMDATAMANAGER] cleanup() 失敗: {e}")
            import traceback
            traceback.print_exc()
...
class RPMAnalysisModule(IAnalysisModule):
    ...
    def initialize_module(self, parent_widget=None, **kwargs) -> bool:
        """初始化模組 - 實現抽象方法"""
        try:
            logger.debug(f"[RPM_MDI] 初始化RPM分析模組")
            ...
            try:
                ...
                logger.info(f"[RPM_MDI] ✅ 已註冊到分析模組管理器: {module_id}")
                
            except ImportError as e:
                logger.warning(f"[RPM_MDI] 無法導入分析模組管理器: {e}")
                self._analysis_manager = None
                self._module_id = None
            except Exception as e:
                logger.error(f"[RPM_MDI] 註冊到分析模組管理器失敗: {e}")
                self._analysis_manager = None
                self._module_id = None
            
            self._initialized = True
            logger.info(f"[RPM_MDI] RPM分析模組初始化完成")
            return True
            
        except Exception as e:
            logger.error(f"[RPM_MDI] 模組初始化失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _update_info_label(self):
        ...
        try:
            ...
            if sync_enabled:
                ...
                logger.debug(f"[RPM_MDI] 同步模式：隱藏資訊標籤")
                return
            ...
            logger.debug(f"[RPM_MDI] 取消同步模式：顯示資訊標籤")
            
        except Exception as e:
            logger.error(f"[RPM_MDI] 更新資訊標籤失敗: {e}")
    
    def _update_chart(self, data: dict):
        """更新圖表"""
        try:
            logger.debug(f"[RPM_MDI] ========== 更新RPM圖表回調 ==========")
            logger.debug(f"[RPM_MDI] 📦 接收到數據類型: {type(data)}")
            logger.debug(f"[RPM_MDI] 📦 接收到數據鍵值: {list(data.keys()) if isinstance(data, dict) else 'Not a dict'}")
            if isinstance(data, dict) and 'rpm_data' in data:
                rpm_data = data['rpm_data']
                logger.debug(f"[RPM_MDI] 📊 rpm_data 鍵值: {list(rpm_data.keys())}")
                logger.debug(f"[RPM_MDI] 📊 distance 點數: {len(rpm_data.get('distance', []))}")
                logger.debug(f"[RPM_MDI] 📊 driver1_rpm 點數: {len(rpm_data.get('driver1_rpm', []))}")
                logger.debug(f"[RPM_MDI] 📊 driver2_rpm 點數: {len(rpm_data.get('driver2_rpm', []))}")
            
            if self.rpm_chart_widget:
                logger.debug(f"[RPM_MDI] 🎨 調用 rpm_chart_widget.update_rpm_data...")
                self.rpm_chart_widget.update_rpm_data(data)
                logger.info(f"[RPM_MDI] ✅ 圖表更新完成")
                
                # 數據載入完成後，應用時間軸設定
                use_time_axis = getattr(self, 'use_time_axis', False)
                if use_time_axis and hasattr(self.rpm_chart_widget, 'set_time_axis_mode'):
                    logger.debug(f"[RPM_MDI] ⏱️ 數據載入完成，應用時間軸模式: {use_time_axis}")
                    self.rpm_chart_widget.set_time_axis_mode(use_time_axis)
                
                # 更新工具欄狀態信息
                self._update_toolbar_status(data)
            else:
                logger.error(f"[RPM_MDI] ❌ rpm_chart_widget 未初始化")
                
        except Exception as e:
            # 🔴 簡化錯誤日誌避免 traceback 持有 frame（RPMAnalysisModule 實例）
            logger.error(f"[RPM_MDI] 圖表更新失敗: {e}")
            # 調試時可以取消註解：
            # import traceback
            # traceback.print_exc()
    ...
    def get_window_title(self, year: str = None, race: str = None, session: str = None) -> str:
        ...
        logger.debug(f"[RPM_TITLE_DEBUG] 🏷️ 生成視窗標題: '{title}'")
        return title
        return title
    ...
    def update_window_title(self) -> None:
        """更新視窗標題"""
        try:
            logger.debug(f"[RPM_TITLE_DEBUG] 🔄 開始更新視窗標題...")
            logger.debug(f"[RPM_TITLE_DEBUG] 📋 當前狀態檢查:")
            
            # 檢查 parent_window 屬性（MDI 子視窗引用）
            parent = getattr(self, 'parent_window', None)
            logger.debug(f"[RPM_TITLE_DEBUG] - parent_window 存在: {parent is not None}")
            
            if parent and hasattr(parent, 'setWindowTitle'):
                old_title = parent.windowTitle()
                logger.debug(f"[RPM_TITLE_DEBUG] - 舊標題: '{old_title}'")
                
                new_title = self.get_window_title(self.current_year, self.current_race, self.current_session)
                logger.debug(f"[RPM_TITLE_DEBUG] - 新標題: '{new_title}'")
                
                if old_title != new_title:
                    logger.debug(f"[RPM_TITLE_DEBUG] 🔄 標題需要更新，執行更新...")
                    ...
                    logger.info(f"[RPM_TITLE_DEBUG] ✅ 標題更新完成: '{updated_title}'")
                    
                    # 如果直接更新失敗，使用延遲更新
                    if updated_title != new_title:
                        logger.warning(f"[RPM_TITLE_DEBUG] ⚠️ 直接更新失敗，嘗試延遲更新...")
                        self._delayed_title_update(new_title)
                else:
                    logger.info(f"[RPM_TITLE_DEBUG] ✅ 標題無需更新")
            else:
                logger.warning(f"[RPM_TITLE_DEBUG] ⚠️ 無法更新標題:")
                logger.debug(f"[RPM_TITLE_DEBUG] - parent_window: {parent}")
                logger.debug(f"[RPM_TITLE_DEBUG] - 有setWindowTitle方法: {hasattr(parent, 'setWindowTitle') if parent else False}")
        
        except Exception as e:
            logger.error(f"[RPM_TITLE_DEBUG] 更新視窗標題失敗: {e}")
            import traceback
            traceback.print_exc()
    
    def _delayed_title_update(self, title: str) -> None:
        """延遲標題更新 - 採用進站分析模式"""
        logger.debug(f"[RPM_TITLE_DEBUG] ⏰ 啟動延遲標題更新: '{title}'")
        ...
        def update_title():
            try:
                if self.parent_window and hasattr(self.parent_window, 'setWindowTitle'):
                    ...
                    logger.info(f"[RPM_TITLE_DEBUG] ✅ 延遲更新完成: '{final_title}'")
                else:
                    logger.error(f"[RPM_TITLE_DEBUG] ❌ 延遲更新失敗: parent_window 不可用")
            except Exception as e:
                logger.error(f"[RPM_TITLE_DEBUG] 延遲更新異常: {e}")
    ...
    def update_lap_parameters(self, year: str, race: str, session: str, 
        ...
        try:
            logger.debug(f"[RPM_MDI] ========== 圈速參數更新 ==========")
            logger.debug(f"[RPM_MDI] 收到參數: {year} {race} {session}")
            logger.debug(f"[RPM_MDI] 車手: {driver1} vs {driver2}")
            logger.debug(f"[RPM_MDI] 圈數: 第{lap1}圈 vs 第{lap2}圈")
            logger.debug(f"[RPM_MDI] 最速圈: {is_fastest}")
            ...
            logger.debug(f"[RPM_MDI] ⏱️  使用時間軸: {use_time_axis}")
            ...
            # 檢查是否需要最速圈數據
            if is_fastest:
                logger.debug(f"[RPM_MDI] 🏁 用戶選擇了最速圈選項，檢查遙測分析數據...")
                fastest_laps = self._ensure_telemetry_data_for_fastest_laps()
                if fastest_laps:
                    # 使用最速圈數據更新圈數
                    if driver1 in fastest_laps:
                        lap1 = fastest_laps[driver1]
                        logger.debug(f"[RPM_MDI] 🏁 車手 {driver1} 最速圈: 第{lap1}圈")
                    if driver2 and driver2 in fastest_laps:
                        lap2 = fastest_laps[driver2]
                        logger.debug(f"[RPM_MDI] 🏁 車手 {driver2} 最速圈: 第{lap2}圈")
                else:
                    logger.warning(f"[RPM_MDI] ⚠️ 無法獲取最速圈數據，使用預設圈數")
            ...
            logger.debug(f"[RPM_MDI] 參數是否變化: {params_changed}")
            ...
            if self.rpm_chart_widget:
                ...
                logger.info(f"[RPM_MDI] ✅ 已更新圖表組件的圈數顯示")
            
            if params_changed:
                logger.debug(f"[RPM_MDI] 🔄 參數已變化，開始重載數據...")
                
                # 載入新數據
                if self.data_manager:
                    logger.debug(f"[RPM_MDI] 📡 調用數據管理器載入新數據...")
                    ...
                    if success:
                        logger.info(f"[RPM_MDI] ✅ 圈速參數更新後數據重載成功")
                        
                        # 應用時間軸設定到圖表
                        logger.debug(f"[TIME_AXIS_DEBUG] 步驟 5: 準備設置圖表時間軸模式")
                        logger.debug(f"[TIME_AXIS_DEBUG]   self.rpm_chart_widget 存在: {self.rpm_chart_widget is not None}")
                        if self.rpm_chart_widget:
                            logger.debug(f"[TIME_AXIS_DEBUG]   hasattr(rpm_chart_widget, 'set_time_axis_mode'): {hasattr(self.rpm_chart_widget, 'set_time_axis_mode')}")
                        
                        if self.rpm_chart_widget and hasattr(self.rpm_chart_widget, 'set_time_axis_mode'):
                            logger.debug(f"[TIME_AXIS_DEBUG]   調用 rpm_chart_widget.set_time_axis_mode({use_time_axis})")
                            self.rpm_chart_widget.set_time_axis_mode(use_time_axis)
                            logger.debug(f"[RPM_MDI] ⏱️  已設置圖表時間軸模式: {use_time_axis}")
                        ...
                        logger.debug(f"[RPM_MDI] 📋 已更新資訊標籤")
                        ...
                        if parent and hasattr(parent, 'setWindowTitle'):
                            new_title = self.get_window_title(self.current_year, self.current_race, self.current_session)
                            parent.setWindowTitle(new_title)
                            logger.debug(f"[RPM_MDI] 🏷️ 視窗標題已更新為: {new_title}")
                        else:
                            logger.warning(f"[RPM_MDI] ⚠️ 無法更新視窗標題 - 父視窗引用未設置")
                        
                        return True
                    else:
                        logger.error(f"[RPM_MDI] ❌ 圈速參數更新後數據重載失敗")
                        return False
                else:
                    logger.error(f"[RPM_MDI] ❌ 數據管理器未初始化")
                    return False
            else:
                logger.debug(f"[RPM_MDI] ℹ️ 參數未變更，跳過數據重載")
                return True
                
        except Exception as e:
            logger.error(f"[RPM_MDI] update_lap_parameters 失敗: {str(e)}")
            return False
    ...
    def update_cross_event_comparison(self, year1: str, race1: str, session1: str, driver1: str, lap1: int,
        ...
        try:
            logger.debug(f"[RPM-CROSS-EVENT] ========== 跨賽事比較更新 ==========")
            logger.debug(f"[RPM-CROSS-EVENT] 車手 1: {year1} {race1} {session1} {driver1} 第{lap1}圈")
            logger.debug(f"[RPM-CROSS-EVENT] 車手 2: {year2} {race2} {session2} {driver2} 第{lap2}圈")
            logger.debug(f"[RPM-CROSS-EVENT] 🕒 時間軸模式: {use_time_axis}")
            ...
            logger.warning(f"[RPM-CROSS-EVENT] ⚠️ 已停用同步模式 (sync_driver_lap_enabled = False)")
            
            # 保存時間軸設定
            self.use_time_axis = use_time_axis
            logger.debug(f"[RPM-CROSS-EVENT] 🕒 已保存時間軸設定: use_time_axis={use_time_axis}")
            
            # 更新資訊標籤（顯示跨賽事比較資訊）
            self._update_info_label()
            
            # 實作跨賽事比較邏輯：調用 API 端點
            logger.debug(f"[RPM-CROSS-EVENT] 開始調用 API 端點: /api/v2/analysis/cross-event-comparison")
            
            # 停止舊的 Worker（如果存在）
            if hasattr(self, '_cross_event_worker') and self._cross_event_worker:
                try:
                    if self._cross_event_worker.isRunning():
                        logger.debug(f"[RPM-CROSS-EVENT] 停止舊的 Worker...")
            ...
            # 創建 API Worker
            try:
                logger.debug(f"[RPM-CROSS-EVENT] 🚀 創建跨賽事比較 Worker...")
                ...
                logger.info(f"[RPM-CROSS-EVENT] ✅ Worker 創建成功")
            except Exception as e:
                error_msg = f"創建 API Worker 失敗: {e}"
                logger.error(f"[RPM-CROSS-EVENT] {error_msg}")
                import traceback
                traceback.print_exc()
                return False
            ...
            try:
                ...
                logger.info(f"[RPM-CROSS-EVENT] ✅ 信號連接成功")
            except Exception as e:
            ...
            # 啟動 Worker
            try:
                logger.debug(f"[RPM-CROSS-EVENT] 🔄 啟動 API 請求...")
                api_worker.start()
                logger.info(f"[RPM-CROSS-EVENT] ✅ API Worker 已啟動")
            except Exception as e:
            ...
            logger.error(f"[RPM-CROSS-EVENT] 跨賽事比較更新失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _on_cross_event_data_loaded(self, result: Dict[str, Any]) -> None:
        ...
        try:
            logger.info(f"[RPM-CROSS-EVENT] ✅ 數據載入成功")
            
            # 提取數據
            data = result.get("data", {})
            meta = result.get("meta", {})
            
            logger.debug(f"[RPM-CROSS-EVENT] 數據鍵值: {list(data.keys())}")
            logger.debug(f"[RPM-CROSS-EVENT] 元數據: {meta}")
            
            # 檢查是否有遙測比較數據
            if "telemetry_comparison" in data:
                telemetry_comp = data["telemetry_comparison"]
                logger.debug(f"[RPM-CROSS-EVENT] 遙測參數: {list(telemetry_comp.keys())}")
                ...
                if "RPM" in telemetry_comp:
                    ...
                    logger.debug(f"[RPM-CROSS-EVENT] 構建圖表數據:")
                    logger.debug(f"[RPM-CROSS-EVENT]   距離點數: {len(chart_data['rpm_data']['distance'])}")
                    logger.debug(f"[RPM-CROSS-EVENT]   車手1 RPM點數: {len(chart_data['rpm_data']['driver1_rpm'])}")
                    logger.debug(f"[RPM-CROSS-EVENT]   車手2 RPM點數: {len(chart_data['rpm_data']['driver2_rpm'])}")
                    logger.debug(f"[RPM-CROSS-EVENT]   車手1 時間點數: {len(chart_data['rpm_data']['driver1_time_seconds'])}")
                    logger.debug(f"[RPM-CROSS-EVENT]   車手2 時間點數: {len(chart_data['rpm_data']['driver2_time_seconds'])}")
                    logger.debug(f"[RPM-CROSS-EVENT]   🕒 時間軸模式: {chart_data['use_time_axis']}")
                    
                    # ⚠️ 關鍵：先設置時間軸模式，再更新圖表
                    use_time_axis = chart_data.get('use_time_axis', False)
                    if self.rpm_chart_widget and hasattr(self.rpm_chart_widget, 'set_time_axis_mode'):
                        logger.debug(f"[RPM-CROSS-EVENT] 🕒 設置圖表時間軸模式: {use_time_axis}")
                        self.rpm_chart_widget.set_time_axis_mode(use_time_axis)
                    
                    # 直接調用圖表更新方法
                    logger.debug(f"[RPM-CROSS-EVENT] 開始更新圖表...")
                    self._update_chart(chart_data)
                    logger.info(f"[RPM-CROSS-EVENT] ✅ 跨賽事比較完成")
                else:
                    logger.warning(f"[RPM-CROSS-EVENT] API 回應中沒有 RPM 遙測數據")
                    self.module_error.emit("跨賽事比較失敗: API 回應中沒有 RPM 數據")
            else:
                logger.warning(f"[RPM-CROSS-EVENT] API 回應中沒有 telemetry_comparison")
                self.module_error.emit("跨賽事比較失敗: API 回應格式錯誤")
                
        except Exception as e:
            logger.error(f"[RPM-CROSS-EVENT] 數據處理失敗: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_cross_event_load_error(self, error_msg: str) -> None:
        """處理跨賽事比較數據載入錯誤"""
        try:
            logger.error(f"[RPM-CROSS-EVENT] 數據載入失敗: {error_msg}")
            self.module_error.emit(f"跨賽事比較失敗: {error_msg}")
        except Exception as e:
            logger.error(f"[RPM-CROSS-EVENT] 錯誤處理失敗: {e}")
    ...
    def update_from_shared_params(self, params: dict):
        ...
        if self._updating_from_shared:
            logger.warning(f"[RPM_MDI] [SHARED_PARAMS] ⚠️  正在更新中，防止遞迴")
            return
        
        self._updating_from_shared = True
        try:
            logger.debug(f"[RPM_MDI] [SHARED_PARAMS] 🔄 從全域共享池更新參數")
            logger.debug(f"[RPM_MDI] [SHARED_PARAMS] 收到參數: {params}")
            ...
            if is_cross_event:
                logger.debug(f"[RPM_MDI] [SHARED_PARAMS] 🌍 檢測到跨賽事比較:")
                logger.debug(f"[RPM_MDI] [SHARED_PARAMS]   車手 1: {year1} {race1} {session1} {driver1} 第{lap1}圈")
                logger.debug(f"[RPM_MDI] [SHARED_PARAMS]   車手 2: {year2} {race2} {session2} {driver2} 第{lap2}圈")
                
                # ✅ RPM 模組已支援跨賽事比較
                logger.info(f"[RPM_MDI] [SHARED_PARAMS] ✅ RPM 模組支援跨賽事比較功能")
                logger.debug(f"[RPM_MDI] [SHARED_PARAMS] � 將通過全域參數池更新")
                ...
                # 標準模式（同一賽事比較）
                logger.info(f"[RPM_MDI] [SHARED_PARAMS] ✅ 標準比較模式:")
                logger.debug(f"[RPM_MDI] [SHARED_PARAMS]   賽事: {year1} {race1} {session1}")
                logger.debug(f"[RPM_MDI] [SHARED_PARAMS]   車手: {driver1} vs {driver2}")
                logger.debug(f"[RPM_MDI] [SHARED_PARAMS]   圈數: 第{lap1}圈 vs 第{lap2}圈")
            ...
            if self.rpm_chart_widget and hasattr(self.rpm_chart_widget, 'set_time_axis_mode'):
                ...
                logger.debug(f"[RPM_MDI] [SHARED_PARAMS] 🕒 已設置時間軸模式: {self.use_time_axis}")
            
            # ⚠️ [參數資訊標籤] 更新資訊標籤顯示
            self._update_info_label()
            logger.debug(f"[RPM_MDI] [SHARED_PARAMS] 📋 已更新資訊標籤")
            
            # 調用標準更新方法重新載入數據
            logger.debug(f"[RPM_MDI] [SHARED_PARAMS] 🔄 調用 update_lap_parameters 重新載入數據")
            ...
            if success:
                logger.info(f"[RPM_MDI] [SHARED_PARAMS] ✅ 全域參數同步完成")
            else:
                logger.error(f"[RPM_MDI] [SHARED_PARAMS] ❌ 數據重載失敗")
                
        except Exception as e:
            logger.error(f"[RPM_MDI] [SHARED_PARAMS] 更新失敗: {e}")
            import traceback
            traceback.print_exc()
        finally:
    ...
    def _update_chart(self, data: dict):
        """更新圖表"""
        try:
            logger.debug(f"[RPM_MDI] 更新RPM圖表")
            if self.rpm_chart_widget:
                self.rpm_chart_widget.update_rpm_data(data)
    ...
    def _handle_error(self, error_message: str):
        """處理錯誤"""
        logger.error(f"[RPM_MDI] {error_message}")
    ...
    def _update_toolbar_status(self, data: dict):
        ...
        try:
            ...
            logger.debug(f"[RPM_MDI] 已更新工具欄狀態: {module_name}")
            
        except Exception as e:
            logger.error(f"[RPM_MDI] 更新工具欄狀態失敗: {e}")
    ...
    def _get_main_window(self):
        ...
        try:
            ...
            logger.error(f"[RPM_MDI] 獲取主視窗失敗: {e}")
            return None

    def _on_lap_numbers_changed(self, lap1: int, lap2: int):
        """處理圈數變更"""
        try:
            logger.debug(f"[RPM_MDI] ========== 圈數變更處理 ==========")
            logger.debug(f"[RPM_MDI] 新圈數: 第{lap1}圈 vs 第{lap2}圈")
            ...
            logger.debug(f"[RPM_MDI] 圈數變更: 第{old_lap1}圈 vs 第{old_lap2}圈 → 第{lap1}圈 vs 第{lap2}圈")
            
            # 重新載入數據
            if self.data_manager:
                logger.debug(f"[RPM_MDI] 🔄 因圈數變更重新載入數據...")
                success = self.data_manager.load_rpm_data(
                    year=self.current_year,
                    race=self.current_race,
                    session=self.current_session,
                ...
                if success:
                    logger.info(f"[RPM_MDI] ✅ 圈數變更後數據重載成功")
                else:
                    logger.error(f"[RPM_MDI] ❌ 圈數變更後數據重載失敗")
            else:
                logger.error(f"[RPM_MDI] ❌ 數據管理器未初始化，無法重載數據")
                
        except Exception as e:
            logger.error(f"[RPM_MDI] 處理圈數變更失敗: {e}")
            import traceback
            traceback.print_exc()
    ...
    def cleanup_module(self):
        """清理模組資源和信號連接"""
        try:
            logger.debug(f"[RPM_MDI] 🧹 清理RPM分析模組...")
            ...
            if self.data_manager:
                ...
                try:
                    ...
                    logger.warning(f"[RPM_MDI] 斷開數據管理器信號時發生警告: {e}")
            ...
            if self.rpm_chart_widget and hasattr(self.rpm_chart_widget, 'lap_numbers_changed'):
                try:
                    ...
                    logger.warning(f"[RPM_MDI] 斷開圖表組件信號時發生警告: {e}")
            
            logger.info(f"[RPM_MDI] ✅ 模組清理完成")
                
        except Exception as e:
            logger.warning(f"[RPM_MDI] 清理模組時發生警告: {e}")
    
    def reset_chart_view(self):
        """重置圖表視圖 - 與 Show All Data 按鈕整合"""
        logger.debug(f"[RPM_MDI] 🔄 reset_chart_view() 被調用")
        if hasattr(self, 'rpm_chart_widget') and self.rpm_chart_widget:
            logger.info(f"[RPM_MDI] ✅ 找到 rpm_chart_widget，調用 reset_chart_view()")
            self.rpm_chart_widget.reset_chart_view()
        else:
            logger.error(f"[RPM_MDI] ❌ 未找到 rpm_chart_widget 屬性")
        ...
        try:
            ...
            if hasattr(self, '_analysis_manager') and self._analysis_manager and hasattr(self, '_module_id'):
                try:
                    ...
                    logger.info(f"[RPM_MDI] ✅ 已從分析模組管理器解除註冊: {self._module_id}")
                    
                except Exception as e:
                    logger.error(f"[RPM_MDI] 從分析模組管理器解除註冊失敗: {e}")
                ...
                try:
                    ...
                    if linkage_manager:
                        ...
                        if hasattr(self.rpm_chart_widget, 'chart_widget') and self.rpm_chart_widget.chart_widget:
                            ...
                            logger.info(f"[RPM_MDI] ✅ 已從連動管理器解除註冊內部圖表組件 (chart_widget)")
                except Exception as e:
                    logger.error(f"[RPM_MDI] 從連動管理器解除註冊失敗: {e}")
            ...
            logger.debug(f"[CLEANUP] RPM分析模組資源清理完成")
        except Exception as e:
            logger.error(f"RPM分析模組清理失敗: {e}")
    ...
    def _check_and_load_telemetry_if_needed(self, year: Optional[str] = None,
        ...
        try:
            ...
            logger.debug(f"[RPM_MDI] 🔍 [API-ONLY] 檢查遙測分析本地緩存: {target_year} {target_race} {target_session}")
            ...
            if telemetry_file:
                logger.debug(f"[RPM_MDI] 📂 [API-ONLY] 找到本地遙測分析緩存: {telemetry_file}")
                return True

            # ❌ 禁止：自動創建視窗或啟動 CLI
            # 改為僅提示用戶通過 API 或主視窗遙測模組獲取數據
            logger.warning("[RPM_MDI] [API-ONLY] 遙測分析數據不存在於本地緩存")
            logger.debug("💡 [RPM_MDI] [API-ONLY] 提示：請先透過主視窗遙測模組或 REST API 獲取遙測數據")
            logger.debug("💡 [RPM_MDI] [API-ONLY] 或者手動執行 CLI: python f1_analysis_modular_main.py -f 8")
            return False

        except Exception as e:
            logger.error(f"[RPM_MDI] _check_and_load_telemetry_if_needed 失敗: {e}")
            return False

    def _ensure_telemetry_data_for_fastest_laps(self) -> Optional[Dict[str, int]]:
        """確保最速圈數據的遙測分析可用 - 與速度分析相同功能"""
        try:
            logger.debug(f"[RPM_MDI] 🔍 檢查最速圈遙測數據可用性...")
            
            # 首先檢查是否已有遙測分析檔案
            telemetry_file = self._find_telemetry_analysis_file()
            
            if not telemetry_file:
                logger.debug(f"[RPM_MDI] 📡 遙測分析數據不存在，開始自動載入...")
                success = self._check_and_load_telemetry_if_needed()
                if success:
                    # 重新檢查檔案
                    telemetry_file = self._find_telemetry_analysis_file()
                else:
                    logger.error(f"[RPM_MDI] ❌ 遙測分析載入失敗")
                    return None
            
            if telemetry_file:
                logger.debug(f"[RPM_MDI] 📂 找到遙測分析檔案: {telemetry_file}")
                return self._extract_fastest_laps_from_telemetry(telemetry_file)
            else:
                logger.warning(f"[RPM_MDI] ⚠️ 無法獲取遙測分析數據")
                return None
                
        except Exception as e:
            logger.error(f"[RPM_MDI] _ensure_telemetry_data_for_fastest_laps 失敗: {e}")
            return None
    ...
    def _find_telemetry_analysis_file(self, year: Optional[str] = None,
        ...
        try:
            ...
            if not (target_year and target_race_raw and target_session):
                logger.warning("[RPM_MDI] 遙測檔案搜尋缺少必要參數")
                return None
            ...
            if os.path.exists(json_dir):
                for filename in os.listdir(json_dir):
                    if (filename.endswith('.json') and
                        ...
                        logger.debug(f"[RPM_MDI] 📂 找到遙測分析檔案: {full_path}")
                        return full_path

            logger.debug(f"[RPM_MDI] 📂 未找到遙測分析檔案 ({target_year} {race_key} {target_session})")
            return None

        except Exception as e:
            logger.error(f"[RPM_MDI] _find_telemetry_analysis_file 失敗: {e}")
            return None
    
    def _trigger_telemetry_analysis(self) -> bool:
        """觸發遙測分析載入/生成 - 與速度分析相同功能"""
        try:
            logger.debug(f"[RPM_MDI] 🚀 觸發遙測分析載入: {self.current_year} {self.current_race} {self.current_session}")
                ...
                if hasattr(main_window, 'mdi_area'):
                    ...
                    for sub_window in main_window.mdi_area.subWindowList():
                        ...
                        if "遙測分析" in window_title:
                            logger.debug(f"[RPM_MDI] 🎯 找到現有遙測分析視窗: {window_title}")
                            # 激活並刷新遙測分析視窗
                            main_window.mdi_area.setActiveSubWindow(sub_window)
                            return True
                    
                    # API-ONLY 模式：不自動創建視窗
                    logger.debug(f"[RPM_MDI] � [API-ONLY] 未找到現有遙測分析視窗")
                    logger.debug(f"[RPM_MDI] 💡 提示：請手動開啟遙測分析模組或通過 API 獲取數據")
                    return False
            
            # 方法2: 透過 API 檢查本地數據（不自動創建）
            logger.debug(f"[RPM_MDI] 🔍 檢查本地遙測分析數據...")
            return self._check_and_load_telemetry_if_needed()
            
        except Exception as e:
            logger.error(f"[RPM_MDI] _trigger_telemetry_analysis 失敗: {e}")
            return False
    
    def _extract_fastest_laps_from_telemetry(self, telemetry_file: str) -> Optional[Dict[str, int]]:
        """從遙測分析JSON檔案中提取最速圈數據 - 與速度分析相同功能"""
        try:
            logger.debug(f"[RPM_MDI] 📊 從遙測分析中提取最速圈數據: {telemetry_file}")
            ...
            logger.info(f"[RPM_MDI] ✅ 最速圈數據提取完成: {fastest_laps}")
            return fastest_laps if fastest_laps else None
            
        except Exception as e:
            logger.error(f"[RPM_MDI] _extract_fastest_laps_from_telemetry 失敗: {e}")
            return None
    
    def receive_main_window_update_notification(self, param_type, value):
        """接收主視窗參數更新通知 - 與速度分析相同功能"""
        try:
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] ========== 收到主視窗更新通知 ==========")
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] 📡 原始參數:")
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] - param_type: {param_type}")
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] - value: {value}")
            ...
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] 📊 當前模組狀態:")
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] - 當前年份: {self.current_year}")
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] - 當前賽事: {self.current_race}")
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] - 當前賽段: {self.current_session}")
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] - 當前車手: {getattr(self, 'driver1', 'VER')} vs {getattr(self, 'driver2', 'VER')}")
            logger.debug(f"[RPM_NOTIFICATION_DEBUG] - 當前圈數: 第{getattr(self, 'lap1', 1)}圈 vs 第{getattr(self, 'lap2', 1)}圈")
            ...
            # 重新載入數據 - 與速度分析模組保持一致
            if hasattr(self, 'data_manager') and self.data_manager:
                logger.debug(f"[REFRESH] 重新載入RPM數據...")
            ...
            logger.info(f"[NOTIFICATION] ⚡ RPM分析模組內容更新成功")
            
        except Exception as e:
            logger.error(f"[RPM_MDI] receive_main_window_update_notification 失敗: {e}")
            import traceback
            traceback.print_exc()
    ...
    def export_data(self, export_path: str, export_format: str = "json") -> bool:
        """匯出數據 - 實現抽象方法"""
        try:
            logger.debug(f"[RPM_MDI] 匯出數據功能尚未實現 (路徑: {export_path}, 格式: {export_format})")
            return False
        except Exception as e:
            logger.error(f"[RPM_MDI] export_data 失敗: {e}")
            return False
    ...
    def closeEvent(self, event):
        ...
        """
        logger.debug(f"[RPM_MDI] 🧹 視窗關閉事件觸發，開始清理資源...")
        
        try:
            # 清理數據載入器的執行緒
            if hasattr(self, 'data_manager') and self.data_manager:
                if hasattr(self.data_manager, '_rpm_loader'):
                    logger.debug(f"[RPM_MDI] 清理 DataLoader 執行緒...")
            ...
            logger.info(f"[RPM_MDI] ✅ 資源清理完成")
            
        except Exception as e:
            logger.warning(f"[RPM_MDI] ⚠️ 清理過程發生錯誤: {e}")
    ...
    def load_data(self, **kwargs) -> bool:
        ...
        try:
            ...
            logger.debug(f"[RPM_MDI] load_data 參數: use_time_axis={use_time_axis}")
            ...
            if self.data_manager:
                ...
                # 數據載入成功後設置時間軸模式
                if success and use_time_axis and self.rpm_chart_widget:
                    if hasattr(self.rpm_chart_widget, 'set_time_axis_mode'):
                        logger.debug(f"[RPM_MDI] 設置圖表時間軸模式: {use_time_axis}")
                        self.rpm_chart_widget.set_time_axis_mode(use_time_axis)
                
                return success
            return False
        except Exception as e:
            logger.error(f"[RPM_MDI] load_data 失敗: {e}")
            return False
    ...
    def get_current_data(self) -> dict:
        ...
        try:
            ...
            logger.error(f"[RPM_MDI] get_current_data 失敗: {e}")
            return {}

    def clear_data(self) -> None:
        """清除數據 - 實現抽象方法"""
        try:
            logger.debug(f"[RPM_MDI] 清除數據...")
            ...
            logger.error(f"[RPM_MDI] clear_data 失敗: {e}")

    def update_parameters(self, year: int, race: str, session: str) -> bool:
        """更新分析參數 - 實現抽象方法"""
        try:
            logger.debug(f"[RPM_PARAMS_DEBUG] ========== RPM參數更新開始 ==========")
            logger.debug(f"[RPM_PARAMS_DEBUG] 收到參數: year={year}, race={race}, session={session}")
            logger.debug(f"[RPM_PARAMS_DEBUG] 當前參數: year={self.current_year}, race={self.current_race}, session={self.current_session}")
            ...
            logger.debug(f"[RPM_PARAMS_DEBUG] 參數變化檢查: {params_changed}")
            logger.debug(f"[RPM_PARAMS_DEBUG] 舊參數: {old_year} {old_race} {old_session}")
            logger.debug(f"[RPM_PARAMS_DEBUG] 新參數: {new_year} {new_race} {new_session}")
            ...
            # 檢查是否需要載入數據
            logger.debug(f"[RPM_PARAMS_DEBUG] 檢查數據載入需求...")
            if params_changed or not hasattr(self, '_data_loaded'):
                logger.debug(f"[RPM_PARAMS_DEBUG] 需要載入數據：參數變化={params_changed}, 未載入過={not hasattr(self, '_data_loaded')}")
                ...
                if hasattr(self, 'data_manager') and self.data_manager:
                    ...
                    if success:
                        ...
                        logger.info(f"[RPM_PARAMS_DEBUG] ✅ RPM 數據重載成功")
                        return True
                    else:
                        logger.error(f"[RPM_PARAMS_DEBUG] ❌ RPM 數據重載失敗")
                        return False
                else:
                    # 檢查並創建數據管理器
                    logger.debug(f"[RPM_PARAMS_DEBUG] 數據管理器不存在，嘗試創建...")
                    try:
                        self.data_manager = RPMDataManager()
                        self.data_manager.data_loaded.connect(self._update_chart)
                        self.data_manager.error_occurred.connect(self._handle_error)
                        logger.info(f"[RPM_PARAMS_DEBUG] ✅ 數據管理器創建成功，開始載入數據...")
                        ...
                        if success:
                            self._data_loaded = True
                            logger.info(f"[RPM_PARAMS_DEBUG] ✅ RPM 數據載入成功")
                            return True
                        else:
                            logger.error(f"[RPM_PARAMS_DEBUG] ❌ RPM 數據載入失敗")
                            return False
                            
                    except Exception as e:
                        logger.error(f"[RPM_PARAMS_DEBUG] ❌ 數據管理器創建失敗: {e}")
                        logger.warning(f"[RPM_PARAMS_DEBUG] ⚠️ 參數更新完成（無數據載入）: {self.current_year} {self.current_race} {self.current_session}")
                        return False
            else:
                logger.debug(f"[RPM_PARAMS_DEBUG] 跳過數據載入：參數無變化且已載入過")
                return True
            
        except Exception as e:
            logger.error(f"[RPM_PARAMS_DEBUG] update_parameters 失敗: {e}")
            logger.error(f"[RPM_PARAMS_DEBUG] update_parameters 失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def refresh_analysis(self) -> None:
        """重新分析 - 實現抽象方法"""
        try:
            logger.debug(f"[RPM_MDI] 重新分析...")
            self._refresh_data()
        except Exception as e:
            logger.error(f"[RPM_MDI] refresh_analysis 失敗: {e}")
...
try:
    ...
    logger.info(f"[MODULE_FACTORY] RPM analysis module registered")
except ImportError as e:
    logger.warning(f"[MODULE_FACTORY] RPM分析模組註冊失敗: {e}")
