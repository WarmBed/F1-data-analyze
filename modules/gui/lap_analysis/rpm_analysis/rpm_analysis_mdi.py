...
class CrossEventComparisonWorker(QThread):
    ...
    def __init__(self, driver1: str, year1: int, race1: str, session1: str, lap1: int,
        ...
        try:
            ...
            try:
                ...
                print(f"[RPM-CROSS-EVENT-WORKER] ✅ API base URL: {self.base_url}")
            except Exception as e:
                # 如果解析失敗，使用硬編碼的公開 API
                from core.api_base_url import PUBLIC_API_BASE_URL
                self.base_url = PUBLIC_API_BASE_URL.rstrip('/')
                print(f"[RPM-CROSS-EVENT-WORKER] ⚠️ API URL 解析失敗，使用預設: {self.base_url}")
                print(f"[RPM-CROSS-EVENT-WORKER] 錯誤: {e}")
                
        except Exception as e:
            print(f"[ERROR] [RPM-CROSS-EVENT-WORKER] Worker 初始化失敗: {e}")
            import traceback
            traceback.print_exc()
    ...
    def run(self):
        ...
        try:
            print(f"[RPM-CROSS-EVENT-WORKER] 開始執行 API 請求")
            ...
            print(f"[RPM-CROSS-EVENT-WORKER] 目標端點: {endpoint}")
            ...
            print(f"[RPM-CROSS-EVENT-WORKER] 請求參數: {query_params}")
            ...
            print(f"[RPM-CROSS-EVENT-WORKER] 發送 POST 請求...")
            ...
            print(f"[RPM-CROSS-EVENT-WORKER] 收到回應，狀態碼: {response.status_code}")
            response.raise_for_status()

            payload = response.json()
            print(f"[RPM-CROSS-EVENT-WORKER] JSON 解析成功")
            ...
            print(f"[RPM-CROSS-EVENT-WORKER] ✅ 請求成功，發送 success 信號")
            self.success.emit({"data": data, "meta": meta})
            
        except requests.exceptions.Timeout as e:
            error_msg = f"API 請求超時 ({self.timeout}秒): {e}"
            print(f"[RPM-CROSS-EVENT-WORKER] ❌ {error_msg}")
            ...
            try:
                ...
                print(f"[RPM-CROSS-EVENT-WORKER] Worker 執行完成")
            except:
