...
class RPMChartWidget(QWidget, LapAnalysisLinkageMixin, LapAnalysisLinkageDrawingMixin):
    ...
    def set_rpm_data(self, distance: List[float], driver1_rpm: List[float], 
        ...
        if lap1 is not None and lap2 is not None and lap1 != lap2 and driver1_name == driver2_name:
            ...
            print(f"[RPM_CHART] 🔄 雙圈比較模式: {original_driver} {driver1_name} vs {driver2_name}")
    ...
    def set_time_axis_mode(self, use_time_axis: bool):
        ...
        if self.use_time_axis != use_time_axis:
            ...
            print(f"[RPM_CHART] 🕒 時間軸模式切換: {use_time_axis}")
            
            # 重新計算 X 軸範圍
            if self.use_time_axis and self.driver1_time:
                self.min_distance = min(self.driver1_time)
                self.max_distance = max(self.driver1_time)
                print(f"[RPM_CHART]    時間範圍: {self.min_distance:.2f}s - {self.max_distance:.2f}s")
            elif self.distance_data:
                self.min_distance = min(self.distance_data)
                self.max_distance = max(self.distance_data)
                print(f"[RPM_CHART]    距離範圍: {self.min_distance:.0f}m - {self.max_distance:.0f}m")
...
class RPMAnalysisChartWidget(QWidget, LapAnalysisLinkageMixin, LapAnalysisLinkageDrawingMixin):
    ...
    def set_time_axis_mode(self, use_time_axis: bool):
        ...
        if hasattr(self, 'chart_widget') and self.chart_widget:
            ...
            print(f"[RPM_WRAPPER] ✅ 時間軸模式已設置: {use_time_axis}")
    ...
    def update_rpm_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            print(f"[RPM_CHART] 🕒 車手1 時間數據點: {len(driver1_time)}")
            print(f"[RPM_CHART] 🕒 車手2 時間數據點: {len(driver2_time)}")
            ...
            if len(drivers) >= 2:
                ...
                print(f"[RPM_CHART] 🔢 提取圈數: lap1={lap1}, lap2={lap2}")
            ...
            if metadata.get('is_single_driver', False):
                ...
                if lap1 is not None and lap2 is not None and lap1 != lap2:
                    ...
                    is_single_driver_mode = False
                    print(f"[RPM_CHART] � 檢測到雙圈比較模式: {driver1_name} 第{lap1}圈 vs 第{lap2}圈")
                else:
                    # 同車手相同圈數或無圈數信息 → 單車手模式
                    is_single_driver_mode = True
                    print(f"[RPM_CHART] 🔍 檢測到相同車手比較（單車手模式）: {driver1_name}")
            elif len(drivers) == 1:
                # 只有一個車手的數據
                is_single_driver_mode = True
            ...
            if is_single_driver_mode:
                ...
                print(f"[RPM_CHART] 🔄 使用雙圈比較模式顯示: {driver1_name} 第{lap1}圈 vs 第{lap2}圈")
    ...
    def reset_chart_view(self):
        """重置圖表視圖 - 與速度分析保持一致"""
        print(f"[RPM_ANALYSIS] 🔄 reset_chart_view() 被調用")
        if hasattr(self, 'chart_widget') and self.chart_widget:
            print(f"[RPM_ANALYSIS] ✅ 找到 chart_widget，調用 reset_view()")
            self.chart_widget.reset_view()
        else:
            print(f"[RPM_ANALYSIS] ❌ 未找到 chart_widget 屬性")
        ...
        try:
            ...
            try:
                ...
                if linkage_manager:
                    ...
                    print(f"[RPM_CHART]   ✅ 已從連動管理器解除註冊")
            except Exception as e:
                print(f"[RPM_CHART]   ⚠️ 解除註冊警告: {e}")
            ...
            if hasattr(self, 'chart_widget') and self.chart_widget:
                if hasattr(self.chart_widget, 'figure') and self.chart_widget.figure:
                    try:
                        ...
                        print(f"[RPM_CHART]   ✅ Matplotlib 圖表已清理")
                    except Exception as e:
                        print(f"[RPM_CHART]   ⚠️ Matplotlib 清理警告: {e}")
                ...
                if hasattr(self.chart_widget, 'canvas') and self.chart_widget.canvas:
                    try:
                        ...
                        print(f"[RPM_CHART]   ✅ Canvas 已清理")
                    except Exception as e:
                        print(f"[RPM_CHART]   ⚠️ Canvas 清理警告: {e}")
            ...
            if hasattr(self, 'stats_table') and self.stats_table:
                try:
                    ...
                    print(f"[RPM_CHART]   ✅ QTableWidget 已完全清理")
                except Exception as e:
                    print(f"[RPM_CHART]   ⚠️ QTableWidget 清理警告: {e}")
            ...
            if hasattr(self, 'receiver') and self.receiver:
                try:
                    ...
                    self.receiver = None
                    print(f"[RPM_CHART]   ✅ Signal Receiver 已清理")
                except Exception as e:
                    print(f"[RPM_CHART]   ⚠️ Receiver 清理警告: {e}")
            ...
            if hasattr(self, 'chart_widget') and self.chart_widget:
                try:
                    ...
                    print(f"[RPM_CHART]   ✅ ChartWidget 已清理")
                except Exception as e:
                    print(f"[RPM_CHART]   ⚠️ ChartWidget 清理警告: {e}")
            
            # 6. 清理資料載入器引用
            if hasattr(self, 'rpm_loader'):
                self.rpm_loader = None
                print(f"[RPM_CHART]   ✅ 資料載入器引用已清空")
            
            print(f"[RPM_CHART] ✅ 資源清理完成")
            
        except Exception as e:
            print(f"[ERROR] [RPM_CHART] cleanup 失敗: {e}")
            import traceback
            traceback.print_exc()
