                ...
                if os.path.exists(directory):
                    for pattern in telemetry_patterns:
                        ...
                        if os.path.exists(file_path):
                            print(f"📁 [brake_MDI_DATA] 找到現有遙測檔案: {file_path}")
                            return True

            print("⚠️ [brake_MDI_DATA] API-ONLY 模式下未找到遙測檔案，請先透過主視窗遙測模組或 REST API 取得資料")
            return False
...
class BrakeAnalysisModule(IAnalysisModule):
    ...
    def _update_info_label(self):
        ...
        try:
            ...
            if hasattr(self, 'info_label'):
                ...
                print(f"[BRAKE_MDI] 已更新資訊標籤: {info_text}")
                
        except Exception as e:
            print(f"[ERROR] [BRAKE_MDI] _update_info_label 失敗: {e}")
    ...
    def update_cross_event_comparison(self, year1: str, race1: str, session1: str, driver1: str, lap1: int,
        ...
        """更新跨賽事比較參數（支援跨年度/跨賽段）"""
        try:
            print(f"[BRAKE-CROSS-EVENT] ========== 跨賽事比較更新 ==========")
            print(f"[BRAKE-CROSS-EVENT] 車手 1: {year1} {race1} {session1} {driver1} 第{lap1}圈")
            print(f"[BRAKE-CROSS-EVENT] 車手 2: {year2} {race2} {session2} {driver2} 第{lap2}圈")
            print(f"[BRAKE-CROSS-EVENT] 🕒 時間軸模式: {use_time_axis}")
            ...
            print(f"[BRAKE-CROSS-EVENT] ⚠️ 已停用同步模式 (sync_driver_lap_enabled = False)")
            
            # 保存時間軸設定
            self.use_time_axis = use_time_axis
            print(f"[BRAKE-CROSS-EVENT] 🕒 已保存時間軸設定: use_time_axis={use_time_axis}")
            ...
            print(f"[ERROR] [BRAKE-CROSS-EVENT] 跨賽事比較更新失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _on_cross_event_data_loaded(self, result: Dict[str, Any]) -> None:
        """處理跨賽事比較數據載入成功"""
        try:
            print(f"[BRAKE-CROSS-EVENT] ✅ 數據載入成功")
            
            # 提取數據
            data = result.get("data", {})
            meta = result.get("meta", {})
            
            print(f"[BRAKE-CROSS-EVENT] 數據鍵值: {list(data.keys())}")
            print(f"[BRAKE-CROSS-EVENT] 元數據: {meta}")
            
            # 檢查是否有遙測比較數據
            if "telemetry_comparison" in data:
                telemetry_comp = data["telemetry_comparison"]
                print(f"[BRAKE-CROSS-EVENT] 遙測參數: {list(telemetry_comp.keys())}")
                ...
                if "Brake" in telemetry_comp:
                    ...
                    if self.brake_chart_widget and hasattr(self.brake_chart_widget, 'set_time_axis_mode'):
                        print(f"[BRAKE-CROSS-EVENT] 🕒 設置圖表時間軸模式: {use_time_axis}")
                        self.brake_chart_widget.set_time_axis_mode(use_time_axis)
                    
                    # 更新圖表
                    print(f"[BRAKE-CROSS-EVENT] 🔄 更新圖表...")
                    self._update_chart(chart_data)
                    print(f"[BRAKE-CROSS-EVENT] ✅ 跨賽事比較完成")
                else:
                    print(f"[BRAKE-CROSS-EVENT] ⚠️ API 回應中沒有 Brake 數據")
            else:
                print(f"[BRAKE-CROSS-EVENT] ⚠️ API 回應中沒有 telemetry_comparison")
                
        except Exception as e:
            print(f"[ERROR] [BRAKE-CROSS-EVENT] 處理數據失敗: {e}")
            import traceback
            traceback.print_exc()
    
    def _on_cross_event_load_error(self, error_msg: str) -> None:
        """處理跨賽事比較數據載入錯誤"""
        print(f"[BRAKE-CROSS-EVENT] ❌ 跨賽事比較失敗: {error_msg}")
    ...
    def update_from_shared_params(self, params: dict):
        ...
        if self._updating_from_shared:
            print(f"[BRAKE_MDI] [SHARED_PARAMS] ⚠️  正在更新中，防止遞迴")
            return
        
        self._updating_from_shared = True
        try:
            print(f"[BRAKE_MDI] [SHARED_PARAMS] 🔄 從全域共享池更新參數")
            print(f"[BRAKE_MDI] [SHARED_PARAMS] 收到參數: {params}")
            ...
            if is_cross_event:
                print(f"[BRAKE_MDI] [SHARED_PARAMS] 🌍 檢測到跨賽事比較:")
                print(f"[BRAKE_MDI] [SHARED_PARAMS]   車手 1: {year1} {race1} {session1} {driver1} 第{lap1}圈")
                print(f"[BRAKE_MDI] [SHARED_PARAMS]   車手 2: {year2} {race2} {session2} {driver2} 第{lap2}圈")
                
                # 調用跨賽事比較方法
                print(f"[BRAKE_MDI] [SHARED_PARAMS] 🔄 調用 update_cross_event_comparison")
                ...
                if success:
                    print(f"[BRAKE_MDI] [SHARED_PARAMS] ✅ 跨賽事比較更新成功")
                    # ⚠️ [參數資訊標籤] 更新資訊標籤顯示
                    self._update_info_label()
                    print(f"[BRAKE_MDI] [SHARED_PARAMS] 📋 已更新資訊標籤")
                else:
                    print(f"[BRAKE_MDI] [SHARED_PARAMS] ❌ 跨賽事比較更新失敗")
            else:
                # 標準模式（同一賽事比較）
                print(f"[BRAKE_MDI] [SHARED_PARAMS] ✅ 標準比較模式:")
                print(f"[BRAKE_MDI] [SHARED_PARAMS]   賽事: {year1} {race1} {session1}")
                print(f"[BRAKE_MDI] [SHARED_PARAMS]   車手: {driver1} vs {driver2}")
                print(f"[BRAKE_MDI] [SHARED_PARAMS]   圈數: 第{lap1}圈 vs 第{lap2}圈")
                
                # 調用標準更新方法
                print(f"[BRAKE_MDI] [SHARED_PARAMS] 🔄 調用 update_lap_parameters")
                ...
                if success:
                    print(f"[BRAKE_MDI] [SHARED_PARAMS] ✅ 標準參數更新成功")
                    # ⚠️ [參數資訊標籤] 更新資訊標籤顯示
                    self._update_info_label()
                    print(f"[BRAKE_MDI] [SHARED_PARAMS] 📋 已更新資訊標籤")
                else:
                    print(f"[BRAKE_MDI] [SHARED_PARAMS] ❌ 標準參數更新失敗")
                
        except Exception as e:
            print(f"[ERROR] [BRAKE_MDI] [SHARED_PARAMS] 更新失敗: {e}")
            import traceback
            traceback.print_exc()
        finally:
    ...
    def reset_chart_view(self):
        ...
        print(f"[BRAKE_MDI] 🔄 reset_chart_view() 被調用")
        if hasattr(self, 'brake_chart_widget') and self.brake_chart_widget:
            print(f"[BRAKE_MDI] ✅ 找到 brake_chart_widget，調用 reset_chart_view()")
            self.brake_chart_widget.reset_chart_view()
        else:
            print(f"[BRAKE_MDI] ❌ 未找到 brake_chart_widget 屬性")
    ...
    def _check_and_load_telemetry_if_needed(self, year: Optional[str] = None,
        ...
        try:
            ...
            print(f"[ERROR] [brake_MDI] _check_and_load_telemetry_if_needed 失敗: {e}")
            return False
    ...
    def _find_telemetry_analysis_file(self, year: Optional[str] = None,
        ...
        try:
            ...
            if not (target_year and target_race_raw and target_session):
                print("⚠️ [brake_MDI] 遙測檔案搜尋缺少必要參數")
                return None
            ...
            print(f"[brake_MDI] 📂 未找到遙測分析檔案 ({target_year} {race_key} {target_session})")
            return None
