    ...
    def update_lap_parameters(self, year: str, race: str, session: str, 
        ...
        try:
            ...
            if self.brake_chart_widget:
                ...
                print(f"[brake_MDI] ✅ 已設置時間軸模式: {use_time_axis}")
        ...
        try:
            ...
            if is_fastest and hasattr(self, '_ensure_telemetry_data_for_fastest_laps'):
                ...
                if fastest_laps:
                    if driver1 in fastest_laps:
                        lap1 = fastest_laps[driver1]
                        print(f"[brake_MDI] 🏁 {driver1} 最速圈: 第{lap1}圈")
                    if driver2 and driver2 in fastest_laps:
                        lap2 = fastest_laps[driver2]
                        print(f"[brake_MDI] 🏁 {driver2} 最速圈: 第{lap2}圈")
                else:
                    print("[brake_MDI] ⚠️ 無法獲取最速圈數據，改用現有圈數")
            ...
            if hasattr(self.brake_chart_widget, 'set_lap_numbers'):
                self.brake_chart_widget.set_lap_numbers(self.lap1, self.lap2)
            ...
            if not params_changed:
                print("[brake_MDI] ℹ️ 參數無變化，保留現有數據")
                return True

            if not self.data_manager:
                print("[brake_MDI] ❌ 數據管理器未初始化，無法更新數據")
                return False
            ...
            if success:
                ...
                print("[brake_MDI] ✅ 圈速參數更新成功")
                return True

            print("[brake_MDI] ❌ 數據載入失敗")
            return False

        except Exception as e:
            print(f"[ERROR] [brake_MDI] update_lap_parameters 失敗: {e}")
            import traceback
            traceback.print_exc()
            return False
