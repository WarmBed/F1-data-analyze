...
class distancediffChartWidget(QWidget, LapAnalysisLinkageMixin, LapAnalysisLinkageDrawingMixin):
    """distancediff圖表繪製組件 - 使用 PyQt5 原生繪圖"""
    ...
    def __init__(self, parent=None):
        ...
        # 數據存儲
        self.distance_data = []
        self.driver1_distancediff = []
        self.driver2_distancediff = []
        ...
        # 數據範圍 - 累積距離差範圍設為-100到+100
        self.min_distance = 0
        self.max_distance = 5807
        self.min_distancediff = -100  # 改為-100
        self.max_distancediff = 100   # 改為+100
    ...
    def set_distancediff_data(self, distance: List[float], driver1_distancediff: List[float], 
        ...
        # 計算累積距離差的動態範圍
        all_distancediffs = []
        if driver1_distancediff:
            all_distancediffs.extend(driver1_distancediff)
        if driver2_distancediff:
            all_distancediffs.extend(driver2_distancediff)
            
        if all_distancediffs:
            # 為累積距離差計算合適的範圍
            data_min = min(all_distancediffs)
            data_max = max(all_distancediffs)
            
            # 添加一些邊距以便更好地顯示
            margin = max(10, (data_max - data_min) * 0.1)  # 至少10單位的邊距
            
            self.min_distancediff = data_min - margin
            self.max_distancediff = data_max + margin
    ...
    def _draw_grid(self, painter: QPainter, chart_rect: QRect):
        ...
        current_min_distance = self.view_min_distance if self.view_min_distance is not None else self.min_distance
        current_max_distance = self.view_max_distance if self.view_max_distance is not None else self.max_distance
        current_min_distancediff = self.view_min_distancediff if self.view_min_distancediff is not None else self.min_distancediff
        current_max_distancediff = self.view_max_distancediff if self.view_max_distancediff is not None else self.max_distancediff
        ...
        # 水平網格線 (distancediff) - 修正：與速度分析保持一致使用10條線
        distancediff_range = current_max_distancediff - current_min_distancediff
        if distancediff_range > 0:
            num_h_lines = 10  # 修正：改為10條線與速度分析一致
            for i in range(num_h_lines + 1):
                distancediff = current_min_distancediff + (distancediff_range * i / num_h_lines)
                y = chart_rect.bottom() - (distancediff - current_min_distancediff) / distancediff_range * chart_rect.height()
    ...
    def _draw_axes(self, painter: QPainter, chart_rect: QRect):
        ...
        # Y軸標籤 (distancediff) - 修正：與速度分析一致，只顯示偶數刻度
        distancediff_range = current_max_distancediff - current_min_distancediff
        if distancediff_range > 0:
            num_labels = 10  # 使用10個間隔
            for i in range(0, num_labels + 1, 2):  # 只顯示偶數刻度
                distancediff = current_min_distancediff + (distancediff_range * i / num_labels)
                y = chart_rect.bottom() - (distancediff - current_min_distancediff) / distancediff_range * chart_rect.height()
    ...
    def _draw_distancediff_curves(self, painter: QPainter, chart_rect: QRect):
        """繪製distancediff曲線 - 支援時間軸模式"""
        ...
        if distance_range <= 0 or distancediff_range <= 0:
            return
        
        # 繪製零線 (Y=0 的水平線)
        if current_min_distancediff <= 0 <= current_max_distancediff:
            zero_y = chart_rect.bottom() - (0 - current_min_distancediff) / distancediff_range * chart_rect.height()
        ...
        # 繪製累積距離差異曲線 - 支援時間軸模式
        if self.driver1_distancediff and len(self.driver1_distancediff) == len(self.distance_data):
            points = []
            ...
            # 預先計算所有點位置
            for i, (x_value, distancediff) in enumerate(zip(x_data_source, self.driver1_distancediff)):
                if current_min_distance <= x_value <= current_max_distance:
                    ...
                    y = chart_rect.bottom() - (distancediff - current_min_distancediff) / distancediff_range * chart_rect.height()
                    points.append((QPoint(int(x), int(y)), distancediff))
    ...
    def _draw_tracking_line(self, painter: QPainter, chart_rect: QRect, x_pos: int, is_fixed: bool):
        ...
        if distance_range > 0 and self.distance_data:
            ...
            # 找到最接近的數據點來獲取真實的distancediff值
            driver1_distancediff_at_position = None
            driver2_distancediff_at_position = None
            ...
            if search_data and len(search_data) > 0:
                ...
                # 獲取對應的distancediff值
                if closest_index < len(self.driver1_distancediff):
                    driver1_distancediff_at_position = self.driver1_distancediff[closest_index]
                if closest_index < len(self.driver2_distancediff):
                    driver2_distancediff_at_position = self.driver2_distancediff[closest_index]
            
            # 計算需要顯示的車手數量來調整標籤大小
            drivers_to_show = []
            
            # 只添加有效且不重複的車手資訊
            if driver1_distancediff_at_position is not None and self.driver1_name:
                drivers_to_show.append((self.driver1_name, driver1_distancediff_at_position, self.driver1_color))
            
            # 只有在非單車手模式且第二個車手數據不同時才添加第二個車手
            if (not getattr(self, 'is_single_driver', False) and 
                driver2_distancediff_at_position is not None and 
                self.driver2_name and 
                self.driver2_name != self.driver1_name):
                drivers_to_show.append((self.driver2_name, driver2_distancediff_at_position, self.driver2_color))
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self.middle_dragging and not self.last_drag_pos.isNull():
            ...
            if chart_rect.width() > 0 and chart_rect.height() > 0:
                ...
                # Y軸移動（distancediff）
                distancediff_range = (self.view_max_distancediff or self.max_distancediff) - (self.view_min_distancediff or self.min_distancediff)
                distancediff_move = dy * distancediff_range / chart_rect.height()  # Y軸是倒置的
                
                # 更新視圖範圍
                if self.view_min_distance is None:
                    self.view_min_distance = self.min_distance
                    self.view_max_distance = self.max_distance
                if self.view_min_distancediff is None:
                    self.view_min_distancediff = self.min_distancediff
                    self.view_max_distancediff = self.max_distancediff
                
                self.view_min_distance += distance_move
                self.view_max_distance += distance_move
                self.view_min_distancediff += distancediff_move
                self.view_max_distancediff += distancediff_move
    ...
    def wheelEvent(self, event: QWheelEvent):
        ...
        if chart_rect.contains(event.pos()):
            ...
            distancediff_range = self.view_max_distancediff - self.view_min_distancediff
            
            mouse_distance = self.view_min_distance + mouse_rel_x * distance_range
            mouse_distancediff = self.view_min_distancediff + mouse_rel_y * distancediff_range
            
            # 計算新的範圍
            new_distance_range = distance_range / zoom_factor
            new_distancediff_range = distancediff_range / zoom_factor
            ...
            self.view_min_distancediff = max(self.min_distancediff, 
                                  mouse_distancediff - new_distancediff_range * mouse_rel_y)
            self.view_max_distancediff = min(self.max_distancediff, 
                                  mouse_distancediff + new_distancediff_range * (1 - mouse_rel_y))
...
class distancediffAnalysisChartWidget(QWidget, LapAnalysisLinkageMixin, LapAnalysisLinkageDrawingMixin):
    """distancediff分析圖表組件主容器"""
    ...
    def __init__(self, parent=None):
        ...
        # 🔴 移除容器類的重複註冊（內部的 distancediffChartWidget 將在 _create_chart_area() 中註冊）
        # 避免雙重註冊導致的記憶體洩漏問題
        # if linkage_manager:
        #     linkage_manager.register_module(self, "distancediff_analysis")
    ...
    def _create_chart_area(self) -> QWidget:
        ...
        # 創建圖表組件
        self.chart_widget = distancediffChartWidget()
        layout.addWidget(self.chart_widget)
        
        # 確保內部圖表組件也註冊到連動管理器
        if linkage_manager:
            linkage_manager.register_module(self.chart_widget, "distancediff_analysis_chart")
    ...
    def update_distancediff_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            reference = distancediff_data.get('reference', '')
    ...
    def _prepare_chart_data(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """準備圖表數據"""
        try:
            if 'distancediff_telemetry' in data:
                # 直接distancediff數據
                return self._parse_distancediff_telemetry(data['distancediff_telemetry'])
            
            elif 'speed_data' in data:
                # 從速度數據模擬distancediff數據
                return self._simulate_distancediff_from_speed(data['speed_data'])
    ...
    def _parse_distancediff_telemetry(self, distancediff_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 解析車手1數據
        if 'driver1_distancediff_data' in distancediff_data:
            for point in distancediff_data['driver1_distancediff_data']:
                distance.append(point.get('distance', 0))
                driver1_distancediff.append(point.get('distancediff', 0))
        
        # 解析車手2數據
        if 'driver2_distancediff_data' in distancediff_data:
            for point in distancediff_data['driver2_distancediff_data']:
                driver2_distancediff.append(point.get('distancediff', 0))
        
        return {
            'distance': distance,
            'driver1_distancediff': driver1_distancediff,
            'driver2_distancediff': driver2_distancediff,
            'driver1_name': distancediff_data.get('driver1_name', 'Driver 1'),
            'driver2_name': distancediff_data.get('driver2_name', 'Driver 2'),
            'sectors': distancediff_data.get('sectors', []),
            'engine_info': distancediff_data.get('engine_info', {}),
            'track_info': distancediff_data.get('track_info', {})
        }
    ...
    def _update_statistics_table(self, statistics: Dict, driver1_name: str, driver2_name: str):
        ...
        try:
            ...
            rows = [
                ("最大距離差 (m)", 
                 f"{driver1_stats.get('max_distancediff', 0):.1f}",
                 f"{driver2_stats.get('max_distancediff', 0):.1f}",
                 f"{comparison.get('max_distancediff_diff', 0):.1f}"),
                ("平均距離差 (m)",
                 f"{driver1_stats.get('avg_distancediff', 0):.1f}",
                 f"{driver2_stats.get('avg_distancediff', 0):.1f}",
                 f"{comparison.get('avg_distancediff_diff', 0):.1f}"),
                ("最小距離差 (m)",
                 f"{driver1_stats.get('min_distancediff', 0):.1f}",
                 f"{driver2_stats.get('min_distancediff', 0):.1f}",
    ...
    def reload_data(self):
        ...
        if self.current_data:
            self.update_distancediff_data(self.current_data)
...
if __name__ == "__main__":
    ...
    # 測試distancediff圖表組件
    widget = distancediffAnalysisChartWidget()
    widget.setWindowTitle("🔄 distancediff分析圖表測試")
