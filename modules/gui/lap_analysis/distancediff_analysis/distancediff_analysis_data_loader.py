...
class DistanceDiffAnalysisDataLoader(TelemetryDataLoader):
    ...
    def __init__(self, parent=None):
        ...
        Args:
            parent: 父級 QObject
        """
        # 初始化基類，指定遙測類型為 'distancediff'
        super().__init__('distancediff', parent)
    ...
    def get_distancediff_statistics(self) -> Optional[Dict[str, Any]]:
        """
        獲取距離差統計數據
        
        Returns:
            Dict: 距離差統計數據，如果沒有數據則返回 None
        """
    ...
    def get_max_distancediff(self, driver: str = 'driver1') -> Optional[float]:
        """
        獲取指定車手的最大距離差
        ...
        Returns:
            float: 最大距離差值，如果沒有數據則返回 None
        """
        stats = self.get_distancediff_statistics()
        if stats and driver in stats:
            return stats[driver].get('max', None)
        return None
    
    def get_avg_distancediff(self, driver: str = 'driver1') -> Optional[float]:
        """
        獲取指定車手的平均距離差
...
def create_distancediff_loader(parent=None) -> DistanceDiffAnalysisDataLoader:
    """
    創建距離差數據載入器的工廠函數
    ...
    Returns:
        DistanceDiffAnalysisDataLoader: 距離差數據載入器實例
    """
    return DistanceDiffAnalysisDataLoader(parent)


# ========== 向後兼容的類別別名 ==========

# 為了支持舊代碼中的 camelCase 導入方式，添加別名
distancediffAnalysisDataLoader = DistanceDiffAnalysisDataLoader
...
if __name__ == "__main__":
    ...
    distancediff_loader.status_changed.connect(on_status_changed)
