...
"""
全車手直線速度分析 - 雙視圖容器
All Drivers Straight Line Speed - Dual View Container
...
from .all_drivers_straight_line_speed_widget import AllDriversStraightLineSpeedWidget

# 導入國際化
from core.gui_i18n import tr


class AllDriversStraightLineSpeedDualView(QWidget):
    """
    全車手直線速度分析 - 雙視圖容器
    ...
    def _init_ui(self):
        ...
        # ===== Tab 1: 表格視圖（立即創建）=====
        self.table_view = AllDriversStraightLineSpeedTableWidget()
        self.tab_widget.addTab(
            self.table_view,
            tr('speed_analysis_tab_table', '📊 表格視圖')
        )
        ...
        self.tab_widget.addTab(
            ...
            tr('speed_analysis_tab_chart', '📈 圖表視圖')
        )
    ...
    def update_data(self, data: Dict[str, Any]):
        ...
        Args:
            data: 包含 driver_speeds 的字典
        """
        try:
            print(f"[DUAL_VIEW] 接收數據，準備更新視圖")
            ...
            if self.chart_view is not None:
                ...
                print("[DUAL_VIEW] 圖表視圖已更新")
            else:
                print("[DUAL_VIEW] 圖表視圖尚未創建，將在切換時延遲載入")
            
            # 發射信號
            self.data_loaded.emit(data)
            
            print(f"[DUAL_VIEW] 視圖更新完成")
            
        except Exception as e:
            print(f"[ERROR] [DUAL_VIEW] 更新數據失敗: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_tab_changed(self, index: int):
        ...
        """
        if index == 0:
            print("[DUAL_VIEW] 切換到表格視圖")
        elif index == 1:
            print("[DUAL_VIEW] 切換到圖表視圖")
            
            # ✅ 延遲載入：第一次切換時才創建 Matplotlib widget
            if self.chart_view is None:
                print("[DUAL_VIEW] 首次切換，創建 Matplotlib widget...")
                self.chart_view = AllDriversStraightLineSpeedWidget()
                ...
                print("✅ [DUAL_VIEW] Matplotlib widget 創建完成")
    ...
    def export_current_view(self, file_path: str) -> bool:
        ...
        try:
            ...
            print(f"[ERROR] [DUAL_VIEW] 匯出失敗: {e}")
            return False
