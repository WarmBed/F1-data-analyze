...
try:
    ...
    output = []
    output.append(f"總記錄數: {len(records)}")
    output.append(f"比賽時長約 2 小時 = 7200 秒")
    output.append(f"平均採樣率: {len(records) / 7200:.2f} Hz")

    output.append("")
    output.append("時間戳樣本 (連續 20 筆):")
    for i, rec in enumerate(records[1000:1020]):
        output.append(f"  {i}: {rec['timestamp']}")
    ...
    if intervals:
        ...
        output.append(f"  平均間隔: {avg_interval*1000:.1f} ms")
        output.append(f"  最小間隔: {min_interval*1000:.1f} ms")  
        output.append(f"  最大間隔: {max_interval*1000:.1f} ms")
        output.append(f"  採樣率: {1/avg_interval:.1f} Hz")

    # 測試：找到起跑時刻各車手的第一個非零速度時間
    output.append("")
    output.append("起跑反應時間測試:")
    start_times = {}
    for rec in records:
        ts = rec['timestamp']
