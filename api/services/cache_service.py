...
class F1AnalysisCacheService:
    ...
    def __init__(self, json_dir: str = "json/", cache_dir: str = "cache/"):
        ...
        self.function_file_patterns = {
            ...
            "101": ["F101_season_start_reaction"],  # ✅ 添加 Function 101 - 年度起跑反應分析
    ...
    def _search_exact_match(self, function_id: str, **params) -> Optional[Dict]:
        ...
        for pattern_base in patterns:
            ...
            elif function_id == "100":  # ✅ 歷年旗幟統計 - 精確搜尋優化
                # 檔案格式: historical_flags_{race}_{start_year}-{end_year}.json
                # ⚠️ CLI 使用 _sanitize_token() 處理 race，保持原始大小寫
                # 範例: historical_flags_Australia_2022-2025.json
                #       historical_flags_Abu_Dhabi_2022-2025.json (空格→底線)
                search_patterns = []
                print(f"[CACHE] 🏁 Function 100: 搜尋歷年旗幟統計")
                print(f"[CACHE]    賽道參數: {race_param}")
                
                # ✅ 優先順序 1: 原始格式（空格→底線）
                race_sanitized = race_param.replace(" ", "_") if race_param else "*"
                search_patterns.append(f"{self.json_dir}historical_flags_{race_sanitized}_*-*.json")
                
                # ✅ 優先順序 2: 首字母大寫
                race_title = race_sanitized.title()
                if race_title != race_sanitized:
                    search_patterns.append(f"{self.json_dir}historical_flags_{race_title}_*-*.json")
                
                # ✅ 優先順序 3: 小寫
                race_lower = race_sanitized.lower()
                if race_lower != race_sanitized and race_lower != race_title.lower():
                    search_patterns.append(f"{self.json_dir}historical_flags_{race_lower}_*-*.json")
                
                # ✅ 優先順序 4: 萬用字元備案
                search_patterns.append(f"{self.json_dir}historical_flags_*{race_param.replace(' ', '*')}*_*-*.json")
                
                print(f"[CACHE]    搜尋模式: {search_patterns[0]}")  # 只顯示第一個
