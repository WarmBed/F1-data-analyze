#!/usr/bin/env python3
"""
F1T GUI Demo - 風格H: 專業賽車分析工作站
Demo for Style H: Professional Racing Analysis Workstation
移除參數面板，右鍵功能選單，系統日誌，曲線圖表，賽道圖，純黑底
"""
...
class TelemetryChartWidget(QWidget):
    ...
    def wheelEvent(self, event):
        ...
        if chart_area.contains(event.pos()):
            ...
            if modifiers & Qt.ControlModifier:
                ...
                print(f"🔍 同步縮放: X={self.x_scale:.2f}, Y={self.y_scale:.2f}")
    ...
    def draw_speed_curve(self, painter, chart_area):
        ...
        for i in range(0, chart_area.width(), 2):
            ...
            # 模擬速度變化 (0-350 km/h)
            speed = 150 + 100 * math.sin(real_x * 0.01) + 50 * math.cos(real_x * 0.03) + random.uniform(-10, 10)
            speed = max(0, min(350, speed))  # 限制在合理範圍
    ...
    def draw_brake_curve(self, painter, chart_area):
        ...
        for i in range(0, chart_area.width(), 2):
            ...
            # 模擬煞車壓力 (0-100%)
            brake = max(0, 30 + 40 * math.sin(real_x * 0.02) + 20 * math.cos(real_x * 0.05) + random.uniform(-5, 5))
            ...
            x_pos = chart_area.left() + i
            normalized_brake = brake / 100  # 0-1 範圍
    ...
    def draw_throttle_curve(self, painter, chart_area):
        ...
        for i in range(0, chart_area.width(), 2):
            ...
            # 模擬節流閥位置 (0-100%)
            throttle = 50 + 40 * math.cos(real_x * 0.015) + 20 * math.sin(real_x * 0.04) + random.uniform(-5, 5)
            ...
            x_pos = chart_area.left() + i
            normalized_throttle = throttle / 100  # 0-1 範圍
    ...
    def draw_steering_curve(self, painter, chart_area):
        ...
        for i in range(0, chart_area.width(), 2):
            ...
            # 模擬方向盤轉角 (-100 到 +100 度)
            steering = 30 * math.sin(real_x * 0.03) + 20 * math.cos(real_x * 0.07) + random.uniform(-3, 3)
    ...
    def draw_y_value_at_fixed_line(self, painter, chart_area):
        ...
        print(f"📊 顯示固定值標籤: {value_text} at ({label_x}, {label_y})")  # Debug
...
class DraggableTitleBar(QWidget):
    ...
    def __init__(self, parent_window, title=""):
        ...
        print(f"🎨 DEBUG: Creating CustomTitleBar with title: '{title}'")
        print(f"📋 ObjectName set to: {self.objectName()}")
    ...
    def restore_normal_size(self):
        """恢復視窗到正常大小"""
        print(f"🔄 恢復視窗 '{self.parent_window.windowTitle()}' 到正常大小")
    ...
    def contextMenuEvent(self, event):
        """右鍵選單"""
        menu = QMenu(self)
        restore_action = menu.addAction("🔄 恢復正常大小")
    ...
    def paintEvent(self, event):
        """繪製事件 - 手動繪製背景色以確保顯示"""
        print(f"🎨 DEBUG: CustomTitleBar paintEvent called")
        print(f"📋 ObjectName: {self.objectName()}")
        print(f"📐 Widget size: {self.width()}x{self.height()}")
        print(f"🎨 Current QSS length: {len(self.styleSheet())}")
        if self.styleSheet():
            print(f"🎨 QSS content (first 100 chars): {self.styleSheet()[:100]}...")
        else:
            print("⚠️ No QSS applied to CustomTitleBar")
        ...
        print(f"🎨 Manually painted background with #F0F0F0 (extended rect)")
...
class PopoutSubWindow(QMdiSubWindow):
    ...
    def __init__(self, title="", parent_mdi=None):
        super().__init__()
        print(f"🚀 DEBUG: Creating PopoutSubWindow '{title}'")
        ...
        # 🔥 測試：暫時移除 FramelessWindowHint 看看邊框是否顯示
        # self.setWindowFlags(Qt.SubWindow | Qt.FramelessWindowHint)
        self.setWindowFlags(Qt.SubWindow)
        print(f"🏷️ Window flags set (WITHOUT FramelessWindowHint): {self.windowFlags()}")
        ...
        print(f"✅ Direct QSS applied to subwindow: {len(subwindow_qss)} characters")
        print(f"🎨 QSS content: {subwindow_qss[:100]}...")
    ...
    def paintEvent(self, event):
        ...
        print(f"🎨 DEBUG: PopoutSubWindow paintEvent called for {self.windowTitle()}")
        print(f"📐 Window size: {self.width()}x{self.height()}")
        print(f"📍 Window position: ({self.x()}, {self.y()})")
        print(f"🔲 Window rect: {self.rect()}")
        print(f"🎭 Window frameless: {self.windowFlags() & Qt.FramelessWindowHint}")
        print(f"🎨 Self QSS length: {len(self.styleSheet())}")
    ...
    def setWidget(self, widget):
        """設置內容小部件並添加彈出按鈕"""
        print(f"🔧 DEBUG: PopoutSubWindow.setWidget called for {self.windowTitle()}")
        ...
        print(f"📦 Wrapper margins set to: 0px (標題欄延伸到邊緣)")
        print(f"🎨 Wrapper ObjectName: {wrapper.objectName()}")
        ...
        print(f"🎨 DEBUG: Applied QSS to CustomTitleBar: {len(self.styleSheet())} characters")
    ...
    def setMinimumSize(self, *args):
        ...
        if len(args) == 1:  # QSize 參數
            size = args[0]
            print(f"🚨 setMinimumSize 被調用: {size.width()}x{size.height()}")
        elif len(args) == 2:  # width, height 參數
            width, height = args
            print(f"🚨 setMinimumSize 被調用: {width}x{height}")
    ...
    def minimumSize(self):
        ...
        # print(f"🔍 minimumSize 被查詢，強制返回: {forced_size.width()}x{forced_size.height()}")
        return forced_size
    ...
    def minimumSizeHint(self):
        ...
        from PyQt5.QtCore import QSize
        forced_size = QSize(250, 150)
        # print(f"🔍 minimumSizeHint 被查詢，強制返回: {forced_size.width()}x{forced_size.height()}")
        return forced_size
        
    def resizeEvent(self, event):
        """處理窗口縮放事件，確保不會小於最小尺寸"""
        print(f"🔧 PopoutSubWindow: resizeEvent 被調用，新尺寸: {event.size().width()}x{event.size().height()}")
        super().resizeEvent(event)
        
        # 🔥 強制檢查最小尺寸限制（不依賴 minimumSize()）
        MIN_WIDTH = 250
        MIN_HEIGHT = 150
        ...
        if current_size.width() < MIN_WIDTH:
            new_width = MIN_WIDTH
            needs_resize = True
            print(f"⚠️ 寬度低於最小值，調整: {current_size.width()} -> {new_width}")
            
        if current_size.height() < MIN_HEIGHT:
            new_height = MIN_HEIGHT
            needs_resize = True
            print(f"⚠️ 高度低於最小值，調整: {current_size.height()} -> {new_height}")
    ...
    def on_year_changed(self, year):
        """處理年份變更"""
        window_title = self.windowTitle()
        print(f"📅 [{window_title}] 年份變更為: {year}")
    ...
    def on_race_changed(self, race):
        """處理賽事變更"""
        window_title = self.windowTitle()
        print(f"🏁 [{window_title}] 賽事變更為: {race}")
    ...
    def on_session_changed(self, session):
        """處理賽段變更"""
        window_title = self.windowTitle()
        print(f"🏎️ [{window_title}] 賽段變更為: {session}")
    ...
    def minimize_to_bottom(self):
        ...
        if self.parent():
            ...
            print(f"📍 移動到底部位置: ({new_x}, {bottom_y}, {current_width}, {title_height})")
        else:
            # 如果沒有父視窗，只調整高度
    ...
    def restore_from_minimize(self):
        ...
        if self.original_geometry is not None:
            ...
            print(f"📍 恢復到原始位置: {self.original_geometry}")
        else:
            print(f"⚠️ 無法恢復：原始幾何形狀未保存")
...
class ContextMenuTreeWidget(QTreeWidget):
    ...
    def show_context_menu(self, position):
        ...
        if item.childCount() == 0:
            ...
            export_action = menu.addAction("📊 匯出數據")
...
class StyleHMainWindow(QMainWindow):
    ...
    def create_professional_toolbar(self):
        ...
        self.session_combo.addItems(["R", "Q", "P1", "P2", "P3", "Sprint"])
        self.session_combo.setCurrentText("R")
        self.session_combo.setFixedWidth(50)
        ...
        toolbar.addAction("⚡", self.driver_comparison)
        toolbar.addSeparator()
    ...
    def init_default_tabs(self):
        """初始化預設分頁"""
        # 分頁1: 遙測分析
        telemetry_tab = self.create_telemetry_analysis_tab()
        self.tab_widget.addTab(telemetry_tab, "📈 遙測分析")
        
        # 分頁2: 圈速比較
        laptime_tab = self.create_laptime_comparison_tab()
        self.tab_widget.addTab(laptime_tab, "🏁 圈速比較")
        
        # 分頁3: 賽道分析
        track_tab = self.create_track_analysis_tab()
        self.tab_widget.addTab(track_tab, "🗺️ 賽道分析")
    ...
    def add_new_tab(self):
        ...
        tab_types = [
            ("📈 遙測分析", self.create_telemetry_analysis_tab),
            ("🏁 圈速比較", self.create_laptime_comparison_tab),
            ("🗺️ 賽道分析", self.create_track_analysis_tab),
            ("📊 數據總覽", self.create_data_overview_tab)
        ]
    ...
    def force_black_background(self, mdi_area):
        """深度修復QMdiArea灰底問題"""
        print(f"🎨 DEBUG: force_black_background called for {mdi_area.objectName()}")
        ...
        print(f"✅ Palette set for {mdi_area.objectName()}")
        
        # 方法2: 直接設置背景畫筆
        mdi_area.setBackground(QBrush(QColor(0, 0, 0)))
        print(f"✅ Background brush set for {mdi_area.objectName()}")
        
        # 方法3: 設置viewport背景（QMdiArea內部使用QScrollArea）
        def fix_viewport():
            try:
                print(f"🔧 Fixing viewport for {mdi_area.objectName()}")
                # 查找內部的viewport小部件
                child_count = 0
                ...
                print(f"📦 Fixed {child_count} child widgets")
    ...
    def create_telemetry_analysis_tab(self):
        ...
        print(f"🎨 MDI QSS length after addSubWindow: {len(mdi_area.styleSheet())}")
        print(f"🎨 speed_window QSS length after addSubWindow: {len(speed_window.styleSheet())}")
    ...
    def create_professional_status_bar(self):
        ...
        session_label = QLabel("📊 Japan 2025 Race")
        drivers_label = QLabel("🏁 VER vs LEC")
    ...
    def update_status_bar(self):
        ...
        if hasattr(self, 'year_combo') and hasattr(self, 'race_combo') and hasattr(self, 'session_combo'):
            ...
            # 更新狀態列中的會話信息
            self.findChild(QLabel).setText(f"📊 {race} {year} {session}")
    ...
    def reset_all_charts(self, mdi_area):
        ...
        try:
            print(f"🔄 開始調整 MDI 區域中的所有圖表以顯示完整數據...")
            
            # 獲取所有子視窗
            subwindows = mdi_area.subWindowList()
            reset_count = 0
            
            print(f"📊 MDI區域中共有 {len(subwindows)} 個子視窗")
                ...
                if subwindow and subwindow.widget():
                    ...
                    if telemetry_widgets:
                        for telemetry_widget in telemetry_widgets:
                            print(f"🎯 調整遙測圖表以顯示完整數據: {telemetry_widget.chart_type}")
                            ...
                            if chart_width > 0 and chart_height > 0:
                                ...
                                if x_data is not None and y_data is not None and len(x_data) > 0 and len(y_data) > 0:
                                    ...
                                    print(f"📊 數據範圍分析 {telemetry_widget.chart_type}:")
                                    ...
                                    print(f"⚠️ 無法獲取 {telemetry_widget.chart_type} 的數據範圍，使用預設縮放")
                                ...
                                print(f"✅ 調整完成 {telemetry_widget.chart_type} - X縮放: {telemetry_widget.x_scale:.2f}, Y縮放: {telemetry_widget.y_scale:.2f}, X偏移: {telemetry_widget.x_offset:.1f}, Y偏移: {telemetry_widget.y_offset:.1f}")
                            else:
                                print(f"⚠️ 圖表 {telemetry_widget.chart_type} 尺寸無效，跳過調整")
                    
                    # 檢查是否為其他類型的圖表或可縮放小部件
                    elif hasattr(widget, 'fit_to_view'):
                        # 如果小部件有適合視圖的方法
                        print(f"🔧 使用 fit_to_view 方法調整: {widget_type}")
                        widget.fit_to_view()
                        reset_count += 1
                        
                    elif hasattr(widget, 'zoom_to_fit'):
                        # 如果小部件有縮放適應方法
                        print(f"🔧 使用 zoom_to_fit 方法調整: {widget_type}")
                        widget.zoom_to_fit()
                        reset_count += 1
                    else:
                        print(f"⚠️ 視窗 {i+1} 中沒有找到可調整的圖表組件")
                else:
                    print(f"⚠️ 視窗 {i+1} 沒有有效的widget")
            
            print(f"✅ 調整完成！共調整了 {reset_count} 個圖表以顯示完整數據")
            
        except Exception as e:
            print(f"❌ 調整圖表時發生錯誤: {e}")
            import traceback
            traceback.print_exc()    # 事件處理方法
    ...
    def tile_windows(self):
        ...
        if mdi_area is None:
            print("❌ 當前分頁中沒有找到MDI區域")
            return
            
        # 獲取所有子視窗
        subwindows = mdi_area.subWindowList()
        if not subwindows:
            print("❌ MDI區域中沒有子視窗需要排列")
            return
            
        print(f"📊 開始重新排列 {len(subwindows)} 個子視窗")
        ...
        for i, subwindow in enumerate(subwindows):
            ...
            print(f"🔧 視窗 {i+1}: '{subwindow.windowTitle()}' 移動到 ({x}, {y}) 尺寸 {window_width}x{window_height}")
        
        # 刷新MDI區域
        mdi_area.update()
        print(f"✅ 成功重新排列 {len(subwindows)} 個視窗")
    def cascade_windows(self):
        ...
        subwindows = mdi_area.subWindowList()
        if not subwindows:
            print("❌ MDI區域中沒有子視窗需要層疊")
            return
            
        print(f"📊 開始層疊排列 {len(subwindows)} 個子視窗")
        ...
        for i, subwindow in enumerate(subwindows):
            ...
            print(f"🔧 視窗 {i+1}: '{subwindow.windowTitle()}' 層疊到 ({x}, {y}) 尺寸 {base_width}x{base_height}")
        ...
        # 刷新MDI區域
        mdi_area.update()
        print(f"✅ 成功層疊排列 {len(subwindows)} 個視窗")
    ...
    def minimize_all_windows(self):
        ...
        for subwindow in subwindows:
            ...
            count += 1
            print(f"📉 最小化視窗: '{subwindow.windowTitle()}'")
            
        print(f"✅ 成功最小化 {count} 個視窗")
    ...
    def maximize_all_windows(self):
        ...
        for subwindow in subwindows:
            ...
            count += 1
            print(f"📈 最大化視窗: '{subwindow.windowTitle()}'")
            
        print(f"✅ 成功最大化 {count} 個視窗")
    ...
    def restore_all_windows(self):
        ...
        for subwindow in subwindows:
            subwindow.showNormal()
            count += 1
            print(f"🔄 還原視窗: '{subwindow.windowTitle()}'")
            
        print(f"✅ 成功還原 {count} 個視窗")
    ...
    def apply_style_h(self):
        ...
        print("🎨 DEBUG: Setting main window QSS styles...")
        ...
        print("✅ QSS styles applied successfully")

def main():
    """主函數"""
    app = QApplication(sys.argv)
    app.setApplicationName("F1T GUI Demo - Style H")
    ...
    # 顯示歡迎訊息
    print("🏁 F1T GUI Demo - 風格H (專業賽車分析工作站) 已啟動")
    print("🎯 移除參數面板，右鍵功能選單，系統日誌，曲線圖表")
    print("🗺️ 包含賽道地圖，純黑底設計，專業視窗控制")
