#!/usr/bin/env python3
"""
SeasonStartReactionOpener - 年度起跑反應分析模組開啟器
...
class SeasonStartReactionOpener:
    """年度起跑反應分析模組開啟器"""
    
    def __init__(self, main_window):
        self.main_window = main_window
    
    def open_season_start_reaction_module(self):
        """開啟年度起跑反應分析 MDI 視窗"""
        try:
            logger.info("[SEASON_START_REACTION_OPENER] Opening module...")
            ...
            if not current_tab:
                logger.error("[SEASON_START_REACTION_OPENER] No current tab")
                return
            ...
            if mdi_area is None:
                logger.error("[SEASON_START_REACTION_OPENER] Cannot find MDI area")
                return
            
            # 獲取當前年份
            current_year = self.main_window.get_selected_year()
            
            logger.info(f"[SEASON_START_REACTION_OPENER] Year: {current_year}")
            
            # 導入並創建模組
            from modules.gui.multi_season.season_start_reaction import SeasonStartReactionAnalysis
            
            # 創建模組實例 (initialization happens in __init__)
            module = SeasonStartReactionAnalysis(year=current_year)
            
            # Verify module was initialized successfully
            if not hasattr(module, 'main_widget') or module.main_widget is None:
                logger.error("[SEASON_START_REACTION_OPENER] Module has no main_widget")
            ...
            # 創建 MDI 子視窗
            title = f"{tr('season_start_reaction', 'Season Start Reaction')} - {current_year}"
            ...
            try:
                ...
                logger.info("[SEASON_START_REACTION_OPENER] Data load triggered")
            except Exception as e:
                logger.warning(f"[SEASON_START_REACTION_OPENER] Data load error: {e}")
            ...
            try:
                ...
                module_id = f"season_start_reaction_{id(module)}"
                manager.register_module(module_id, module, "season_start_reaction")
                logger.debug(f"[SEASON_START_REACTION_OPENER] Registered module: {module_id}")
            except Exception as e:
                logger.warning(f"[SEASON_START_REACTION_OPENER] Failed to register to manager: {e}")
            
            logger.info("[SEASON_START_REACTION_OPENER] Module opened successfully")
            
        except ImportError as e:
            logger.error(f"[SEASON_START_REACTION_OPENER] Import error: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.critical(
                self.main_window,
            ...
            logger.error(f"[SEASON_START_REACTION_OPENER] Error: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.critical(
                self.main_window,
