    ...
    def _create_analysis_module(self, function_name, module_type_hint: Optional[str] = None):
        ...
        try:
            ...
            module_alias_groups = {
                ...
                "temp_analysis": [
                    ("temp_analysis", "Temperature Analysis"),
                    "temp_weather",  # Workspace 使用的原始 key
                    "溫度分析",
            ...
            logger.debug(f"[MODULE_FACTORY] temp_analysis 在字典中: {'temp_analysis' in module_alias_groups}")
            if 'temp_analysis' in module_alias_groups:
                logger.debug(f"[MODULE_FACTORY] temp_analysis 別名數量: {len(module_alias_groups['temp_analysis'])}")

            module_mapping = {}
            
            # 調試點 1：確認開始建立映射表
            logger.debug(f"[DEBUG] [MODULE_FACTORY] 步驟1: 開始建立映射表")
            logger.debug(f"[DEBUG] [MODULE_FACTORY] temp_analysis 原始別名數量: {len(module_alias_groups.get('temp_analysis', []))}")

            def _register_module_alias(alias_value, module_type):
                # 調試：顯示註冊過程
                if module_type == "temp_analysis":
            ...
            for module_type, aliases in module_alias_groups.items():
                # 調試點 2：顯示正在處理的模組類型
                if module_type == "temp_analysis":
                    logger.debug(f"[DEBUG] [MODULE_FACTORY] 步驟2: 處理 temp_analysis，別名數量={len(aliases)}")
                    logger.debug(f"[DEBUG] [MODULE_FACTORY] temp_analysis 別名內容: {aliases}")
            ...
            if 'temp_weather' in module_mapping:
                logger.debug(f"[DEBUG] [MODULE_FACTORY] 'temp_weather' 在映射表中: {module_mapping['temp_weather']}")
            else:
                logger.debug(f"[DEBUG] [MODULE_FACTORY] 'temp_weather' 不在映射表中！")
                logger.debug(f"[DEBUG] [MODULE_FACTORY] temp_analysis 別名: {module_alias_groups.get('temp_analysis', [])}")
            ...
            if module_type:
                ...
                # 處理溫度天氣分析模組
                elif module_type == "temp_analysis":
                    try:
                        logger.debug(f"[DEBUG]    [MODULE_FACTORY] 開始創建溫度天氣分析模組...")
                        from modules.gui.race_analysis.temp.temp_analysis_mdi import TempAnalysisUniversal
                        logger.debug(f"[OK] [MODULE_FACTORY] 溫度天氣分析模組導入成功（使用新版 Universal）")
                        ...
                        if parameter_provider:
                            ...
                            logger.debug(f"[INIT] [MODULE_FACTORY] 溫度天氣分析模組參數: {current_year} {current_race} {current_session}")
                            
                            # 創建模組實例（使用新版 Universal MDI）
                            module = TempAnalysisUniversal(
                                year=current_year,
                                race=current_race,
                                session=current_session
                            )
                            logger.debug(f"[OK] 溫度天氣分析模組初始化成功（Universal MDI）")
                            return self.main_window._mark_module_factory_type(module, module_type)
                        else:
                            logger.error(f"[ERROR] 溫度天氣分析模組創建失敗：無參數")
                            return None
                    except Exception as e:
                        logger.error(f"[ERROR] 溫度天氣分析模組創建失敗: {e}")
                        import traceback
                        traceback.print_exc()
                        return None
                ...
                # 處理 FP2→Q 排位賽預測表格模組 ⭐ F76 新增
                elif module_type == "fp2_qualifying_prediction_table":
                    try:
                        logger.debug(f"[DEBUG]    [MODULE_FACTORY] 開始創建 FP2→Q 排位賽預測表格模組 (F76)...")
                        from modules.gui.fp2_qualifying_prediction.fp2_qualifying_prediction_mdi import (
                            FP2QualifyingPredictionMDI
                        )
                        logger.debug(f"[OK] [MODULE_FACTORY] FP2→Q 排位賽預測 MDI 導入成功")
                        
                        # 創建 MDI 實例
                        module = FP2QualifyingPredictionMDI(parent=self.main_window)
                        logger.debug(f"[OK] [MODULE_FACTORY] FP2→Q 排位賽預測 MDI 實例創建成功")
                        ...
                        if parameter_provider:
                            ...
                            logger.debug(f"[INIT] [MODULE_FACTORY] FP2→Q 排位賽預測模組參數預設為: {current_year} {current_race} {current_session}")
                        ...
                        # 初始化模組
                        if not module.initialize_module():
                            logger.error(f"[ERROR] [MODULE_FACTORY] FP2→Q 排位賽預測模組初始化失敗")
                            return None
                        
                        logger.debug(f"[OK] [MODULE_FACTORY] FP2→Q 排位賽預測模組初始化成功")
                        return self.main_window._mark_module_factory_type(module, module_type)
                    except Exception as e:
                        logger.error(f"[ERROR] [MODULE_FACTORY] FP2→Q 排位賽預測模組創建失敗: {e}")
                        import traceback
                        traceback.print_exc()
                        return None
