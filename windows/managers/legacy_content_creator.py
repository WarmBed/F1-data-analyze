...
class LegacyContentCreator:
    ...
    def _create_legacy_content(self, function_name):
        """創建舊版內容 - 保持向後相容性"""
        # 根據功能類型創建相應的內容
        if "降雨分析" in function_name or "溫度分析" in function_name:
            # 使用新的溫度分析模組 (Universal MDI 架構)
            try:
                ...
                content = TempAnalysisUniversal(
                    year=params['year'],
                    race=params['race'],
                    session=params['session']
                )
                logger.debug(f"[OK] 已載入溫度分析模組 (Universal MDI) - {params['year']} {params['race']} {params['session']}")
                return content
