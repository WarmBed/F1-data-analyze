...
class driverLapUniversalDataLoader(UniversalDataLoader):
    ...
    @classmethod
    def _ensure_laptime_registered(cls):
        """確保 laptime 分析類型已註冊（類方法）"""
    ...
    def __init__(self, parent=None):
        # 使用類方法確保 laptime 類型已註冊
        self.__class__._ensure_laptime_registered()
        
        # 初始化 UniversalDataLoader 基類
        super().__init__(analysis_type='laptime', parent=parent)
    
    def load_from_parameters(self, **kwargs):
        """從參數載入數據（兼容性方法）"""
        try:
            print(f"[F28_UNIVERSAL] 從參數載入數據: {kwargs}")
            
            # 調用基類的 load_data 方法
            success = self.load_data(**kwargs)
            
            if success:
                print(f"[F28_UNIVERSAL] ✅ 數據載入成功")
            else:
                print(f"[F28_UNIVERSAL] ⚠️ 數據載入失敗")
                
            return success
            
        except Exception as e:
            print(f"[F28_UNIVERSAL] ❌ 載入參數錯誤: {e}")
            import traceback
            traceback.print_exc()
            self.load_error.emit(f"載入參數錯誤: {str(e)}")
            return False
...
class driverLapDataLoader(QObject):
        ...
        try:
            ...
            if hasattr(self.universal_loader, 'load_from_parameters'):
                ...
                if os.path.exists(json_path):
                    ...
                    print(f"[F28_DATA] 🚀 啟動 CLI Function 28 自動生成")
                    # 啟動 CLI 自動生成機制
                    self._start_cli_generation(year=year, race=race, session=session, driver=driver)
    ...
    def _start_cli_generation(self, year: int, race: str, session: str, driver: str = "all_drivers"):
        """啟動 CLI Function 28 數據生成"""
        try:
            print(f"[F28_DATA] 🚀 啟動 CLI Function 28 數據生成")
            print(f"   參數: year={year}, race={race}, session={session}, driver={driver}")
            
            # 直接創建 CLI 工作器 (不依賴基類方法)
            from modules.gui.base.universal_data_loader_base import CliAnalysisWorker
            self.cli_worker = CliAnalysisWorker(year, race, session, force_mode=28, parent=self)
            ...
            # 連接 CLI 工作器信號
            def on_cli_finished():
                print(f"[F28_DATA] CLI 執行完成，重新載入數據")
                # 重新載入數據
                self.load_from_parameters(**self._generation_params)
            
            self.cli_worker.analysis_completed.connect(on_cli_finished)
            self.cli_worker.progress_updated.connect(self._on_cli_progress_updated)
            
            # 發射狀態變化信號
            self.status_changed.emit("正在生成詳細圈速分析數據...")
            
            # 啟動生成
            self.cli_worker.start()
            
        except Exception as e:
            print(f"[F28_DATA] ❌ CLI 生成啟動失敗: {e}")
            import traceback
            traceback.print_exc()
            self.load_error.emit(f"CLI 生成啟動失敗: {str(e)}")
    
    def _start_generation_monitoring(self, year: int, race: str, session: str, driver: str):
        """啟動生成監控系統"""
        try:
            print(f"[F28_DATA] 📡 啟動生成監控系統")
            ...
            self._generation_monitor_timer = QTimer(self)
            self._generation_monitor_timer.timeout.connect(self._check_generation_progress)
            self._generation_monitor_timer.start(5000)  # 5秒
            ...
            self._generation_timeout_timer.start(180000)  # 180秒
            
            print(f"[F28_DATA] ✅ 監控系統已啟動")
            
        except Exception as e:
            print(f"[F28_DATA] ❌ 監控系統啟動失敗: {e}")
    
    def _check_generation_progress(self):
        """檢查生成進度"""
        try:
            if not hasattr(self, '_monitoring_params'):
                return
                
            params = self._monitoring_params
            json_filename = f"detailed_laptime_analysis_{params['year']}_{params['race']}_{params['session']}_{params['driver']}.json"
            json_path = os.path.join("json", json_filename)
            
            print(f"[F28_DATA] 📊 檢查生成進度: {json_path}")
            
            if os.path.exists(json_path):
                print(f"[F28_DATA] ✅ 檔案生成完成，停止監控")
                self._stop_generation_monitoring()
                
                # 載入生成的檔案
                self.load_from_json(json_path)
            else:
                print(f"[F28_DATA] ⏳ 檔案尚未生成，繼續監控...")
                
        except Exception as e:
            print(f"[F28_DATA] ❌ 監控檢查錯誤: {e}")
    
    def _stop_generation_monitoring(self):
        """停止生成監控"""
        try:
            print(f"[F28_DATA] 🛑 停止生成監控")
            
            if hasattr(self, '_generation_monitor_timer'):
                self._generation_monitor_timer.stop()
                self._generation_monitor_timer.deleteLater()
                delattr(self, '_generation_monitor_timer')
                
            if hasattr(self, '_generation_timeout_timer'):
                self._generation_timeout_timer.stop()
                self._generation_timeout_timer.deleteLater()
                delattr(self, '_generation_timeout_timer')
                
            if hasattr(self, '_monitoring_params'):
                delattr(self, '_monitoring_params')
                
            print(f"[F28_DATA] ✅ 監控系統已完全停止")
            
        except Exception as e:
            print(f"[F28_DATA] ❌ 停止監控錯誤: {e}")
    
    def _on_cli_generation_completed(self, success: bool, message: str):
        """處理 CLI 生成完成"""
        try:
            print(f"[F28_DATA] 📥 CLI 生成完成: success={success}, message={message}")
            
            if not success:
                print(f"[F28_DATA] ❌ CLI 生成失敗: {message}")
                self._stop_generation_monitoring()
                self.load_error.emit(f"數據生成失敗: {message}")
            else:
                print(f"[F28_DATA] ✅ CLI 生成成功")
                # 監控系統會自動檢測到檔案並載入
                
        except Exception as e:
            print(f"[F28_DATA] ❌ CLI 完成處理錯誤: {e}")
    
    def _on_cli_progress_updated(self, progress_message: str):
        """處理 CLI 進度更新"""
        try:
            print(f"[F28_DATA] 📊 CLI 進度: {progress_message}")
            self.status_changed.emit(f"生成進度: {progress_message}")
        except Exception as e:
            print(f"[F28_DATA] ❌ 進度更新處理錯誤: {e}")
    
    def _on_generation_timeout(self):
        """處理生成超時"""
        try:
            print(f"[F28_DATA] ⏰ 生成超時 (180秒)")
            self._stop_generation_monitoring()
            self.load_error.emit("數據生成超時，請檢查網路連線或重試")
        except Exception as e:
            print(f"[F28_DATA] ❌ 超時處理錯誤: {e}")
