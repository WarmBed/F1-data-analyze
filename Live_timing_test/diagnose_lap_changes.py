...
print(f"總記錄數: {len(sorted_timing)}")
    ...
    if '1' in lines:
        ...
        if lap_num != prev_lap:
            if change_count < 20:  # 只顯示前20次變化
                print(f"  [{i}] {timestamp}: {prev_lap} → {lap_num}")
            change_count += 1
            prev_lap = lap_num
...
for i, record in enumerate(sorted_timing):
    ...
    if '1' in lines:
        ...
        if lap_num == 53:
            timestamp = record.get('timestamp')
            print(f"  首次出現在 [{i}] {timestamp}")
            print(f"  這是第 {i+1}/{len(sorted_timing)} 筆記錄 ({(i+1)/len(sorted_timing)*100:.1f}%)")
            break
