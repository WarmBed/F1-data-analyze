...
print(f"  First: {lines1[0][:12]}")
print(f"  Last:  {lines1[-1][:12]}")

# TimingData  
print("\nTimingData time range:")
...
print(f"  First: {lines2[0][:12]}")
print(f"  Last:  {lines2[-1][:12]}")

# 檢查 Lap 10 附近的 CarData
print("\nCarData around 01:11:54 (Lap 10 start):")
...
for line in lines1:
    ...
    if timedelta(hours=1, minutes=11, seconds=50) <= ts <= timedelta(hours=1, minutes=12, seconds=0):
        ...
        if decoded and 'Entries' in decoded:
            ...
            cars = entry.get('Cars', {})
            has_44 = '44' in cars
            print(f"  {line[:12]} | Cars: {len(cars)} | Has 44: {has_44}")
            if has_44:
                print(f"    -> Speed: {cars['44']['Channels'].get('0', 'N/A')}")
            break
