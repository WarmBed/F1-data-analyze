...
logger.info(f"  總行數: {len(pos_lines)}")
logger.info(f"  第一行時間戳: {pos_lines[0][:12]}")
logger.info(f"  第 100 行時間戳: {pos_lines[99][:12] if len(pos_lines) > 99 else 'N/A'}")
logger.info(f"  最後一行時間戳: {pos_lines[-1][:12]}")
...
logger.info(f"  總行數: {len(timing_lines)}")
logger.info(f"  第一行時間戳: {timing_lines[0][:12]}")
logger.info(f"  第 100 行時間戳: {timing_lines[99][:12] if len(timing_lines) > 99 else 'N/A'}")
logger.info(f"  最後一行時間戳: {timing_lines[-1][:12]}")

# 檢查 TimingData 第一行內容
try:
    first_timing_data = json.loads(timing_lines[0][12:])
    if 'Lines' in first_timing_data:
        logger.info(f"  車手數量: {len(first_timing_data['Lines'])}")
        sample_driver = list(first_timing_data['Lines'].keys())[0]
        driver_data = first_timing_data['Lines'][sample_driver]
        logger.info(f"  範例車手 {sample_driver}:")
        logger.info(f"    圈數: {driver_data.get('NumberOfLaps', 'N/A')}")
        logger.info(f"    排名: {driver_data.get('Position', 'N/A')}")
...
logger.info(f"  總行數: {len(cardata_lines)}")
logger.info(f"  第一行時間戳: {cardata_lines[0][:12]}")
logger.info(f"  第 100 行時間戳: {cardata_lines[99][:12] if len(cardata_lines) > 99 else 'N/A'}")
logger.info(f"  最後一行時間戳: {cardata_lines[-1][:12]}")
...
logger.info(f"\n⏰ 時間分析:")
logger.info(f"  Position 第一筆: {pos_lines[0][:12]} ({pos_first_sec:.1f}s)")
logger.info(f"  Position 最後一筆: {pos_lines[-1][:12]} ({pos_last_sec:.1f}s)")
logger.info(f"  Position 時長: {pos_duration:.1f}s = {pos_duration/60:.1f}分鐘")
...
logger.info(f"  Timing 最後一筆: {timing_lines[-1][:12]} ({timing_last_sec:.1f}s)")
logger.info(f"  時間差: Position vs Timing 起始 = {pos_first_sec - timing_first_sec:.1f}s")
