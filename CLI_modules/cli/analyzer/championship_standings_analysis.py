...
def generate_championship_standings(
    ...
    *,
    ...
    min_supported_year = 2018  # FastF1/Ergast 支援的最早年份
    
    if target_year < min_supported_year:
        return {
            "success": False,
            "message": f"不支援 {target_year} 年的數據（最早支援 {min_supported_year} 年）",
            "data": None,
            "function_id": "97",
        }
    ...
    if target_year > current_year:
        return {
            "success": False,
            "message": f"{target_year} 年賽季尚未開始，數據暫不可用（當前年份：{current_year}）",
            "data": None,
            "function_id": "97",
        }
