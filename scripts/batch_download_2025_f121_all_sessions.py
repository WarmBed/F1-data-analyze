"""
批次下載 2025 年所有賽事的 F121 直線速度全圈數分析數據

功能：F121 - 直線速度全圈數分析（官方 API 版本）
會話類型：FP1, FP2, FP3, Q (排位賽), R (正賽)
如果會話不存在則自動跳過

執行方式：
    python batch_download_2025_f121_all_sessions.py
"""
import sys
import time
import subprocess
...
def main():
    """主執行函數"""
    print("=" * 80)
    print("🏎️  F1 2025 全賽季 F121 直線速度數據下載腳本")
    print("=" * 80)
    print(f"📅 目標年份: 2025")
    print(f"🏁 賽事數量: {len(RACES_2025)}")
    print(f"📊 會話類型: {', '.join(SESSIONS)}")
    print(f"🔧 分析功能: F121 直線速度全圈數分析（官方 API 版本）")
    print("=" * 80)
    print()
    ...
    # 儲存結果報告
    report_path = Path("batch_download_2025_f121_report.txt")
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(f"F1 2025 全賽季 F121 直線速度數據下載報告\n")
